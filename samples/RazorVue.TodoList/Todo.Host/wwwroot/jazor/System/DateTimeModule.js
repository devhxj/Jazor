...
import { JDateOnly, JDateTime, JTimeSpan, RuntimeModule } from "System/RuntimeModule.js";
import { _5ad63706a889c294 } from "System/StringModule.js";
import { TimeOnlyModule } from "System/TimeOnlyModule.js";
...
function createDefaultDateTime() {
  return new JDateTime(createLocalDate(1, 1, 1), dateTimeKindUnspecified);
}
function createLocalDate(year, month, day) {
  return RuntimeModule.createLocalDate(year, month, day);
}
function createLocalDateTime(year, month, day, hour, minute, second, millisecond) {
  return RuntimeModule.createLocalDateTime(year, month, day, hour, minute, second, millisecond);
}
...
function CreateFromTicks(ticks, kind) {
  ...
  let ticksSinceUnixEpoch = ticks - unixEpochTicks;
  let milliseconds = ticksSinceUnixEpoch / ticksPerMillisecond;
  let subMillisecondTicks = ticksSinceUnixEpoch % ticksPerMillisecond;
...
function getMicrosecondTicks(microsecond) {
  ...
  return BigInt(microsecond) * ticksPerMicrosecond;
}
function getTicks(instance) {
  ...
  return BigInt(milliseconds) * ticksPerMillisecond + instance.subMillisecondTicks + unixEpochTicks;
}
function GetTicks(date) {
  let milliseconds = Date.UTC(date.getFullYear(), date.getMonth(), date.getDate(), date.getHours(), date.getMinutes(), date.getSeconds(), date.getMilliseconds());
  return BigInt(milliseconds) * ticksPerMillisecond + unixEpochTicks;
}
function getInstantTicks(instance) {
  if (instance.kind === dateTimeKindUtc)
    return getTicks(instance);
  return BigInt(instance.date.getTime()) * ticksPerMillisecond + instance.subMillisecondTicks + unixEpochTicks;
}
function createUtcNow() {
  let now = new Date;
  return new JDateTime(createLocalDateTime(now.getUTCFullYear(), now.getUTCMonth() + 1, now.getUTCDate(), now.getUTCHours(), now.getUTCMinutes(), now.getUTCSeconds(), now.getUTCMilliseconds()), dateTimeKindUtc);
}
...
function getLocalizedDayPeriod(date, locale) {
  ...
  let parts = new DateTimeFormat(locale, { hour: 0, hour12: true }).formatToParts(date);
  for (let i = 0; i < parts.length; i++) {
    let part = parts[i];
...
function getRoundtripSuffix(instance) {
  ...
  if (instance.kind === dateTimeKindLocal)
    return formatOffsetTicks(BigInt(-instance.date.getTimezoneOffset()) * offsetMinuteTicks, 3);
  return "";
}
function formatInvariantGeneralDateTime(instance, includeSeconds) {
  let date = instance.date;
  let text = RuntimeModule.pad2(date.getMonth() + 1) + "/" + RuntimeModule.pad2(date.getDate()) + "/" + RuntimeModule.padLeft(date.getFullYear().toString(), 4) + " " + RuntimeModule.pad2(date.getHours()) + ":" + RuntimeModule.pad2(date.getMinutes());
  if (includeSeconds)
...
function formatInvariantShortDate(instance) {
  let date = instance.date;
  return RuntimeModule.pad2(date.getMonth() + 1) + "/" + RuntimeModule.pad2(date.getDate()) + "/" + RuntimeModule.padLeft(date.getFullYear().toString(), 4);
}
function formatInvariantLongDate(instance) {
  let date = instance.date;
  return getInvariantDayName(date.getDay()) + ", " + RuntimeModule.pad2(date.getDate()) + " " + getInvariantMonthName(date.getMonth() + 1) + " " + RuntimeModule.padLeft(date.getFullYear().toString(), 4);
}
function formatInvariantTime(instance, includeSeconds) {
  let date = instance.date;
  let text = RuntimeModule.pad2(date.getHours()) + ":" + RuntimeModule.pad2(date.getMinutes());
  if (includeSeconds)
    text += ":" + RuntimeModule.pad2(date.getSeconds());
  return text;
}
function formatMonthDay(instance, provider) {
  ...
  return formatLocaleDateTime(instance.date, locale, { month: 0, day: 1 });
}
function formatYearMonth(instance, provider) {
  ...
  return formatLocaleDateTime(instance.date, locale, { year: 0, month: 0 });
}
...
function formatUniversalFullDateTime(instance, provider) {
  ...
  let locale = getProviderLocale(provider);
  if (locale.length === 0) {
    return getInvariantDayName(utc.getUTCDay()) + ", " + RuntimeModule.pad2(utc.getUTCDate()) + " " + getInvariantMonthName(utc.getUTCMonth() + 1) + " " + RuntimeModule.padLeft(utc.getUTCFullYear().toString(), 4) + " " + RuntimeModule.pad2(utc.getUTCHours()) + ":" + RuntimeModule.pad2(utc.getUTCMinutes()) + ":" + RuntimeModule.pad2(utc.getUTCSeconds());
  }
...
function formatRfc1123DateTime(instance) {
  let date = instance.date;
  return getInvariantAbbreviatedDayName(date.getDay()) + ", " + RuntimeModule.pad2(date.getDate()) + " " + getInvariantAbbreviatedMonthName(date.getMonth() + 1) + " " + RuntimeModule.padLeft(date.getFullYear().toString(), 4) + " " + RuntimeModule.pad2(date.getHours()) + ":" + RuntimeModule.pad2(date.getMinutes()) + ":" + RuntimeModule.pad2(date.getSeconds()) + " GMT";
}
...
function formatRoundtripDateTime(instance) {
  let date = instance.date;
  return RuntimeModule.formatDateOnlyText(date.getFullYear(), date.getMonth() + 1, date.getDate()) + "T" + RuntimeModule.pad2(date.getHours()) + ":" + RuntimeModule.pad2(date.getMinutes()) + ":" + RuntimeModule.pad2(date.getSeconds()) + "." + RuntimeModule.pad7(BigInt(date.getMilliseconds()) * ticksPerMillisecond + instance.subMillisecondTicks) + getRoundtripSuffix(instance);
}
function formatSortableDateTime(instance) {
  let date = instance.date;
  return RuntimeModule.formatDateOnlyText(date.getFullYear(), date.getMonth() + 1, date.getDate()) + "T" + RuntimeModule.pad2(date.getHours()) + ":" + RuntimeModule.pad2(date.getMinutes()) + ":" + RuntimeModule.pad2(date.getSeconds());
}
...
function formatCustomToken(instance, token, count, locale, dateSeparator, timeSeparator) {
  ...
  let fraction = BigInt(date.getMilliseconds()) * ticksPerMillisecond + instance.subMillisecondTicks;
  let offset = instance.kind === dateTimeKindLocal ? BigInt(-date.getTimezoneOffset()) * offsetMinuteTicks : BigInt.zero;
...
function validateDateTimeStyles(styles) {
  if (styles < 0 || Math.floor(styles) !== styles)
    throw new Error("ArgumentException: Invalid DateTimeStyles value.");
  let hasRoundtripKind = (styles & dateTimeStylesRoundtripKind) !== 0;
  let hasAdjustToUniversal = (styles & dateTimeStylesAdjustToUniversal) !== 0;
...
function applyDateTimeStyles(value, input, styles) {
  ...
  let noCurrentDateDefault = (styleValue & dateTimeStylesNoCurrentDateDefault) !== 0;
  let adjustToUniversal = (styleValue & dateTimeStylesAdjustToUniversal) !== 0;
  let assumeLocal = (styleValue & dateTimeStylesAssumeLocal) !== 0;
  let assumeUniversal = (styleValue & dateTimeStylesAssumeUniversal) !== 0;
  let roundtripKind = (styleValue & dateTimeStylesRoundtripKind) !== 0;
...
function parseCore(input) {
  ...
  if (__ref$3a7e265ad6e70313a37ac95d = tryParseTimeOnly(s, timeHour, timeMinute, timeSecond, timeMillisecond, timeSubMillisecondTicks, timeKind, timeOffsetTicks), timeHour = __ref$3a7e265ad6e70313a37ac95d[1], timeMinute = __ref$3a7e265ad6e70313a37ac95d[2], timeSecond = __ref$3a7e265ad6e70313a37ac95d[3], timeMillisecond = __ref$3a7e265ad6e70313a37ac95d[4], timeSubMillisecondTicks = __ref$3a7e265ad6e70313a37ac95d[5], timeKind = __ref$3a7e265ad6e70313a37ac95d[6], timeOffsetTicks = __ref$3a7e265ad6e70313a37ac95d[7], __ref$3a7e265ad6e70313a37ac95d[0]) {
    ...
    if (timeKind === dateTimeKindUnspecified)
      return new JDateTime(createLocalDateTime(currentYear, currentMonth, currentDay, timeHour, timeMinute, timeSecond, timeMillisecond), dateTimeKindUnspecified, timeSubMillisecondTicks);
  ...
  if (__ref$9b1bb58b1567532334cd345b = tryParseIsoDate(s, year, month, day), year = __ref$9b1bb58b1567532334cd345b[1], month = __ref$9b1bb58b1567532334cd345b[2], day = __ref$9b1bb58b1567532334cd345b[3], __ref$9b1bb58b1567532334cd345b[0])
    return new JDateTime(createLocalDate(year, month, day), dateTimeKindUnspecified);
  if (__ref$14e2b4da9535e830918ac69e = tryParseIsoDateTime(s, year, month, day, hour, minute, second, millisecond, subMillisecondTicks, kind, offsetTicks), year = __ref$14e2b4da9535e830918ac69e[1], month = __ref$14e2b4da9535e830918ac69e[2], day = __ref$14e2b4da9535e830918ac69e[3], hour = __ref$14e2b4da9535e830918ac69e[4], minute = __ref$14e2b4da9535e830918ac69e[5], second = __ref$14e2b4da9535e830918ac69e[6], millisecond = __ref$14e2b4da9535e830918ac69e[7], subMillisecondTicks = __ref$14e2b4da9535e830918ac69e[8], kind = __ref$14e2b4da9535e830918ac69e[9], offsetTicks = __ref$14e2b4da9535e830918ac69e[10], __ref$14e2b4da9535e830918ac69e[0]) {
    if (kind === dateTimeKindUnspecified)
      return new JDateTime(createLocalDateTime(year, month, day, hour, minute, second, millisecond), dateTimeKindUnspecified, subMillisecondTicks);
  ...
  if (HasUtcSuffix(s))
    return createFromInstantTicks(BigInt(parsed.getTime()) * ticksPerMillisecond + parsedSubMillisecondTicks + unixEpochTicks, dateTimeKindLocal);
  if (HasExplicitOffset(s))
    return new JDateTime(new Date(parsed.getTime()), dateTimeKindLocal, parsedSubMillisecondTicks);
  return new JDateTime(parsed, dateTimeKindUnspecified, parsedSubMillisecondTicks);
}
export function _fad0c74e1c9df5bb() {
  return new JDateTime(createLocalDate(1, 1, 1), dateTimeKindUnspecified);
}
export function _eb38dc04224730ea() {
  return CreateDateTime(createLocalDateTime(9999, 12, 31, 23, 59, 59, 999), dateTimeKindUnspecified, BigInt("9999"));
}
export function _878591efc9a51388() {
  return CreateFromTicks(unixEpochTicks, dateTimeKindUtc);
}
...
export function _4fef4795bcbef97f(date, time) {
  let milliseconds = Number(time.ticks / ticksPerMillisecond);
  let subMillisecondTicks = time.ticks % ticksPerMillisecond;
...
export function _5822b271bb635d64(year, month, day, hour, minute, second, millisecond) {
  return new JDateTime(createLocalDateTime(year, month, day, hour, minute, second, millisecond), dateTimeKindUnspecified);
}
...
export function _9117d26d23769ad1(year, month, day, hour, minute, second, millisecond, microsecond) {
  return CreateDateTime(createLocalDateTime(year, month, day, hour, minute, second, millisecond), dateTimeKindUnspecified, getMicrosecondTicks(microsecond));
}
...
export function _558a3f189d9149d7(instance, value) {
  return CreateFromTicks(getTicks(instance) + createRoundedTicksFromDouble(value * 864000000000), instance.kind);
}
export function _101af978213c19c5(instance, value) {
  return CreateFromTicks(getTicks(instance) + createRoundedTicksFromDouble(value * 36000000000), instance.kind);
}
export function _2b29e4c11fa12daa(instance, value) {
  return CreateFromTicks(getTicks(instance) + createRoundedTicksFromDouble(value * 10000), instance.kind);
}
export function _2b47368c73a3e1f2(instance, value) {
  return CreateFromTicks(getTicks(instance) + createRoundedTicksFromDouble(value * 10), instance.kind);
}
export function _8bdc25943cf2d39b(instance, value) {
  return CreateFromTicks(getTicks(instance) + createRoundedTicksFromDouble(value * 600000000), instance.kind);
}
...
export function _57045f93edac1460(instance, value) {
  return CreateFromTicks(getTicks(instance) + createRoundedTicksFromDouble(value * 10000000), instance.kind);
}
...
export function _f437fad61f0046c7(dateData) {
  let unsignedData = dateData < zeroTicks ? dateData + binaryUnsignedOverflow : dateData;
  let kindBits = unsignedData & binaryKindMask;
  let ticks = unsignedData & binaryTicksMask;
...
export function _12520a637fb85a70(d) {
  return CreateFromTicks(createRoundedTicksFromDouble((d - oaDateUnixOffsetDays) * millisecondsPerDay) * ticksPerMillisecond + unixEpochTicks, dateTimeKindUnspecified);
}
...
export function _9cea54115c704cf7(instance) {
  ...
  return getTicks(instance) + BigInt(instance.kind) * binaryKindShift;
}
...
export function _d3529b55e30e2a12(instance) {
  return RuntimeModule.getInt64HashCode(getTicks(instance));
}
...
export function _34d05014c270366f(instance) {
  return Number(instance.subMillisecondTicks / ticksPerMicrosecond % BigInt(1000));
}
export function _46e11fe2eb2ee869(instance) {
  return Number(instance.subMillisecondTicks % ticksPerMicrosecond * BigInt(100));
}
...
export function _ee9dd166a34a2fa5() {
  return new JDateTime(new Date, dateTimeKindLocal);
}
...
export function _2efdc237be2f31aa(instance) {
  ...
  return new JTimeSpan(BigInt(ms) * ticksPerMillisecond + instance.subMillisecondTicks);
}
export function _4b250155b7c688bb() {
  let now = new Date;
  return new JDateTime(createLocalDate(now.getFullYear(), now.getMonth() + 1, now.getDate()), dateTimeKindLocal);
}
...
export function _fb61bb2ccf4b10b6(instance) {
  return Number(getTicks(instance) - unixEpochTicks) / 10000 / millisecondsPerDay + oaDateUnixOffsetDays;
}
export function _37ee48ca629793fa(instance) {
  return getInstantTicks(instance) - unixEpochTicks + fileTimeUnixEpochTicks;
}
...
export function _db842725d5fd1ca0(instance) {
  ...
  let instantTicks = instance.kind === dateTimeKindUnspecified ? getTicks(instance) : getInstantTicks(instance);
...
export function _bcf4183bef96ea21(instance, date, time) {
  return [new JDateOnly(instance.date.getFullYear(), instance.date.getMonth() + 1, instance.date.getDate()), TimeOnlyModule._a305982aa6859677(instance)];
}
...
export const DateTimeModule = {
  ...
  get_BinaryLocalMask,
  get_BinaryKindMask,
  get_BinaryUnsignedOverflow,
  get_BinaryTicksMask,
  get_MaxDateTimeTicks,
  get_OADateUnixOffsetDays,
  get_MillisecondsPerDay,
  get_DateTimeKindUnspecified,
  get_DateTimeKindUtc,
  ...
  get_DateTimeStylesRoundtripKind,
  ensureWholeNumber,
  createDefaultDateTime,
  createLocalDate,
  createLocalDateTime,
  createFromTicks,
  createFromTicks: CreateFromTicks,
  createDateTime,
  createDateTime: CreateDateTime,
  createFromInstantTicks,
  getKind,
  getMicrosecondTicks,
  getTicks,
  getTicks: GetTicks,
  ...
  formatOffsetTicks,
  getRoundtripSuffix,
  formatInvariantGeneralDateTime,
  ...
  formatYearMonth,
  formatFullDateTime,
  getUniversalDateTimeForFormatting,
  formatUniversalFullDateTime,
  formatRfc1123DateTime,
  formatLocaleDateTime,
  formatGeneralDateTime,
  formatShortDate,
  formatLongDate,
  formatTime,
  formatRoundtripDateTime,
  formatSortableDateTime,
  formatUniversalSortableDateTime,
  formatFraction,
  formatCustomToken,
