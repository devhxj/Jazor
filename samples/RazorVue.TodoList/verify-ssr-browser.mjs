...
async function main() {
  ...
  await waitUntil(
    `(() => { const board = document.querySelector('main[data-todo-template="todo-template-v1"]'); const total = document.getElementById('todo-total-count'); const parameter = board?.getAttribute('data-todo-parameter'); const service = board?.getAttribute('data-todo-service'); const initialized = board?.getAttribute('data-todo-initialized'); const status = board?.getAttribute('data-todo-parameter-status'); const cascade = document.getElementById('todo-cascade-card'); return board && total && total.textContent === "3" && parameter === "SSR ParameterView title" && service === "ssr-provider" && initialized === "ready" && status === "ready" && cascade?.getAttribute('data-todo-cascade') === "SSR ParameterView title" ? "ssr-board" : null; })()`,
  ...
  const boardState = await evaluate(
    `(() => { const board = document.querySelector('main[data-todo-template="todo-template-v1"]'); const total = document.getElementById('todo-total-count'); const parameter = board?.getAttribute('data-todo-parameter'); const service = board?.getAttribute('data-todo-service'); const initialized = board?.getAttribute('data-todo-initialized'); const status = board?.getAttribute('data-todo-parameter-status'); const cascade = document.getElementById('todo-cascade-card'); return board && total && total.textContent === "4" && parameter === "SSR ParameterView title" && service === "ssr-provider" && initialized === "ready" && status === "ready" && cascade?.getAttribute('data-todo-cascade') === "SSR ParameterView title" ? "board-live" : "board-state:" + (total ? total.textContent : "none") + "/" + parameter + "/" + service + "/" + initialized + "/" + status + "/" + cascade?.getAttribute('data-todo-cascade'); })()`);
