...
```powershell
dotnet build .\samples\Wiki\Wiki.csproj
```
...
- `vue` — alias to the same vendored Vue build
...
When the generated shell uses CLR-backed helper members such as `string.Contains(..., StringComparison.OrdinalIgnoreCase)`, `Jazor.CLR` support modules are emitted under `samples/Wiki/wwwroot/jazor/System/...` and resolved in the browser through the import-map prefix `"System/": "/jazor/System/"`.
