...
const defaultRootComponentId = "Vben.ElementPlusInject.Library.VbenDashboardApp";
const defaultRootComponentName = "VbenDashboardApp";
...
async function runOfficialRazorVueConsumerEntry(invocation: ConsumerEntryInvocation): Promise<void> {
  ...
  const entryArgs = [
    ...
    "executeSsrSmoke",
    "--component",
    `VbenDashboardApp=${invocation.rootComponentSelector}`,
    "--mode",
    "both",
    "--production",
  ...
  if (!output.success) {
    ...
    throw new Error(
      [
        "Official RazorVue consumer entry generation failed for ECMAScript.Vben.ElementPlusInject.",
