...
- 新增 `ECMAScript.VueUse`（VueUse 15.0.0）：以组合式 API 绑定 69 个策展 composable，覆盖浏览器/元素、存储/设备、时间、事件与状态/工具分组；`Object`/`Any` 域改用泛型（`CreateEventHook<T>`、`UseAsyncState<T>`、`ComputedAsync<T>`、`ComputedEager<T>`、`UseSorted<T>`），响应式返回值投影到 `ECMAScript.Vue` 的 ref/computed 契约。资源闭包锁定 `@vueuse/core` 与 `@vueuse/shared` 两个逻辑入口。by @devhxj
- 新增 `ECMAScript.FloatingUi`（`@floating-ui/vue` 2.0.1）：提供 `useFloating` 组合式函数、`offset`/`shift`/`flip`/`size`/`arrow`/`autoPlacement`/`hide`/`inline`/`limitShift` 中间件工厂，以及 `autoUpdate`、`computePosition`；资源闭包跨 4 个 `@floating-ui/*` 包，各包版本分别锁定并记入 inventory，作者入口的 `vue` peer 由 `ECMAScript.Vue` 资源库提供。by @devhxj
- 新增 `scripts/csharp/generate-vueuse.cs` 与 `scripts/csharp/generate-floating-ui.cs`：按 registry integrity 锁定上游、按入口裸 import 推导闭包边，并在上游引入未收录包引用时显式失败；生成文本统一 LF 写出以保证 manifest 哈希在 fresh clone 后仍可校验。两个包的测试项目、`vueuse`/`floating-ui` 主线 lane、发布包目录与文档门禁同步接入。by @devhxj

- 新增 P3-C 交互组件与媒体资源绑定：`ECMAScript.VueDraggable`（vue-draggable-plus 0.6.1，`useDraggable` 与泛型组件代理 `VueDraggableList<TItem>`，SortableJS 常用选项与 12 个事件回调）、`ECMAScript.FilePond`（`vue-filepond` 8.0.0 + 核心 `filepond` 4.32.12，核心命令式 API 与 `FilePondInstance`，默认导出工厂代理 `VueFilePond`，随包交付 `filepond.css`）、`ECMAScript.WangEditor`（`@wangeditor/editor-for-vue` 5.1.12 + 核心 `@wangeditor/editor` 5.1.23，`Editor`/`Toolbar` 组件代理、双向 HTML 与生命周期配置，随包交付编辑器样式）。三包测试 12/13/11 项覆盖 manifest/样式哈希、上游导出 drift、组件代理契约、浏览器安全（无顶层 `process.env`）与编译器 emission；`Jazor.EmitTest` 新增 `Materialize_P3CComponentClosures_ResolveTheirEntryGraphs`。**注意**：`@wangeditor/editor-for-vue` 的 npm `latest` 是 Vue 2 版本，必须锁定 5.1.x。by @devhxj
- 新增 P3-B 表单、国际化与服务端状态绑定：`ECMAScript.VeeValidate`（vee-validate 4.15.1，`useForm`/`useField`/`useFieldArray` 与字段状态读写，返回类型为泛型 `VeeValidateFormReturn<TValues>` 等）、`ECMAScript.VueI18n`（vue-i18n 11.4.12，`createI18n`/`useI18n` 与 Composer 的 locale/messages/`t`/`te`/`tm`/`d`，闭包含 `@intlify/*` 三包）、`ECMAScript.VueQuery`（`@tanstack/vue-query` 5.103.1，`VueQueryPlugin`/`useQuery`/`useMutation`/`useIsFetching`/`useIsMutating`，闭包含 `@tanstack/query-core`、`@tanstack/match-sorter-utils`、`vue-demi`）。三包均满足强类型契约、manifest schema 2 与 MIT 许可证；`vue` 与 `@vue/devtools-api` peer 由 `ECMAScript.Vue` 资源库提供。配套测试 VeeValidate 14、VueI18n 8、VueQuery 9 项，含 manifest 哈希、上游导出 drift、inventory fingerprint 与编译器发射覆盖；`Jazor.EmitTest` 新增 `Materialize_P3BClosures_ResolveTheirEntryGraphs` 验证三包真实 materialization 与 peer provider 解析；`test-dotnet.cs` 新增 `vee-validate`/`vue-i18n`/`vue-query` lane。真实 RazorVue 页面与浏览器 smoke 证据归属 P3-B 退出条件，进入 Support 矩阵前保持 Guidance。by @devhxj

#### Features

- Add the P3-B form, internationalization, and server-state bindings: `ECMAScript.VeeValidate` (vee-validate 4.15.1 with `useForm`/`useField`/`useFieldArray` and field state reads/writes typed as generics such as `VeeValidateFormReturn<TValues>`), `ECMAScript.VueI18n` (vue-i18n 11.4.12 with `createI18n`/`useI18n` plus the Composer's locale/messages/`t`/`te`/`tm`/`d`, closing over three `@intlify/*` packages), and `ECMAScript.VueQuery` (`@tanstack/vue-query` 5.103.1 with `VueQueryPlugin`/`useQuery`/`useMutation`/`useIsFetching`/`useIsMutating`, closing over `@tanstack/query-core`, `@tanstack/match-sorter-utils`, and `vue-demi`). All three keep strongly typed contracts, manifest schema 2, and the MIT license; the `vue` and `@vue/devtools-api` peers come from the `ECMAScript.Vue` resource library. Tests cover 14, 8, and 9 cases respectively across manifest hashes, upstream export drift, inventory fingerprints, and compiler emission; `Jazor.EmitTest` adds `Materialize_P3BClosures_ResolveTheirEntryGraphs` for real materialization and peer-provider resolution; `test-dotnet.cs` gains `vee-validate`/`vue-i18n`/`vue-query` lanes. Real RazorVue page and browser smoke evidence belongs to the P3-B exit criteria, so the packages stay Guidance until they enter the Support matrix. by @devhxj
...
- Add `ECMAScript.VueUse` (VueUse 15.0.0): binds 69 curated composables across browser/element, storage/device, time, events, and state/utility groups; `Object`/`Any` domains become generics (`CreateEventHook<T>`, `UseAsyncState<T>`, `ComputedAsync<T>`, `ComputedEager<T>`, `UseSorted<T>`), and reactive returns project onto the `ECMAScript.Vue` ref/computed contracts. The resource closure locks the two logical entries `@vueuse/core` and `@vueuse/shared`. by @devhxj
- Add `ECMAScript.FloatingUi` (`@floating-ui/vue` 2.0.1): provides the `useFloating` composable, the `offset`/`shift`/`flip`/`size`/`arrow`/`autoPlacement`/`hide`/`inline`/`limitShift` middleware factories, and `autoUpdate`/`computePosition`. The resource closure spans four `@floating-ui/*` packages with per-package locked versions recorded in the inventory, and the author entry's `vue` peer is supplied by the `ECMAScript.Vue` library. by @devhxj
- Add `scripts/csharp/generate-vueuse.cs` and `scripts/csharp/generate-floating-ui.cs`: they lock upstream by registry integrity, derive closure edges from each entry's bare imports, and fail explicitly when upstream adds an unlisted package reference; generated text is written as LF so manifest hashes stay verifiable after a fresh clone. Both packages wire in their test projects, `vueuse`/`floating-ui` mainline lanes, publish catalog, and documentation gate. by @devhxj

- Add the P3-C interaction-component and media bindings: `ECMAScript.VueDraggable` (vue-draggable-plus 0.6.1 with `useDraggable` and the generic component proxy `VueDraggableList<TItem>`, plus SortableJS options and 12 event callbacks), `ECMAScript.FilePond` (`vue-filepond` 8.0.0 with core `filepond` 4.32.12, the core imperative API and `FilePondInstance`, the default-export factory proxy `VueFilePond`, shipping `filepond.css`), and `ECMAScript.WangEditor` (`@wangeditor/editor-for-vue` 5.1.12 with core `@wangeditor/editor` 5.1.23, `Editor`/`Toolbar` proxies, two-way HTML and lifecycle config, shipping the editor stylesheet). Tests cover 12/13/11 cases across manifest and style hashes, upstream export drift, component proxy contracts, browser safety (no top-level `process.env`), and compiler emission; `Jazor.EmitTest` adds `Materialize_P3CComponentClosures_ResolveTheirEntryGraphs`. **Note**: the npm `latest` of `@wangeditor/editor-for-vue` targets Vue 2, so 5.1.x is pinned instead. by @devhxj

#### 破坏性变更

绑定声明面、产物布局与 Emit 写入语义在本轮统一收敛到「标准前端项目」契约（详见 `docs/02-architecture/library-artifact-contract.md` 与 `docs/04-roadmap/npm-jsr-binding-tree-shaking-plan.md`）。以下迁移在 `0.x` 阶段以 `MINOR` 交付，逐项给出迁移路径。

- **`[ECMAScript]` 从三参数收敛为单参数**：删除 `Transform` 枚举与 `ECMAScriptAttribute.Validate` 的组合规则，`[ECMAScript]` 只保留一个 import specifier 参数。**迁移**：`[ECMAScript("pkg", Transform.Component, "Export")]` → `[ECMAScript("<上游公开入口>")]` + `[ECMAScriptName("Export")]`；仅当导出名不等于类型名时才需要 `[ECMAScriptName]`（2294 处声明已迁移，232 处补名字，1 处 `default` 显式化）。`[ECMAScript]`（无参）与 `[ECMAScriptModule("path")]` 语义不变。
- **绑定 specifier 改为上游公开入口**：`[ECMAScript("<specifier>")]` 逐字写进生成 import，不再经绑定私有的「逻辑名 → 真入口」别名表。**迁移**：把声明值与 `manifest.json` 的 `imports` 键一起改写为上游可解析路径，例如 `element-plus/affix/ElAffix` → `element-plus/es/components/affix/index.mjs`、`tdesign-vue-next/button/Button` → `tdesign-vue-next/es/button/index.mjs`、`monaco-editor` → `monaco-editor/editor/editor.api.js`。不迁移会在 Deno/NetPack 解析阶段直接失败（旧简写在上游并不存在）。
- **同一模块多处声明按「模块 × 导出名」归并**：一个上游模块导出多个组件是正常形态（`vuetify/components/VCard` 导出 6 个组件），`manifest.json` 的 `imports` 因而不再有「一组件一条」，改为「一模块一条」。**迁移**：只按 specifier 去重，不要假设一组件一条记录。
- **成对 profile 字段退役**：`manifest.json` 中 `development`/`production`、`developmentDependencies`/`productionDependencies` 等成对字段统一为单值；profile 差异交由上游 package 的 `exports` conditions 解析。**迁移**：删去生产向字段，`development` 保留为唯一取值（值必须相同）。
- **`packages/` 包投影退役**：ECMAScript 自有源码 carrier（`clr/**`、`runtime/vu-icons/**`、`dist/**`）按声明路径直接写入项目源码树，不再生成 `jazor/packages/<name>/` 合成本地包，也不再生成合成 `package-lock.json`。**迁移**：`jazor` 下引用 `packages/...` 的路径改为项目相对路径；依赖恢复统一交给 Deno（`node_modules` + `deno.lock`）。
- **Emit 改为就地写入**：删除 `--clean` 与整目录 staging/备份/回滚（`OutputTransaction`、`DirectoryTransaction`、`MaterializationTransaction`），改为逐文件 temp+rename + 清单差异收敛。**迁移**：从 `Jazor.targets` 调用或脚本中移除 `--clean true`；`jazor/` 不再出现 `.jazor-output-*`、`.jazor-emit-*`、`.jazor-library-*`、`.jazor-ssr-materialization-*` 等兄弟目录。增量收敛仍保证删除的组件不再残留。
- **`[Css]` 与形态判别退役，样式统一走 `[Style]`**：不再把样式边分为「JS 模块」与「样式表」两类。**迁移**：样式依赖改写为可重复的 `[Style("<specifier>")]`，挂在对应 `[ECMAScript]` 入口上；编译器在使用点发射普通 side-effect import，下游 CSS 处理交给标准工具链（Jazor 不产出样式清单、不注入 `<link>`、不拼接 CSS）。
- **组件身份改约定判定**：删除按 `Transform.Component` 跳过的逻辑，改为 `ComponentBase` 派生 + Vue marker 判定。**迁移**：自定义组件无需声明 `Transform.Component`，但必须实现 Vue 组件 marker 接口。

#### Breaking Changes

The binding declaration surface, artifact layout, and Emit write semantics now converge on the standard frontend project contract (see `docs/02-architecture/library-artifact-contract.md` and `docs/04-roadmap/npm-jsr-binding-tree-shaking-plan.md`). On `0.x` these land as `MINOR`, and each carries its migration path.

- **`[ECMAScript]` collapses from three parameters to one**: the `Transform` enum and `ECMAScriptAttribute.Validate` combination rules are removed; `[ECMAScript]` keeps a single import-specifier argument. **Migration**: `[ECMAScript("pkg", Transform.Component, "Export")]` becomes `[ECMAScript("<upstream entry>")]` plus `[ECMAScriptName("Export")]`, and the name attribute is only needed when the export name differs from the type name (2294 declarations migrated, 232 names added, 1 explicit `default`). Parameterless `[ECMAScript]` and `[ECMAScriptModule("path")]` keep their meaning.
- **Binding specifiers now name the upstream public entry**: `[ECMAScript("<specifier>")]` is written verbatim into the generated import instead of being resolved through a binding-private alias table. **Migration**: rewrite the declaration and the `manifest.json` `imports` key together to the resolvable upstream path, for example `element-plus/affix/ElAffix` to `element-plus/es/components/affix/index.mjs`, `tdesign-vue-next/button/Button` to `tdesign-vue-next/es/button/index.mjs`, and `monaco-editor` to `monaco-editor/editor/editor.api.js`. The old shorthands do not exist upstream and fail at Deno/NetPack resolution.
- **Multiple declarations of one module merge by module and export name**: a single upstream module exporting several components is normal (`vuetify/components/VCard` exports six), so `manifest.json` `imports` now holds one record per module rather than one per component. **Migration**: dedupe by specifier and do not assume a record per component.
- **Paired profile fields are retired**: `development`/`production` and `developmentDependencies`/`productionDependencies` and their siblings collapse to single values, and profile differences are resolved by the upstream package's `exports` conditions. **Migration**: drop the production-facing fields; `development` remains the single value and the two must agree.
- **`packages/` projection is retired**: ECMAScript-owned source carriers (`clr/**`, `runtime/vu-icons/**`, `dist/**`) are written straight into the project source tree at their declared paths instead of being projected into synthesized local packages under `jazor/packages/<name>/`, and no synthesized `package-lock.json` is produced. **Migration**: references to `packages/...` under `jazor` become project-relative paths, and dependency restore is handed entirely to Deno (`node_modules` plus `deno.lock`).
- **Emit writes in place**: `--clean` and whole-directory staging/backup/rollback (`OutputTransaction`, `DirectoryTransaction`, `MaterializationTransaction`) are removed in favor of per-file temp-plus-rename with manifest-diff convergence. **Migration**: remove `--clean true` from `Jazor.targets` invocations or scripts; sibling directories such as `.jazor-output-*`, `.jazor-emit-*`, `.jazor-library-*`, and `.jazor-ssr-materialization-*` no longer appear. Incremental convergence still removes deleted components.
- **`[Css]` and shape discrimination are retired in favor of `[Style]`**: style edges are no longer split into "JS module" and "stylesheet" classes. **Migration**: declare style dependencies as repeatable `[Style("<specifier>")]` on the corresponding `[ECMAScript]` entry; the compiler emits a plain side-effect import at the use site and leaves CSS processing to the standard toolchain (Jazor produces no style manifest, injects no `<link>`, and concatenates no CSS).
- **Component identity is now convention-based**: the `Transform.Component` skip logic is removed in favor of `ComponentBase` derivation plus a Vue marker. **Migration**: custom components no longer declare `Transform.Component` but must implement the Vue component marker interface.
...
- Element Plus、TDesign 和 Vuetify 绑定在 IDE 中提供对应上游组件库的组件、prop、事件和 slot 原始说明。
- TDesign 生成链保存锁定版本的中英文 web-types 描述，Vuetify 生成链为缺失注释的参数补入官方说明。
- 三个绑定包 README 补充注释来源、C# 属性到 Vue 名称映射及查阅方式。
