...
internal sealed partial class JazorVueParser
{
    ...
    private static readonly Regex CodeStartPattern = new(
        @"@code\s*\{",
        RegexOptions.IgnoreCase | RegexOptions.Compiled);
    ...
    private static IReadOnlyList<JazorImportBinding> ParseBindings(string clause)
    {
        ...
        if (clauseParts.Count > 1)
        {
            ...
            bindings.AddRange(ParseBindings(clauseParts[1].Trim()));
            return bindings;
        }
        ...
        if (clause.StartsWith("{", StringComparison.Ordinal) && clause.EndsWith("}", StringComparison.Ordinal))
        {
            ...
            foreach (var rawPart in content.Split(','))
            {
                ...
                bindings.Add(aliasParts.Length == 2
                    ? new JazorImportBinding(aliasParts[1].Trim(), aliasParts[0].Trim(), JazorImportBindingKind.Named)
                    : new JazorImportBinding(part, part, JazorImportBindingKind.Named));
            }
        ...
        {
            return new[] { new JazorImportBinding(clause.Substring("* as ".Length).Trim(), null, JazorImportBindingKind.Namespace) };
        }
        ...
        for (var index = startIndex; index < sourceText.Length; index++)
        {
            switch (sourceText[index])
            {
                ...
                case '}':
                    ...
                    if (depth == 0)
                    {
                        var codeText = sourceText.Substring(startIndex, index - startIndex);
