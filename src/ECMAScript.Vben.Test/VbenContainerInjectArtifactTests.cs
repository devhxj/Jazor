...
public sealed partial class VbenContainerInjectTests
{
    ...
    [TestMethod]
    public void Vben_MultiShell_ContainerInject_AggregatesInjectedRuntimeShape_IntoVueSfcArtifact()
    {
        var context = CreateMultiShellInjectContext();
        var snapshot = context.CreateSemanticSnapshots()
            .Single(static item => item.ComponentSymbol.Name == "ShellPage");
        var artifact = CreateBuildRenderTreeArtifactFactory().Lower(context, snapshot);

        StringAssert.Contains(artifact.ScriptSetupText, "import { AppHeader as AppHeader } from \"demo-shell/components\";");
        StringAssert.Contains(artifact.ScriptSetupText, "import { ElCard as ElCard, ElContainer as ElContainer, ElMenu as ElMenu } from \"element-plus\";");
        Assert.AreEqual(1, CountOccurrences(artifact.ScriptSetupText, "from \"element-plus\";"), artifact.ScriptSetupText);
        StringAssert.Contains(artifact.TemplateText, "<ElContainer layout-mode=\"mixed\" :menuCollapsed=\"props.collapsed\" @update:menuCollapsed=\"(__value) =&gt; emit(&quot;update:collapsed&quot;, __value)\"");
        StringAssert.Contains(artifact.TemplateText, ":activeMenu=\"props.selectedKey\"");
        StringAssert.Contains(artifact.TemplateText, "@update:activeMenu=\"(__value) =&gt; emit(&quot;update:selectedKey&quot;, __value)\"");
        StringAssert.Contains(artifact.TemplateText, ":openMenus=\"props.expandedKeys\"");
        ...
        StringAssert.Contains(artifact.TemplateText, "<ElMenu :collapse=\"props.collapsed\" :selected-key=\"props.selectedKey\" @update:selected-key=\"(__value) =&gt; emit(&quot;update:selectedKey&quot;, __value)\"");
        StringAssert.Contains(artifact.TemplateText, ":expanded-keys=\"props.expandedKeys\"");
        StringAssert.Contains(artifact.TemplateText, "@update:expanded-keys=\"(__value) =&gt; emit(&quot;update:expandedKeys&quot;, __value)\"");
        StringAssert.Contains(artifact.TemplateText, "<slot name=\"sidebarLogo\" />");
        StringAssert.Contains(artifact.TemplateText, "<ElCard header=\"Dashboard\" subtitle-text=\"Realtime\">");
        StringAssert.Contains(artifact.TemplateText, "<template #header-extra>");
        ...
        Assert.IsFalse(artifact.TemplateText.Contains("selectedKey=", StringComparison.Ordinal), artifact.TemplateText);
        Assert.IsFalse(artifact.TemplateText.Contains("expandedKeys=", StringComparison.Ordinal), artifact.TemplateText);
        CollectionAssert.AreEquivalent(new[] { "vue", "element-plus", "demo-shell/components" }, artifact.Imports.ToArray());
        CollectionAssert.AreEquivalent(
            new[]
            {
                "element-plus/theme-chalk/el-container.css",
                "element-plus/theme-chalk/el-menu.css",
                "element-plus/theme-chalk/el-card.css",
                "demo-shell/styles/header.css"
            },
            artifact.Styles.ToArray());
        CollectionAssert.AreEquivalent(new[] { "element-plus", "demo-shell" }, artifact.PluginRequirements.ToArray());
    }

    [TestMethod]
    public void Vben_MultiShell_ContainerInject_AggregatesInjectedRuntimeShape_IntoPipelineArtifact()
    {
        var context = CreateMultiShellInjectContext();
        var artifact = CreateBuildRenderTreePipeline().Execute(context).Artifacts
        ...
        StringAssert.Contains(artifact.ModuleCode, "import { AppHeader as VbenHeaderBarComponent } from \"demo-shell/components\";");
        StringAssert.Contains(artifact.ModuleCode, "import { ElContainer as VbenAdminLayoutComponent, ElCard as VbenPageContainerComponent, ElMenu as VbenSidebarMenuComponent } from \"element-plus\";");
        Assert.AreEqual(1, CountOccurrences(artifact.ModuleCode, "from \"element-plus\";"), artifact.ModuleCode);
        StringAssert.Contains(artifact.ModuleCode, "\"layout-mode\": \"mixed\"");
        StringAssert.Contains(artifact.ModuleCode, "\"menuCollapsed\": props.collapsed");
        StringAssert.Contains(artifact.ModuleCode, "\"onUpdate:menuCollapsed\": (__value) => emit(\"update:collapsed\", __value)");
        StringAssert.Contains(artifact.ModuleCode, "\"activeMenu\": props.selectedKey");
    ...
    private static RazorVueCompilationContext CreateMultiShellInjectContext()
        => CreateContext(
            """
            using System.Collections.Generic;
            using ECMAScript;
