...
public sealed partial class JazorVueCompiler
{
    ...
    public JazorVueCompilationResult Compile(JazorVueDocument document)
    {
        ...
        if (props.Count > 0)
        {
            ...
            AppendGeneratedLine(builder, generatedLines, "});", props[props.Count - 1].SourceLine);
        ...
        int sourceLine)
    {
        ...
        foreach (var (generatedColumn, sourceColumn) in EnumerateSharedTokenAnchors(generatedText, sourceText))
        {
            ...
            if (segments.Count > 0 && segments[segments.Count - 1].GeneratedColumn == generatedColumn)
            {
                continue;
            }
    ...
    private static int GetCodeSourceLine(JazorVueDocument document, int relativeCodeOffset)
    {
        ...
        var absoluteOffset = document.CodeStartIndex + Math.Max(relativeCodeOffset, 0);
        absoluteOffset = Math.Min(Math.Max(absoluteOffset, 0), document.SourceText.Length);
    ...
    private static string ExtractBlockBody(string code, int bodyStart, out int nextIndex, out int bodyStartOffset)
    {
        ...
        for (var index = bodyStart; index < code.Length; index++)
        {
            ...
            switch (code[index])
            {
                ...
                case '}':
                    ...
                    if (depth == 0)
                    {
                        var body = code.Substring(bodyStart, index - bodyStart);
        ...
        var tail = bodyStart >= code.Length ? string.Empty : code.Substring(bodyStart);
        ...
        ISet<string> shadowedNames)
    {
        var rewritten = RewriteCodeSegments(
            ...
            static segment =>
            {
                var updatedSegment = segment.Replace("this.", string.Empty)
                    .Replace("string.Empty", "\"\"")
                    .Replace("Task.CompletedTask", "Promise.resolve()");
        ...
        ISet<string> shadowedNames)
    {
        ...
        for (var index = 0; index < content.Length; index++)
        {
            ...
            if (character == '{')
            {
                ...
                var expression = content.Substring(index + 1, endIndex - index - 1);
        ...
        ISet<string> shadowedNames)
    {
        ...
        for (var index = 0; index < value.Length;)
        {
            ...
            if (IsIdentifierStart(character))
            {
                ...
                var identifier = value.Substring(start, index - start);
    ...
    private static string RewriteCodeSegments(string value, Func<string, string> rewriteSegment)
    {
        ...
        for (var index = 0; index < value.Length; index++)
        {
            ...
            {
                builder.Append(rewriteSegment(value.Substring(segmentStart, index - segmentStart)));
            }
        ...
        {
            builder.Append(rewriteSegment(value.Substring(segmentStart)));
        }
