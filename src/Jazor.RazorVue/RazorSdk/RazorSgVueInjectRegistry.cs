...
internal sealed class RazorSgVueInjectRegistry
{
    ...
    private static readonly ConditionalWeakTable<Compilation, RazorSgVueInjectRegistry> Cache = new();

    private readonly ImmutableDictionary<INamedTypeSymbol, INamedTypeSymbol> _implementations;

    private RazorSgVueInjectRegistry(
        ImmutableDictionary<INamedTypeSymbol, INamedTypeSymbol> implementations)
    {
        _implementations = implementations;
    }

    public static RazorSgVueInjectRegistry ForCompilation(Compilation compilation)
    {
    ...
    private static RazorSgVueInjectRegistry Create(Compilation compilation)
    {
        var attributeType = compilation.GetTypeByMetadataName(VueInjectAttributeMetadataName);
        if (attributeType is null)
            return new RazorSgVueInjectRegistry(CreateImplementationBuilder().ToImmutable());
        ...
        return new RazorSgVueInjectRegistry(implementations.ToImmutable());
    }
