...
- 普通 lifecycle body 现在也接受 source-stable local-only / terminal-return no-op 形态：仅包含无副作用局部声明、空语句、可选 terminal no-op return，以及 terminal `if (...) { return; }` / `if (...) { return; } else { return; }` 的 `OnInitialized` / `OnParametersSet` / `OnAfterRender` 等方法会被视为没有新增 runtime 行为，不生成 Vue hook/watch。可擦除表达式只接受可证明无观察副作用的值形态，例如常量、`default`、`typeof(...)`、local/method parameter reference、组件 `[Parameter]` prop reference、简单 compiler conversion/unary/binary/conditional/coalesce wrapper 与这些值组成的 tuple；`var count = GetCount();`、side-effect guard condition、object/array creation、mutation、任意 invocation、current-component 非参数 property/field 或一般 statement body 仍显式 unsupported。
