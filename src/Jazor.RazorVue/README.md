...
- `SetParametersAsync` 也仍是受控子集：支持 no-op（含 expression-bodied `=> Task.CompletedTask` 一类空实现）、直达 `ComponentBase.SetParametersAsync(...)` 的 base pass-through，以及“base pass-through 后接 source-stable local / local function / callable local 前缀，再进入线性有序 `InvokeAsync` emit 序列、受控 `if` / `else` emit 分支、受控 guard-return / `if/else` 单分支 `return` emit 分支、受控传统 `switch` emit 分支、受控 `try/finally` emit cleanup、受控 `try/catch` emit recovery 或受控 `try/catch/finally` emit recovery+cleanup（含受控 guard-return 内嵌组合）”这类可稳定映射到一个 Vue `watch` 的形态；共享 source-stable local 只会在 watch body 中物化一次，base-chain emit 与 derived emit 会按 C# 源码顺序 replay。base pass-through 之后的可达无操作 `return;` 会终止当前受控序列，后续不可达语句不会被 replay 成 lifecycle 行为。额外 mutation、非 emit loop / pattern switch 控制流或更一般的方法体仍显式 unsupported。
- `SetParametersAsync` 的受控 `if` / `else`、guard-return / `if/else` 单分支 `return`、传统 `switch`、`try/finally`、`try/catch` 与 `try/catch/finally` 只接受同一 lifecycle payload 子集内的条件 / switch value / constant case label 表达式、emit 语句和 no-op cleanup；条件、switch value、guard condition 或 try 前置值若来自 source-stable local carrier，会在 watch body 中按源码顺序生成一次 `const`，再进入分支、guard 或 `try`，从而保留单次求值和源码顺序。guard-return 当前只接受无 `else`、单个空 `return;` 且后续仍存在受控 emit 行为的形态；`if/else` 单分支 `return` 当前只接受 exactly-one-branch 空 `return;`、另一分支仍落在受控 lifecycle statement 子集内的形态。switch section 必须包含 constant/default label、受控 emit/no-op body，并以 terminal `break` 结束；`try/catch` / `try/catch/finally` 当前只接受单个 catch-all / `System.Exception` catch，`catch (Exception error)` 这类未使用 catch binding 会按 JS `catch { ... }` 发射，不暴露 exception object；不引用 catch 变量的 filter 会按 JS handled gate 发射，filter 为 false 或 filter 自身抛错时重抛原异常；并允许上述受控 lifecycle statement 在 `try` / `catch` / `finally` body 内组合。带值 `return`、裸 `return`、terminal-only guard、双分支 `return`、typed catch 分派、多 catch、catch 变量 payload、filter 内 mutation / lambda / local-function、分支内 current-component property/field mutation、任意外部 invocation、fallthrough 空 section、loop / pattern switch 等更宽控制流仍不属于当前支持面。
