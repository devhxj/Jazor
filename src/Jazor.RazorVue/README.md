...
- `SetParametersAsync` 也仍是受控子集：支持 no-op（含 expression-bodied `=> Task.CompletedTask` 一类空实现）、直达 `ComponentBase.SetParametersAsync(...)` 的 base pass-through，以及“base pass-through 后接 source-stable local / local function / callable local 前缀，再进入线性有序 `InvokeAsync` emit 序列、受控 `if` / `else` emit 分支、受控 guard-return / `if/else` 单分支 `return` emit 分支、terminal-only guard-return / `if/else` 双分支 `return` no-op、受控传统 `switch` emit 分支、受控 `try/finally` emit cleanup、受控 `try/catch` emit recovery 或受控 `try/catch/finally` emit recovery+cleanup（含 no-op `try` cleanup-only、no-op `catch` recovery、no-op try+catch cleanup-only finally、try body 顶层裸 `return;` + finally cleanup、no-op `finally` cleanup 与受控 guard-return 内嵌组合）”这类可稳定映射到一个 Vue `watch` 或稳定证明无新增 runtime 行为的形态；共享 source-stable local 只会在 watch body 中物化一次，base-chain emit 与 derived emit 会按 C# 源码顺序 replay。base pass-through 之后的可达无操作 `return;` 会终止当前受控序列，后续不可达语句不会被 replay 成 lifecycle 行为。额外 mutation、非 emit loop / pattern switch 控制流或更一般的方法体仍显式 unsupported。
- `SetParametersAsync` 的受控 `if` / `else`、guard-return / `if/else` 单分支 `return`、terminal no-op return、传统 `switch`、`try/finally`、`try/catch` 与 `try/catch/finally` 只接受同一 lifecycle payload 子集内的条件 / switch value / constant case label 表达式、emit 语句和 no-op cleanup；条件、switch value、guard condition 或 try 前置值若来自 source-stable local carrier，会在 watch body 中按源码顺序生成一次 `const`，再进入分支、guard 或 `try`。terminal-only guard-return / 双分支 `return` no-op 只在顶层 post-base sequence、return 前没有已确认 emit 且 condition/local initializer 无可忽略副作用时保持 TemplateOnly。switch section 必须包含 constant/default label、受控 emit/no-op body，并以 terminal `break` 结束；`try/catch` / `try/catch/finally` 当前只接受单个 catch-all / `System.Exception` catch，`catch (Exception error)` 这类未使用 catch binding 会按 JS `catch { ... }` 发射，不暴露 exception object；no-op catch body 会保留真实 JS `catch {}` 边界以表达吞掉异常；try/catch body 可以是 no-op，但 `try/catch/finally` 只有在 try / catch / finally 至少一侧存在受控 emit 行为时才生成 watch；try body 顶层裸 `return;` 只在存在 finally cleanup emit 时作为真实 JS `return;` 支持，并会先物化 pending source-stable local。`finally { }`、`finally { await Task.CompletedTask; }` 或等价 no-op lifecycle cleanup 会保留真实 JS `finally {}` 边界但不生成额外 emit payload。带值 `return`、无 finally cleanup 的 try-body 裸 `return;`、catch/finally 内 `return`、emit 后双分支 `return`、typed catch 分派、多 catch、catch 变量 payload、filter 内 mutation / lambda / local-function、分支内 current-component property/field mutation、任意外部 invocation、fallthrough 空 section、loop / pattern switch 等更宽控制流仍不属于当前支持面。
