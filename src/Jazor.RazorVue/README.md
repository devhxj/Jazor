...
- RazorVue 的 lifecycle/setup lowering 仍是受控子集，但当前已正式支持 `ShouldRender` cached render gate、受控 base-pass-through、线性局部前缀语句体、表达式/条件中的 declaration local、recursive/list pattern 顶层声明 local、local-only mutation statement，以及受控 `if` / `else`、传统 `switch`、guarded pattern `switch`（含受控 block / `if` / loop / `try/catch/finally` 内嵌套形态）、`while` / `do-while` / `for` / 同步 `foreach`、同步 `try/catch/finally`、同步异常分支条件语句体。
- `ShouldRender` 当前接受的安全形态包括：
  ...
  - guarded pattern `switch`，包括非穷尽 guarded case 后接 trailing `return`，case body 内的 `break` fallthrough，以及受控 block / `if` / loop / `try/catch/finally` 内嵌套形态；RazorVue 只桥接 `SemanticWalker` 生成的 pattern-switch IIFE 返回值，不手写 pattern/member 语义
  - 受控同步 `while` / `do-while` / `for` / `foreach`，loop body 可包含局部声明、local-only mutation、受控 `if` / 传统 `switch`、`return bool`，并在 loop scope 内保留 `break` / `continue`；`for` header 同样只允许局部声明和 local-only mutation，`foreach` collection 表达式仍禁止 mutation / lambda / local-function，例如 `while (Value > 0) { return true; } return false;`、`for (var index = 0; index < Value; index++) { if (index == 1) continue; } return true;` 或 `foreach (var item in Items) { if (item > 0) return true; } return false;`
  - 受控同步 `try/catch/finally`，`try` / `catch` / `finally` body 复用同一受控语句子集并必须能证明正常路径最终返回 bool；catch filter 允许同一安全表达式子集但禁止 mutation / lambda / local-function，catch 参数会和其他 `ShouldRender` local 一样稳定 alias，避免源码名 `props` / render gate 保留名遮蔽 setup binding，例如 `try { return Value > 0; } catch (Exception props) { return props is not null && Value > 1; } finally { local++; }`
...
- 普通 lifecycle body 现在也接受 source-stable local-only / terminal-return no-op 形态：仅包含无副作用局部声明、空语句、no-op await expression statement、可选 terminal no-op return，以及 terminal `if (...) { return; }` / `if (...) { return; } else { return; }` 的 `OnInitialized` / `OnParametersSet` / `OnAfterRender` 等方法会被视为没有新增 runtime 行为，不生成 Vue hook/watch。可擦除表达式只接受可证明无观察副作用的值形态，例如常量、`default`、`typeof(...)`、local/method parameter reference、组件 `[Parameter]` prop reference、当前组件或源码可分析 base class 上受支持 source-stable value member 读取（要求 declaration initializer 本身递归落在同一 no-op 值子集内）、当前组件 private 同步 helper 且 helper 返回表达式递归落在同一 no-op 值子集内（无参，或所有实参同样落在 no-op value 子集内且形参均为普通按值参数）、简单 compiler conversion/unary/binary/conditional/coalesce wrapper 与这些值组成的 tuple；no-op await 仅复用既有 lifecycle no-op awaitable 判定，例如 `await Task.CompletedTask;`。无法证明无副作用的 invocation、`await Task.Delay(1);`、side-effect guard condition、object/array creation initializer、mutation、带副作用实参、`ref` / `out` / `in` / `params` helper 参数、async/`Task`/`ValueTask`/ref-return/外部/非 private helper、public mutable property、custom getter、存在后续可观察写入的 private mutable member 或一般 statement body 仍显式 unsupported。
...
- lifecycle payload 当前也已补齐一条受控 current-component helper/method-call 路径：
  ...
  - 调用点参数必须能按 Roslyn 绑定结果稳定落位；普通按值参数、非写入 `in` 只读值参数、named argument out-of-order、omitted optional default、以及 `params` 单数组形参均已支持，`ref` / `out` 仍显式 fail-fast
...
- 这条 helper payload 扩面依然不是“任意执行模型放开”：
  - `async` helper、`Task` / `ValueTask` 返回 helper、`ref` / `out` 参数、`in` 参数 by-reference 转发/逃逸、越出当前 setup lowering / `SemanticWalker` 合同的 helper body、以及更宽的动态 member/dataflow 仍显式报 `UnsupportedLifecycleLowering` / analyzer `JAZORVUE005`
...
- `SetParametersAsync` 也仍是受控子集：支持 no-op（含 expression-bodied `=> Task.CompletedTask` 一类空实现）、直达 `ComponentBase.SetParametersAsync(...)` 的 base pass-through，以及“base pass-through 后接 source-stable local / local function / callable local 前缀，再进入线性有序 `InvokeAsync` emit 序列、受控 `if` / `else` emit 分支、受控 guard-return / `if/else` 单分支 `return` emit 分支、terminal-only guard-return / `if/else` 双分支 `return` no-op、emit 后 terminal `if/else` 双分支 `return`、受控传统 `switch` emit 分支、受控 `try/finally` emit cleanup、受控 `try/catch` emit recovery 或受控 `try/catch/finally` emit recovery+cleanup（含 no-op `try` cleanup-only、no-op `catch` recovery、no-op try+catch cleanup-only finally、try body 顶层裸 `return;` + catch recovery / finally cleanup、无 runtime-before-return 的 terminal try/catch no-op（含不引用 catch 变量的受控 filter 语法噪声）、no-op `finally` cleanup 与受控 guard-return 内嵌组合）”这类可稳定映射到一个 Vue `watch` 或稳定证明无新增 runtime 行为的形态；共享 source-stable local 会在首个 runtime lifecycle statement 前或对应 watch body 中物化一次，base-chain emit 与 derived emit 会按 C# 源码顺序 replay。base pass-through 之后的可达无操作 `return;` 会终止当前受控序列，后续不可达语句不会被 replay 成 lifecycle 行为。额外 mutation、非 emit loop / pattern switch 控制流或更一般的方法体仍显式 unsupported。
- `SetParametersAsync` 的受控 `if` / `else`、guard-return / `if/else` 单分支 `return`、terminal no-op return、emit 后 terminal `if/else` 双分支 `return`、传统 `switch`、`try/finally`、`try/catch` 与 `try/catch/finally` 只接受同一 lifecycle payload 子集内的条件 / switch value / constant case label 表达式、emit 语句和 no-op cleanup；条件、switch value、guard condition 或 try 前置值若来自 source-stable local carrier，会按源码顺序生成一次 `const`，再进入分支、guard、`try` 或后续 emit。terminal-only guard-return / 双分支 `return` no-op 只在顶层 post-base sequence、return 前没有已确认 emit 且 condition/local initializer 无可忽略副作用时保持 TemplateOnly；无 runtime-before-return 的 terminal `try { return; } catch { ... }` 也只在顶层 post-base sequence、尚未发生 emit、catch 变量不参与、无 finally、try body 前缀可安全擦除时保持 TemplateOnly，不会 replay 不可达 catch recovery 或 filter gate；存在 catch filter 时，filter 仍必须不引用 catch 变量并落在 lifecycle payload 安全子集内。emit 后 terminal 双分支 `return` 只在已有受控 runtime 行为、`if/else` 是顶层最后一条语句且两侧都是空 `return;` 时发射真实 JS `if/else`，不会 replay 任何额外语句。switch section 必须包含 constant/default label、受控 emit/no-op body，并以 terminal `break` 结束；`try/catch` / `try/catch/finally` 当前只接受单个 catch-all / `System.Exception` catch，`catch (Exception error)` 这类未使用 catch binding 会按 JS `catch { ... }` 发射，不暴露 exception object；no-op catch body 会保留真实 JS `catch {}` 边界以表达吞掉异常；try/catch body 可以是 no-op，但 `try/catch/finally` 只有在 try / catch / finally 至少一侧存在受控 emit 行为时才生成 watch；try body 顶层裸 `return;` 只在存在 catch recovery emit 或 finally cleanup emit 时作为真实 JS `return;` 支持，并会先物化 pending source-stable local。`finally { }`、`finally { await Task.CompletedTask; }` 或等价 no-op lifecycle cleanup 会保留真实 JS `finally {}` 边界但不生成额外 emit payload。带值 `return`、没有 catch recovery/finally cleanup 且存在可观察前缀的 try-body 裸 `return;`、catch/finally 内 `return`、typed catch 分派、多 catch、catch 变量 payload、filter 内 mutation / lambda / local-function、分支内 current-component property/field mutation、任意外部 invocation、object/array creation 前缀、fallthrough 空 section、loop / pattern switch 等更宽控制流仍不属于当前支持面。
...
- 上述 `SetParametersAsync` contract 现在已在 analyzer / lowering / generator 三层对齐；analyzer 会构建同一 semantic snapshot 并复用 lowering 侧 support-shape 判定，expression-bodied no-op、base+local-prefix emit 和 base+derived 多 emit 序列都不会再出现“pipeline 接受但 analyzer 仍报 `JAZORVUE006`”的漂移。
...
- render helper 的非 builder `in` 参数按只读值参数进入同一 captured scope 合同；RazorVue 只支持在 helper body 内读取该参数，若继续把它转发到任何 `ref` / `out` / `in` by-reference 调用，会显式 fail-fast，避免把 CLR byref/别名语义错误降为普通 JS 值传递。
...
- 对于带额外值参数的 render helper，当前只支持：
  ...
  - 仍不支持 `ref` / `out` 参数、`in` 参数 by-reference 转发/逃逸、跨 helper 留下未闭合 frame、关闭/重开 caller-owned frame、region 逃逸/不平衡、改变最终 active caller-owned frame，或需要跨 helper 推断 frame shape 的协议
