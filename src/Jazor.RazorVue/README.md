...
- 普通 lifecycle body 现在也接受 source-stable local-only / terminal-return no-op 形态：仅包含无副作用局部声明、空语句、no-op await expression statement、可选 terminal no-op return，以及 terminal `if (...) { return; }` / `if (...) { return; } else { return; }` 的 `OnInitialized` / `OnParametersSet` / `OnAfterRender` 等方法会被视为没有新增 runtime 行为，不生成 Vue hook/watch。可擦除表达式只接受可证明无观察副作用的值形态，例如常量、`default`、`typeof(...)`、local/method parameter reference、组件 `[Parameter]` prop reference、当前组件或源码可分析 base class 上受支持 source-stable value member 读取（要求 declaration initializer 本身递归落在同一 no-op 值子集内）、当前组件 private 无参同步 helper 且 helper 返回表达式递归落在同一 no-op 值子集内、简单 compiler conversion/unary/binary/conditional/coalesce wrapper 与这些值组成的 tuple；no-op await 仅复用既有 lifecycle no-op awaitable 判定，例如 `await Task.CompletedTask;`。无法证明无副作用的 invocation、`await Task.Delay(1);`、side-effect guard condition、object/array creation initializer、mutation、带参数/async/`Task`/`ValueTask`/ref-return/外部/非 private helper、public mutable property、custom getter、存在后续可观察写入的 private mutable member 或一般 statement body 仍显式 unsupported。
