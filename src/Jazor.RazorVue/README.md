...
- 普通 lifecycle body 现在也接受 source-stable local-only no-op 形态：仅包含无副作用局部声明、空语句和可选 terminal no-op return 的 `OnInitialized` / `OnParametersSet` / `OnAfterRender` 等方法会被视为没有新增 runtime 行为，不生成 Vue hook/watch。局部 initializer 只在可证明无观察副作用时才会被擦除，例如常量、`default`、`typeof(...)`、local/parameter reference、简单 compiler conversion/unary wrapper 与这些值组成的 tuple；`var count = GetCount();`、object/array creation、mutation、任意 invocation 或一般 statement body 仍显式 unsupported。
