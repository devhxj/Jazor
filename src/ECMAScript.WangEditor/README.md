...
完整性与许可证元数据；样式由绑定入口的 `[Style]` 声明。

Strongly typed C# bindings for WangEditor and its official Vue 3 adapter, shipped as a Jazor JS
resource library with locked upstream package versions. Runtime files remain in npm; binding
`[Style]` declarations carry stylesheet edges while metadata describes the package entries.
...
| 样式 | `@wangeditor/editor/dist/css/style.css`（由 WangEditor 入口的 `[Style]` 声明） |
