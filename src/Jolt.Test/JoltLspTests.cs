...
[TestClass]
public sealed class JoltLspTests
{
    private const string DevServerFileChangeDebounceIntervalEnvironmentVariable = "JOLT_DEVSERVER_FILE_CHANGE_DEBOUNCE_MS";
    private const string DevServerFileChangePollingIntervalEnvironmentVariable = "JOLT_DEVSERVER_FILE_CHANGE_POLLING_MS";
    ...
    [TestMethod]
    public async Task Jolt_Lsp_Initialize_ReturnsCapabilities()
    {
        ...
        Assert.AreEqual("Jolt", result.GetProperty("serverInfo").GetProperty("name").GetString());

        await client.ShutdownAsync();
    }

    [TestMethod]
    public async Task Jolt_Lsp_Initialize_AdvertisesWorkspaceFolderCapabilities()
    {
        await using var client = await LspTestClient.StartAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WorkspaceFoldersChanged_ContinuesServingWorkspaceSymbolRequests()
    {
        await using var client = await LspTestClient.StartAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_Initialize_SucceedsWhenDenoIsEnabledWithInvalidCommand()
    {
        ...
        try
        {
            ...
            Assert.AreEqual("Jolt", response.RootElement.GetProperty("result").GetProperty("serverInfo").GetProperty("name").GetString());
        }
        finally
        {
    ...
    [TestMethod]
    public async Task Jolt_Lsp_TemplateRequests_ReturnNullOrEmptyWhenFrontendLaneHasNoAnswer()
    {
    ...
    [TestMethod]
    public async Task Jolt_Lsp_DocumentHighlight_ReturnsJazorTemplateMatches()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_DocumentLink_ReturnsImportTargets()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    public async Task Jolt_Lsp_DocumentLink_IgnoresFakeModuleDirectivesInsideCodeBlocks()
    {
        ...
        try
        {
            ...
            var links = await documentService.GetDocumentLinksAsync(document, CancellationToken.None);
            var targets = links
                .Select(static link => link.Target)
                .Where(static target => !string.IsNullOrWhiteSpace(target))
                .ToArray();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_TypeDefinition_ReturnsLocations()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_SelectionRange_ReturnsNestedRanges()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_LinkedEditingRange_ReturnsTagNameRanges()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_Formatting_TrimsTrailingWhitespace()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CompletionResolve_ReturnsCompletionItem()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CallHierarchy_ReturnsIncomingAndOutgoingCalls()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_TypeHierarchy_ReturnsSuperAndSubTypes()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_SemanticTokens_ReturnEmptyWhenFrontendLaneHasNoAnswer()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_SemanticTokens_ReturnRoslynCodeTokensWithoutFrontendWorker()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_SemanticTokens_ReturnFrontendTokensFromBundledDenoWorker()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_DidOpenDidChangeAndDidClose_PublishDiagnostics()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_StillServesHttpAndProcessesLspMessages()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedJazorCodeBehindDidChange_BroadcastsHmrJavaScriptUpdate()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        await using var client = await LspTestClient.StartAsync(
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedJazorCodeBehindSignatureDidChange_BroadcastsFullReload()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        await using var client = await LspTestClient.StartAsync(
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedJazorCodeBehindDidChangeIsLaterSaved_DoesNotBroadcastDuplicateHmrMessage()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        await using var client = await LspTestClient.StartAsync(
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedVueDidChange_BroadcastsHmrJavaScriptUpdate()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        await File.WriteAllTextAsync(Path.Combine(tempDirectory, "index.html"), "<html><body>combined</body></html>");
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedVueDidChangeIsLaterSaved_DoesNotBroadcastDuplicateHmrMessage()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        await using var client = await LspTestClient.StartAsync(
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedVueStyleOnlyDidChange_BroadcastsStyleUpdate()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        var documentPath = Path.Combine(tempDirectory, "Counter.vue");
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedVueStyleOnlyDidChangeIsLaterSaved_DoesNotBroadcastDuplicateHmrMessage()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        var documentPath = Path.Combine(tempDirectory, "Counter.vue");
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedCssDidChange_UsesWorkspaceTextAndSuppressesDuplicateHmrMessage()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedTypeScriptDidChange_BroadcastsHmrJavaScriptUpdate()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        await File.WriteAllTextAsync(Path.Combine(tempDirectory, "index.html"), "<html><body>combined</body></html>");
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedTypeScriptDidChangeIsLaterSaved_DoesNotBroadcastDuplicateHmrMessage()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        await using var client = await LspTestClient.StartAsync(
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedJazorDidChange_BroadcastsHmrJavaScriptUpdate()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        await File.WriteAllTextAsync(Path.Combine(tempDirectory, "index.html"), "<html><body>combined</body></html>");
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedJazorDescriptorDidChange_BroadcastsFullReload()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        await using var client = await LspTestClient.StartAsync(
    ...
    [TestMethod]
    public async Task Jolt_Lsp_WithDevMode_WhenUnsavedJazorDidChangeIsLaterSaved_DoesNotBroadcastDuplicateHmrMessage()
    {
        var tempDirectory = CreateTemporaryDirectory();
        var port = GetFreePort();
        await using var client = await LspTestClient.StartAsync(
    ...
    [TestMethod]
    public async Task Jolt_Lsp_TemplateImportedComponent_RemainsCompletionHoverAndDefinitionCapable()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_RazorMarkupComponent_RemainsCompletionHoverAndDefinitionCapable()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_RazorMarkupComponentWithoutTemplateOrVueImport_RemainsFeatureCapable()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueDocument_RemainsCompletionHoverDefinitionAndDiagnosticCapable()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_TypeScriptDocument_RemainsCompletionHoverDefinitionReferencesAndRenameCapable()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueDocument_ScriptBlock_RemainsCompletionHoverAndDefinitionCapable()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueDocument_ReferencesAndRename_IncludeNearbyJazorDocumentsOnDisk()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueScriptImport_ReferencesAndRename_IncludeNearbyJazorDocumentsOnDisk()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueScriptImport_DefinitionRemainsNativeWhileReferencesAndRenameBridgeIntoJazor()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
        ...
        try
        {
            ...
            Assert.IsTrue(string.Equals(
                ...
                JoltWorkspaceResolver.NormalizePath(LspProtocolHelpers.ToDocumentPath(definitions[0].GetProperty("uri").GetString()!)),
    ...
    [TestMethod]
    public async Task Jolt_Lsp_TypeScriptVueImport_ReferencesAndRename_IncludeNearbyJazorDocumentsOnDisk()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_JavaScriptVueImport_DefinitionRemainsNativeWhileReferencesAndRenameBridgeIntoJazor()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_TypeScriptDocument_ReturnsFrontendScriptCompletionHoverAndDefinition()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CSharpDocument_DefinitionReferencesAndRename_WorkEndToEnd()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CSharpDocument_DefinitionReferencesAndRename_IncludeUnopenedDiskCSharpDocument()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CSharpDocument_DefinitionReferencesAndRename_IncludeUnopenedDiskBackedWorkspaceDocuments()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_JazorCodeDeclaration_ReferencesAndRename_IncludeUnopenedDiskBackedCSharpAndJazorDocuments()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_JazorCodeUsage_Definition_ResolvesUnopenedDiskBackedCSharpDeclaration()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CSharpDocument_CompletionHoverAndDocumentSymbols_WorkEndToEnd()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CSharpDocument_CompletionAndHover_UseUnopenedDiskBackedWorkspaceDeclaration()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueScriptBlock_ReturnsFrontendScriptCompletionAndHover()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueScriptBlock_ResolvesRelativeImportSymbolsConservatively()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueScriptBlock_ResolvesReExportedAliasAndDefaultImportSymbolsConservatively()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueScriptBlock_UsesBundledTypeScriptServiceForImportedMemberCompletionAndHover()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_Hover_ReturnsImportDetailsForComponentTag()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_Completion_ReturnsDirectiveAndTemplateItems()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_DidChangeAndDidClose_UpdateObservableProjectionBackedResults()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_TrackedVueWorkspaceDocument_SupportsCompletionHoverAndDefinition()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_ReferencesAndRename_IncludeOtherOpenJazorDocuments()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_TrackedVueWorkspaceDocument_SuppressesMissingComponentDiagnostics_AndReappearsOnClose()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_JazorDocument_ResolvesWorkspaceVueComponentOutsideNearbyDirectories()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueDocument_ReferencesAndRename_IncludeOpenUnsavedJazorDocuments_WithoutTouchingCodeRegion()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_VueDocument_ReferencesAndRename_IncludeWorkspaceDiskJazorDocumentsOutsideNearbyDirectories()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_Definition_ReturnsResolvedImportTarget()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_DefaultFallbackAnalysis_SupportsDiagnosticsHoverAndDefinition()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_RoutesDirectiveTemplateAndCodeRegionsThroughDistinctObservableBehaviors()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_TemplateAndCodePositions_RemainFeatureCapableAfterLaneDispatch()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CodeRegion_UsesInProcRoslynForCompletionHoverAndDefinition()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_SignatureHelp_InCodeBlock_TracksActiveParameterAcrossInvocationArguments()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_DocumentSymbols_AggregateJazorStructureAndCodeMembers()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CodeRegion_ReferencesAndRename_StayInsideCodeLane()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CodeRegion_ReferencesAndRename_WorkAcrossOpenJazorDocuments()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CodeBehindFile_ReferencesAndRename_StayInsideRoslynLane_WithoutMarkupBridge()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_ReferencesAndRename_ReturnWorkspaceLocationsAndEdits()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CodeAction_ReturnsQuickFixForPrivateMethodDiagnostic()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_CodeAction_DoesNotOfferLegacyVueImportQuickFix()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_JazorCode_CompletionHoverAndSignatureHelp_UseUnopenedDiskBackedCSharpDeclaration()
    {
        await using var client = await LspTestClient.StartAsync("--no-deno-worker");
        await client.InitializeAsync();
        ...
        try
        {
            ...
            var text =
                ...
                @code {
                    private int RenderCompletion()
                    {
                        return SharedState.Cou/*completion*/;
                    }

                    private int RenderHover()
                    {
                        return SharedState.Count;
                    }
            ...
            var completionPosition = GetPosition(text, "SharedState.Cou", advance: "SharedState.Cou".Length);
            var completionLabels = await client.RequestCompletionLabelsAsync(
                requestId: 17971,
                documentUri,
                completionPosition.Line,
            ...
            var hoverPosition = GetPosition(text, "SharedState.Count", advance: "SharedState.".Length + 1);
            var hover = await client.RequestHoverAsync(
                requestId: 17972,
                documentUri,
                hoverPosition.Line,
    ...
    [TestMethod]
    public async Task Jolt_Lsp_JazorCode_Diagnostics_UseUnopenedDiskBackedCSharpDeclaration()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_LegacyVueImportText_ProducesUnsupportedDiagnosticAndQuickFix()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_LegacyJsImportText_ProducesUnsupportedDiagnosticAndQuickFix()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    [TestMethod]
    public async Task Jolt_Lsp_LegacyImportText_ProducesUnsupportedDiagnosticAndQuickFix()
    {
        await using var client = await LspTestClient.StartAsync();
        await client.InitializeAsync();
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "jolt-lsp-tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(path);
        WriteScopedWorkspaceFiles(path, "JoltLspTests");
        return path;
    }

    // rooted 临时工作区必须带上 .slnx 和 .csproj，才能覆盖生产里的项目边界解析。
    private static void WriteScopedWorkspaceFiles(string projectRoot, string projectName)
    {
        var projectFileName = projectName + ".csproj";
        var projectPath = Path.Combine(projectRoot, projectFileName);
        File.WriteAllText(
            projectPath,
            """
    ...
    private static bool ContainsNormalizedUri(JsonElement locations, string documentPath)
        => locations.EnumerateArray().Any(location =>
            string.Equals(
                JoltWorkspaceResolver.NormalizePath(LspProtocolHelpers.ToDocumentPath(location.GetProperty("uri").GetString()!)),
    ...
    private static bool ContainsNormalizedChange(JsonElement changes, string documentPath)
        => changes.EnumerateObject().Any(change =>
            string.Equals(
                JoltWorkspaceResolver.NormalizePath(LspProtocolHelpers.ToDocumentPath(change.Name)),
                JoltWorkspaceResolver.NormalizePath(documentPath),
                StringComparison.OrdinalIgnoreCase));
    ...
    {
        private const int ReadRetryCount = 120;
        private static readonly TimeSpan ReadRetryDelay = TimeSpan.FromMilliseconds(25);
        private static readonly TimeSpan ReadAttemptTimeout = TimeSpan.FromMilliseconds(250);
            ...
            string sectionName)
        {
            for (var attempt = 0; attempt < ReadRetryCount; attempt++)
            {
                try
                {
                    using var timeoutSource = new CancellationTokenSource(ReadAttemptTimeout);
                    var read = await _output.ReadAsync(buffer, timeoutSource.Token);
                    if (read > 0)
                    {
                        return read;
                    }
                ...
                {
                    // Treat timed-out pipe reads as an empty attempt so the test fails fast
                    // instead of hanging indefinitely on an unresponsive child process.
                }
