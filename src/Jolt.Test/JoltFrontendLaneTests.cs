...
public sealed class JoltFrontendLaneTests
{
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_CompileSfc_WhenWorkerCrashes_RetriesWithinSameRequestAndRecovers()
    {
        var workerProcess = new FakeDenoWorkerProcess();
        workerProcess.SetResult(
            "compile/sfc",
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_CompileSfc_WithIgnoreStartupFailure_RetriesWithoutReturningFallback()
    {
        var workerProcess = new FakeDenoWorkerProcess();
        workerProcess.SetResult(
            "compile/sfc",
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_TemplateCompletion_WhenWorkerCrashes_RetriesWithinSameRequestAndRecovers()
    {
        var workerProcess = new FakeDenoWorkerProcess();
        workerProcess.SetResult(
            "template/completion",
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_CompileSfcAsync_WithBundledWorker_ReturnsCompiledVueModuleAndColumnAwareSourceMap()
    {
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_CompileCssModuleAsync_PreservesScopedClassHashAcrossDeclarationOnlyChanges()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_WithBundledWorker_AllowsConcurrentCompileRequestsOnSingleInstance()
    {
        var tempDirectory = CreateTemporaryDirectory();

        try
        {
            var vuePath = Path.Combine(tempDirectory, "App.vue");
            const string vueText = """
                <template>
                ...
                """;
            await File.WriteAllTextAsync(vuePath, vueText);

            var tsPath = Path.Combine(tempDirectory, "helper.ts");
            const string tsText = """
            ...
            var compileVueTask = host.CompileSfcAsync(
                vuePath,
                vueText,
                Path.GetFileName(vuePath),
                CancellationToken.None).AsTask();
            var compileTypeScriptTask = host.CompileTypeScriptAsync(
                tsPath,
                tsText,
                Path.GetFileName(tsPath),
                CancellationToken.None).AsTask();

            await Task.WhenAll(compileVueTask, compileTypeScriptTask);

            var vueResult = await compileVueTask;
            var typeScriptResult = await compileTypeScriptTask;
            Assert.IsNotNull(vueResult);
            Assert.IsNotNull(typeScriptResult);
            StringAssert.Contains(vueResult.JsContent, "message");
            StringAssert.Contains(vueResult.CssContent ?? string.Empty, ".app");
            StringAssert.Contains(typeScriptResult.JsContent, "count");
            StringAssert.Contains(typeScriptResult.JsContent, "label");
        }
        finally
        {
            DeleteDirectoryWithRetry(tempDirectory);
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_CompileTypeScript_StartsWorkerAndReturnsTypedResult()
    {
        var workerProcess = new FakeDenoWorkerProcess();
        workerProcess.SetResult(
            "compile/ts",
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_GetTemplateDiagnostics_StartsWorkerAndReturnsTypedDiagnostics()
    {
        var workerProcess = new FakeDenoWorkerProcess();
        workerProcess.SetResult(
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_GetTemplateDocumentSymbols_StartsWorkerAndReturnsTypedSymbols()
    {
        var workerProcess = new FakeDenoWorkerProcess();
        workerProcess.SetResult(
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_GetTemplateSemanticTokens_StartsWorkerAndReturnsTypedTokens()
    {
        var workerProcess = new FakeDenoWorkerProcess();
        workerProcess.SetResult(
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_TypeScriptDocument_ReturnsSameFileScriptSymbolResults()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_VueScriptSetupDocument_ReturnsSameFileScriptSymbolResults()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_VueDocument_UsesVolarSyntaxDiagnostics()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_JazorDocument_UsesFrontendMetadataForDiagnostics()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_TypeScriptDocument_ResolvesRelativeImportSymbolsConservatively()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_TypeScriptDocument_ResolvesReExportedAliasAndDefaultImportSymbolsConservatively()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_DenoFrontendHost_TypeScriptDocument_UsesBundledTypeScriptServiceForImportedMemberCompletionAndHover()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetDiagnostics_DeduplicatesMatchingDenoDiagnostics()
    {
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetDiagnostics_DoesNotRequireLegacyVueImportWhenNearbyComponentExists()
    {
        var lane = CreateLane(new FakeDenoFrontendHost());
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetCompletionItems_ReturnsEmptyWhenDenoReturnsEmpty()
    {
        var lane = CreateLane(new FakeDenoFrontendHost());
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetHover_ReturnsNullWhenDenoReturnsNull()
    {
        var lane = CreateLane(new FakeDenoFrontendHost());
        var document = CreateDocument("""
            <UserCard />
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetCompletionItems_PrefersDenoResults()
    {
        var lane = CreateLane(new FakeDenoFrontendHost
        {
            CompletionItems = new[]
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetCompletionItems_UsesWorkspaceGraphWhenDenoEnabledButTemporarilyNotRunning()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetDocumentSymbols_PrefersDenoResults()
    {
        var lane = CreateLane(new FakeDenoFrontendHost
        {
            DocumentSymbols =
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetSemanticTokens_PrefersDenoResults()
    {
        var lane = CreateLane(new FakeDenoFrontendHost
        {
            SemanticTokens =
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_UsesPrimaryProjectedVueDocument_ForJazorTemplateRequests()
    {
        var denoHost = new FakeDenoFrontendHost
        {
            HoverResult = new LspHoverResult
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_FallsBackToSource_WhenPrimaryProjectedVueDocumentIsMissing()
    {
        var denoHost = new FakeDenoFrontendHost
        {
            HoverResult = new LspHoverResult
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_UsesPrimaryProjectedVueDocument_ForJazorTemplateRequestMatrix()
    {
        var document = CreateDocument("""
            <UserCard />
            """);
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_FallsBackToSource_ForJazorTemplateRequestMatrix_WhenPrimaryProjectedVueDocumentIsMissing()
    {
        var document = CreateDocument("""
            <UserCard />
            """);
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetDefinition_ReturnsEmptyWhenDenoReturnsEmpty()
    {
        var lane = CreateLane(new FakeDenoFrontendHost());
        var document = CreateDocument("""
            <UserCard />
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetRename_ReturnsNullWhenDenoReturnsNull()
    {
        var lane = CreateLane(new FakeDenoFrontendHost());
        var document = CreateDocument("""
            <UserCard />
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetReferences_ReturnsNativeScriptLocations_ForTypeScriptVueImport()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetRename_ReturnsNativeScriptEdits_ForTypeScriptVueImport()
    {
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetCodeActions_DoesNotOfferLegacyVueImportQuickFixForMissingComponent()
    {
        var lane = CreateLane(new FakeDenoFrontendHost());
        var document = CreateDocument("""
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetCodeActions_DoesNotOfferLegacyVueImportQuickFixWhenNoImportsExist()
    {
        var lane = CreateLane(new FakeDenoFrontendHost());
        var document = CreateDocument("""
            <MissingCard />
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetCodeActions_DoesNotOfferLegacyVueImportQuickFixWhenNearbyComponentExists()
    {
        var lane = CreateLane(new FakeDenoFrontendHost());
        var tempDirectory = CreateTemporaryDirectory();
    ...
    [TestMethod]
    public async Task Jolt_FrontendLaneService_GetCodeActions_DoesNotOfferLegacyVueImportPathRewrite()
    {
        var lane = CreateLane(new FakeDenoFrontendHost());
        var tempDirectory = CreateTemporaryDirectory();
    ...
    private static string CreateTemporaryDirectory()
        => JoltIntegrationRootedProjectDirectory.Create(
            scenarioName: nameof(JoltFrontendLaneTests),
            solutionName: nameof(JoltFrontendLaneTests),
            projectName: nameof(JoltFrontendLaneTests));
    ...
    {
        var builtProjectOutputPath = Path.Combine(
            GetRepositoryRoot(),
            "src",
            "Jolt",
            "bin",
        ...
        if (IsUsableDenoHostBaseDirectory(testOutputPath)
        ...
        Assert.Fail(
            $"Expected built Jolt output '{builtProjectOutputPath}' or a complete shadow copy '{testOutputPath}' to contain a ready Deno host.");
        return builtProjectOutputPath;
    }
    ...
    // 纯内存夹具默认使用相对路径，避免无关用例被 .slnx 项目边界校验拦截。
