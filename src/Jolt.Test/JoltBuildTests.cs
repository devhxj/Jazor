...
[TestClass]
public sealed class JoltBuildTests
{
    [TestMethod]
    ...
    [TestMethod]
    public async Task DenoBuildImportMapGenerator_GenerateAsync_ReusesExistingFile_WhenImportsAreUnchanged()
    {
        var tempDir = CreateTemporaryDirectory();
        try
        {
            ...
            var importMapPath = await DenoBuildImportMapGenerator.GenerateAsync(tempDir, CancellationToken.None);
            var originalJson = await File.ReadAllTextAsync(importMapPath);
            var originalWriteTime = SetDistinctLastWriteTimeUtc(importMapPath, TimeSpan.FromMinutes(-5));

            var secondImportMapPath = await DenoBuildImportMapGenerator.GenerateAsync(tempDir, CancellationToken.None);

            Assert.AreEqual(importMapPath, secondImportMapPath);
            Assert.AreEqual(originalWriteTime, File.GetLastWriteTimeUtc(importMapPath));
            var regeneratedJson = await File.ReadAllTextAsync(importMapPath);
            Assert.AreEqual(originalJson, regeneratedJson);
            Assert.IsTrue(
                regeneratedJson.IndexOf("\"@vueuse/core\"", StringComparison.Ordinal) < regeneratedJson.IndexOf("\"typescript\"", StringComparison.Ordinal),
                "Expected import map serialization to stay deterministic across dependency groups.");
            Assert.IsTrue(
                regeneratedJson.IndexOf("\"typescript\"", StringComparison.Ordinal) < regeneratedJson.IndexOf("\"vue\"", StringComparison.Ordinal),
                "Expected default and package imports to be serialized in stable key order.");
        }
        finally
        {
            DeleteDirectory(tempDir);
