...
internal static class RazorSourceGeneratorTailOutput
{
        ...
        bool requiresTailOutput)
    {
        ...
        if (!TryBuildVueRenderArtifacts(
                context,
                razorVueBinding!,
                out var vueRenderArtifacts,
                out var artifactFailure))
        {
            ReportTailFailure(context, artifactFailure ?? "Unknown RazorVue render artifact generation failure.");
            EmitTraceIfEnabled(
                context,
                options,
                documents.Length,
                ...
                "vue-render-artifact-failed",
                sourceTypeName: artifactFailure,
                binding: razorVueBinding);
            return;
        }

        EmitFinalDocumentEvidence(context, razorVueBinding!);
        EmitVueRenderCatalog(context, vueRenderArtifacts);
        EmitTraceIfEnabled(
            context,
            options,
            documents.Length,
    ...
    private static void EmitVueRenderCatalog(
        SourceProductionContext context,
        ImmutableArray<RazorSgVueComponentModuleArtifact> artifacts)
    {
        if (artifacts.IsDefaultOrEmpty)
            return;

        context.AddSource(
            "Jazor.Generated.VueRenderCatalog.g.cs",
            BuildVueRenderCatalogSource(artifacts));
    }
