...
- the current logic lane is still a conservative subset, but it now includes lifecycle safe-subset lowering plus a minimal setup-side logic closure for simple fields and helper calls whose arguments can be lowered safely, including fixed-depth two-level helper composition with explicit `JAZORVGA006` rejection beyond that boundary
...
- minimal setup-side logic lowering for simple instance fields and helper methods whose arguments can be projected safely into `setup()`, now including fixed-depth two-level helper composition and explicit `JAZORVGA006` projection for deeper or unsupported setup helper chains
