// .tmp/pinia-build/node_modules/pinia/dist/pinia.mjs
import { hasInjectionContext, inject, toRaw as toRaw2, watch, unref, markRaw, effectScope, ref, isRef as isRef2, isReactive as isReactive2, getCurrentScope, onScopeDispose, getCurrentInstance, reactive, toRef, nextTick, computed, toRefs } from "vue";

// .tmp/pinia-build/node_modules/@vue/devtools-shared/dist/index.js
...
// .tmp/pinia-build/node_modules/perfect-debounce/dist/index.mjs
...
// .tmp/pinia-build/node_modules/hookable/dist/index.mjs
...
// .tmp/pinia-build/node_modules/@vue/devtools-kit/dist/index.js
...
function patchObject(newState, oldState) {
  for (const key in oldState) {
    ...
    if (isPlainObject(targetValue) && isPlainObject(subPatch) && !isRef2(subPatch) && !isReactive2(subPatch)) {
...
function mergeReactiveObjects(target2, patchToApply) {
  ...
  for (const key in patchToApply) {
    ...
    if (isPlainObject(targetValue) && isPlainObject(subPatch) && target2.hasOwnProperty(key) && !isRef2(subPatch) && !isReactive2(subPatch)) {
...
function createSetupStore($id, setup, options = {}, pinia, hot, isOptionsStore) {
  ...
  for (const key in setupStore) {
    ...
    if (isRef2(prop) && !isComputed(prop) || isReactive2(prop)) {
      if (hot) {
  ...
  assign(toRaw2(store), setupStore);
...
function storeToRefs(store) {
  ...
  for (const key in rawStore) {
    ...
    } else if (isRef2(value) || isReactive2(value)) {
      refs[key] = // ---
      toRef(store, key);
    }
  }
