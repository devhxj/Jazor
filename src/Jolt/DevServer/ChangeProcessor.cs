...
internal sealed class ChangeProcessor
{
    ...
    private static IEnumerable<string> EnumerateWorkspaceSourcePaths(string effectivePath)
    {
        ...
        foreach (var companionPath in JoltWorkspaceResolver.GetCoLocatedCodeBehindPaths(effectivePath))
        {
        ...
        IReadOnlyDictionary<string, DocumentSnapshot>? documentOverrides)
    {
        ...
        foreach (var companionPath in JoltWorkspaceResolver.GetCoLocatedCodeBehindPaths(changedPath))
        {
        ...
        string EffectivePath)
    {
        ...
        {
            if (JoltWorkspaceResolver.TryResolveOwningJazorPath(originalPath, out var jazorDocumentPath))
            {
