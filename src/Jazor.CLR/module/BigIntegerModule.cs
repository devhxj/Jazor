...
public static class BigIntegerModule
{
	...
	[Jazor(Op.Allowed, "System.Numerics.BigInteger.BigInteger(int)")]
	...
	[Jazor(Op.Allowed, "System.Numerics.BigInteger.BigInteger(uint)")]
	...
	[Jazor(Op.Allowed, "System.Numerics.BigInteger.BigInteger(long)")]
	...
	[Jazor(Op.Allowed, "System.Numerics.BigInteger.BigInteger(ulong)")]
	...
	[Jazor(Op.Allowed, "System.Numerics.BigInteger.BigInteger(float)")]
	...
	[Jazor(Op.Allowed, "System.Numerics.BigInteger.BigInteger(double)")]
	...
	[Jazor(Op.Allowed, "System.Numerics.BigInteger.BigInteger(System.Decimal)")]
	...
	public static BigInt _c1e724fa6dbf63eb(Array<byte> value)
	{
		...
		for (; i + 8 <= value.Length; i += 8)
			result = (result << Global.BigInt(64)) | view.GetBigUint64(i, false);
		...
		if (i < value.Length)
		{
			...
			for (; i < value.Length; i++)
				remaining = (remaining << Global.BigInt(8)) | Global.BigInt(value[i]);

			result = (result << Global.BigInt((value.Length - i) * 8u)) | remaining;
		}
	...
	public static BigInt _9c321a7400e5ff9b(Array<byte> value, bool isUnsigned, bool isBigEndian)
	{
		...
		if (value.Length == 1)
		{
			if (isUnsigned)
			{
				return Global.BigInt(value[0]);
			}
			else
			{
				// 有符号数：如果最高位为1，则为负数
				return (value[0] & 0x80) == 0
					? Global.BigInt(value[0])
					: Global.BigInt(value[0]) - Global.BigInt(0x100);
			}
		}
		...
		if (value.Length <= 8)
		{
			...
			{
				2 => isUnsigned ?
					Global.BigInt(view.GetUint16(0, !isBigEndian)) :
					Global.BigInt(view.GetInt16(0, !isBigEndian)),
				4 => isUnsigned ?
					Global.BigInt(view.GetUint32(0, !isBigEndian)) :
					Global.BigInt(view.GetInt32(0, !isBigEndian)),
				8 => isUnsigned ?
		...
		static BigInt ProcessNonStandardLength(Array<byte> bytes, bool isUnsigned, bool isBigEndian)
		{
			...
			{
				// 计算位宽度 = 字节数 * 8
				var bitWidth = Global.BigInt(processedBytes.Length) * Global.BigInt(8);
		...
		static BigInt BuildBigIntFromLEBytes(Array<byte> littleEndianBytes)
		{
			...
			{
				// 移位8位拼接当前字节值
				result = (result << Global.BigInt(8)) | Global.BigInt(littleEndianBytes[i] & 0xFF);
			}
	...
	[Jazor(Op.Inline, "System.Numerics.BigInteger.Sign.get", "@#{0} == 0n ? 0 : (@#{0} > 0n ? 1 : -1)")]
	...
	[Jazor(Op.Inline, "static System.Numerics.BigInteger.Parse(string)", "BigInt(@#{0})")]
	public extern static BigInt _155212572c9a3297(object value);
	...
	public extern static BigInt _a077721686cadcd9(object value, object style);
	...
	public extern static BigInt _d1543aa14ab94729(object value, Intl.NumberFormat? provider);
	...
	public extern static BigInt _8adf758c3f22af12(object value, object style, Intl.NumberFormat? provider);
	...
	{
		{
			...
			{
				if (value?.Length > 0)
					return [true, Global.BigInt(value)];
			}
			catch { }
			return [false, null];
		}
	...
	public extern static bool _85cd9c4a9c2dadf4(object value, object style, Intl.NumberFormat? provider, Box<BigInt?> result);
	...
	public extern static bool _d733f0a0a427d970(Uint32Array value, object style, Intl.NumberFormat? provider, Box<BigInt> result);
	...
	[Jazor(Op.Compile, "static System.Numerics.BigInteger.Compare(System.Numerics.BigInteger, System.Numerics.BigInteger)")]
	...
	public extern static BigInt _598611fb2b8a064a(BigInt dividend, BigInt divisor, Box<BigInt> remainder);
	...
	public extern static bool _76ae4e496fc976fd(BigInt instance, Uint8Array destination, Box<Number> bytesWritten, object isUnsigned, object isBigEndian);
	...
	public extern static string _1650d30e3e9172f5(BigInt instance, object format);
	...
	public extern static string _93b0cfb45a1832e9(BigInt instance, object format, Intl.NumberFormat? provider);
	...
	public extern static bool _10999a356af78aba(object s, Intl.NumberFormat? provider, Box<BigInt?> result);
