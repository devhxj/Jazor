...
/// <summary>
/// Generated Blazor navigation extension signatures with browser-runtime imports.
...
{
	[Jazor(Op.Import, "static Microsoft.AspNetCore.Components.NavigationManagerExtensions.GetUriWithQueryParameter(Microsoft.AspNetCore.Components.NavigationManager, string, bool)", "getUriWithQueryParameterBoolean")]
	...
	public static string _a73974608eac29dc(Object navigationManager, string name, string? value)
		=> GetUriWithQueryParameterCore(GetUri(navigationManager), name, value);

	[Jazor(Op.Import, "static Microsoft.AspNetCore.Components.NavigationManagerExtensions.GetUriWithQueryParameters(Microsoft.AspNetCore.Components.NavigationManager, System.Collections.Generic.IReadOnlyDictionary<string, object>)", "getUriWithQueryParameters")]
	public static string _ca7b14a85eb99749(Object navigationManager, global::System.Collections.Generic.IReadOnlyDictionary<string, object> parameters)
		=> GetUriWithQueryParametersCore(GetUri(navigationManager), parameters);

	[Jazor(Op.Import, "static Microsoft.AspNetCore.Components.NavigationManagerExtensions.GetUriWithQueryParameters(Microsoft.AspNetCore.Components.NavigationManager, string, System.Collections.Generic.IReadOnlyDictionary<string, object>)", "getUriWithQueryParametersFromUri")]
	...
	internal static string GetUri(object navigationManager)
	{
		...
		var uri = ECMAScript.Reflect.Get(navigationManager, "uri");
		if (ECMAScript.Global.TypeOf(uri) != "string")
			throw new Error("InvalidOperationException: NavigationManager does not expose a URI.");

		return (string)uri!;
	}

	internal static string GetUriWithQueryParameterCore(string uri, string name, object? value)
		=> RebuildQuery(uri, [CreateQueryUpdate(name, value)]);

	internal static string GetUriWithQueryParametersCore(
		string uri,
		global::System.Collections.Generic.IReadOnlyDictionary<string, object> parameters)
	{
		...
		var updates = new Array<Array<object?>>();
		foreach (var parameter in parameters)
			updates.Push(CreateQueryUpdate(parameter.Key, parameter.Value));
		return RebuildQuery(uri, updates);
	}

	internal static string GetUriWithQueryParametersObjectCore(
		string uri,
		object parameters)
	{
		if (parameters is null)
		...
		for (var index = 0; index < names.Length; index++)
		{
			...
			updates.Push(CreateQueryUpdate(name, ECMAScript.Reflect.Get(parameters, name)));
		}
		return RebuildQuery(uri, updates);
	}

	internal static string GetUriWithFragmentCore(string uri, string? fragment)
	{
		if (uri is null)
			throw new Error("ArgumentNullException: uri is null.");

		var hashIndex = uri.IndexOf("#", StringComparison.Ordinal);
		var prefix = hashIndex < 0 ? uri : uri[..hashIndex];
		return string.IsNullOrEmpty(fragment)
			? prefix
			: prefix + (fragment!.StartsWith("#", StringComparison.Ordinal) ? fragment : "#" + fragment);
	}

	private static Array<object?> CreateQueryUpdate(string name, object? value)
	{
	...
	private static string RebuildQuery(string uri, Array<Array<object?>> updates)
	{
		if (uri is null)
			throw new Error("ArgumentNullException: uri is null.");

		var hashIndex = uri.IndexOf("#", StringComparison.Ordinal);
		var hash = hashIndex < 0 ? "" : uri[hashIndex..];
		var beforeHash = hashIndex < 0 ? uri : uri[..hashIndex];
		var queryIndex = beforeHash.IndexOf("?", StringComparison.Ordinal);
		var prefix = queryIndex < 0 ? beforeHash : beforeHash[..queryIndex];
		...
		if (query.Length != 0)
		{
			...
			for (var entryIndex = 0; entryIndex < entries.Length; entryIndex++)
			{
				...
				var equalsIndex = entry.IndexOf("=", StringComparison.Ordinal);
				var encodedName = equalsIndex < 0 ? entry : entry[..equalsIndex];
				var updateIndex = FindUpdateIndex(updates, encodedName);
				if (updateIndex < 0)
				{
					emitted.Push(entry);
					continue;
	...
	private static int FindUpdateIndex(Array<Array<object?>> updates, string encodedName)
	{
		for (var index = 0; index < updates.Length; index++)
		{
			if (encodedName.ToLower() == EscapeQueryComponent((string)updates[index][0]!).ToLower())
				return index;
		}
		return -1;
	}

	private static bool AppendNextValue(Array<string> emitted, Array<object?> update, Number offset)
	{
		var name = (string)update[0]!;
		var suppliedValue = update[1];
		if (suppliedValue is null)
			return false;

		if (Array<object?>.IsArray(suppliedValue))
		{
			var values = (Array<object?>)suppliedValue;
	...
	private static string EscapeQueryComponent(string value)
		=> ECMAScript.Global.EncodeURIComponent(value)
			.Replace("!", "%21")
			.Replace("'", "%27")
			.Replace("(", "%28")
			.Replace(")", "%29")
	...
	{
		if (ECMAScript.Global.TypeOf(value) == "boolean")
			return (bool)value ? "True" : "False";

		return RuntimeModule.GetStringRepresentation(value);
	}
}
