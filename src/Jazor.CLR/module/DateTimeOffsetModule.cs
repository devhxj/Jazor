...
public static class DateTimeOffsetModule
{
	...
	private static BigInt MaxOffsetTicks => BigIntValue("504000000000");
	private static BigInt MaxDateTimeTicks => BigIntValue("3155378975999999999");
	private static BigInt MinUnixTimeMilliseconds => BigIntValue("-62135596800000");
	private static BigInt MaxUnixTimeMilliseconds => BigIntValue("253402300799999");
	private static BigInt MinUnixTimeSeconds => BigIntValue("-62135596800");
	private static BigInt MaxUnixTimeSeconds => BigIntValue("253402300799");
	...
	{
		var utcTicks = BigIntValue(utcDateTime.GetTime()) * TicksPerMillisecond + utcSubMillisecondTicks + UnixEpochTicks;
	...
	private static BigInt GetDateTimeInstantTicks(RuntimeModule.JDateTime dateTime)
	{
		...
		return BigIntValue(dateTime.Date.GetTime()) * TicksPerMillisecond + dateTime.SubMillisecondTicks + UnixEpochTicks;
	}
	...
	private static BigInt GetUtcTicks(RuntimeModule.JDateTimeOffset instance)
		=> BigIntValue(instance.UtcDateTime.GetTime()) * TicksPerMillisecond + instance.UtcSubMillisecondTicks + UnixEpochTicks;
	...
	private static RuntimeModule.JDateTimeOffset CreateFromUtcTicks(BigInt utcTicks, BigInt offsetTicks)
	{
		...
		return CreateDateTimeOffset(new Date(NumberValue(milliseconds)), offsetTicks, utcSubMillisecondTicks);
	}
	...
	private static RuntimeModule.JDateTimeOffset AddMonthsCore(RuntimeModule.JDateTimeOffset instance, Number months)
	{
		...
		var local = new Date(instance.UtcDateTime.GetTime() + NumberValue(instance.OffsetTicks) / 10000);
	...
	{
		var offsetTicks = BigIntValue(-utcDateTime.GetTimezoneOffset()) * OffsetMinuteTicks;
	...
	private static Date GetOffsetLocalDate(RuntimeModule.JDateTimeOffset instance)
		=> new(instance.UtcDateTime.GetTime() + NumberValue(instance.OffsetTicks) / 10000);
	...
	private static string FormatCustomToken(RuntimeModule.JDateTimeOffset instance, char token, int count, string locale, string dateSeparator, string timeSeparator)
	{
		...
		var fraction = BigIntValue(local.GetUTCMilliseconds()) * TicksPerMillisecond + instance.UtcSubMillisecondTicks;
		...
		out BigInt offsetTicks)
	{
		...
		offsetTicks = BigIntValue(offsetHours * 60 + offsetMinutes) * OffsetMinuteTicks;
		if (sign == '-')
	...
	private static Number GetDateTimeStylesValue(object styles)
	{
		...
		if (styles is System.Globalization.DateTimeStyles enumStyle)
			return NumberValue((int)enumStyle);
		if (styles == null)
			return 0;
	...
	private static BigInt ResolveParsedOffsetTicks(string input, Date parsedDate)
	{
		...
		if (input.Length >= 6)
		{
			...
			{
				var hours = NumberValue(input.Substring(input.Length - 5, 2));
				var minutes = NumberValue(input.Substring(input.Length - 2, 2));
				if (signIndex > timeIndex && !IsNaN(hours) && !IsNaN(minutes) && minutes >= 0 && minutes < 60)
				{
					var ticks = BigIntValue(hours * 60 + minutes) * OffsetMinuteTicks;
		...
		if (input.Length >= 5)
		{
			...
			if (sign == '+' || sign == '-')
			{
				var hours = NumberValue(input.Substring(input.Length - 4, 2));
				var minutes = NumberValue(input.Substring(input.Length - 2, 2));
				if (signIndex > timeIndex && !IsNaN(hours) && !IsNaN(minutes) && minutes >= 0 && minutes < 60)
				{
					var ticks = BigIntValue(hours * 60 + minutes) * OffsetMinuteTicks;
		...
		if (input.Length >= 3)
		{
			...
			if (sign == '+' || sign == '-')
			{
				var hours = NumberValue(input.Substring(input.Length - 2, 2));
				if (signIndex > timeIndex && !IsNaN(hours))
				{
					var ticks = BigIntValue(hours * 60) * OffsetMinuteTicks;
					return sign == '-' ? -ticks : ticks;
				}
			}
		}

		return BigIntValue(-parsedDate.GetTimezoneOffset()) * OffsetMinuteTicks;
	}
	...
	private static RuntimeModule.JDateTimeOffset ParseCore(string input)
	{
		...
		if (TryParseTimeOnly(s, out var timeHour, out var timeMinute, out var timeSecond, out var timeMillisecond, out var timeSubMillisecondTicks, out var timeHasExplicitOffset, out var timeOffsetTicks))
		{
			...
			if (!timeHasExplicitOffset)
			{
				...
				var localOffsetTicks = BigIntValue(-localDateTime.GetTimezoneOffset()) * OffsetMinuteTicks;
				var utcTicks = BigIntValue(localDateTime.GetTime()) * TicksPerMillisecond + timeSubMillisecondTicks + UnixEpochTicks;
		...
		if (TryParseIsoDate(s, out var year, out var month, out var day))
		{
			...
			var utcTicks = BigIntValue(date.GetTime()) * TicksPerMillisecond + UnixEpochTicks;
		...
		if (TryParseIsoDateTime(s, out year, out month, out day, out var hour, out var minute, out var second, out var millisecond, out var subMillisecondTicks, out var hasExplicitOffset, out var offsetTicks))
		{
			if (!hasExplicitOffset)
			{
				...
				var localOffsetTicks = BigIntValue(-localDateTime.GetTimezoneOffset()) * OffsetMinuteTicks;
				var utcTicks = BigIntValue(localDateTime.GetTime()) * TicksPerMillisecond + subMillisecondTicks + UnixEpochTicks;
				return CreateFromUtcTicks(utcTicks, localOffsetTicks);
			}
		...
		return CreateFromUtcTicks(BigIntValue(parsed.GetTime()) * TicksPerMillisecond + parsedSubMillisecondTicks + UnixEpochTicks, resolvedOffsetTicks);
	}
	...
	public static RuntimeModule.JDateTimeOffset _d45d439f0b97ae0e() => CreateDateTimeOffset(new Date(253402300799999d), ZeroTicks, BigIntValue("9999"));
	...
	public static RuntimeModule.JDateTimeOffset _106dabc0cc502aa4(RuntimeModule.JDateTime dateTime, RuntimeModule.JTimeSpan offset)
	{
		...
		{
			var expectedOffset = BigIntValue(-dateTime.Date.GetTimezoneOffset()) * OffsetMinuteTicks;
	...
	public static RuntimeModule.JDateTimeOffset _04123d597aa761a3(Number year, Number month, Number day, Number hour, Number minute, Number second, Number millisecond, Number microsecond, RuntimeModule.JTimeSpan offset)
	{
		...
		var localTicks = CreateLocalTicks(year, month, day, hour, minute, second, millisecond) + BigIntValue(microsecond) * BigIntValue("10");
		return CreateFromUtcTicks(localTicks - offset.Ticks, offset.Ticks);
	}
	...
	public static RuntimeModule.JDateTime _2b7dd675863ae961(RuntimeModule.JDateTimeOffset instance)
	{
		...
		return new RuntimeModule.JDateTime(
			RuntimeModule.CreateLocalDateTime(
				NumberValue(_127105b7a40a7665(instance)),
				NumberValue(_79eb4c93cea58d59(instance)),
				NumberValue(_ba8df912681fe784(instance)),
				NumberValue(_b7fc65477ef4df45(instance)),
				NumberValue(_0fe8054b55f9f1c7(instance)),
				NumberValue(_822de224fed5bb6b(instance)),
				NumberValue(_0c1b2675cd7a2faa(instance))),
			0,
	...
	public static Number _b69ef2b7d0abde1a(RuntimeModule.JDateTimeOffset instance)
	{
		...
		return Math.FloorFunc((local.GetTime() - start) / 86400000);
	}
	...
	public static Number _ae3a48995f0953ed(RuntimeModule.JDateTimeOffset instance)
		=> NumberValue((instance.UtcSubMillisecondTicks / BigIntValue("10")) % BigIntValue(1000));
	...
	public static Number _f9acef215c7d5168(RuntimeModule.JDateTimeOffset instance)
		=> NumberValue((instance.UtcSubMillisecondTicks % BigIntValue("10")) * BigIntValue(100));
	...
	{
		var normalized = GetTicks(instance) % BigIntValue("864000000000");
		return new RuntimeModule.JTimeSpan(normalized < ZeroTicks ? normalized + BigIntValue("864000000000") : normalized);
	}
	...
	public static RuntimeModule.JDateTimeOffset _fb7d72712794a2e4(BigInt seconds)
	{
		...
		return CreateDateTimeOffset(new Date(NumberValue(seconds * BigIntValue(1000))), ZeroTicks);
	}
	...
	public static RuntimeModule.JDateTimeOffset _89071e7da78164f5(BigInt milliseconds)
	{
		...
		return CreateDateTimeOffset(new Date(NumberValue(milliseconds)), ZeroTicks);
	}
	...
	public static BigInt _8bc213443653978d(RuntimeModule.JDateTimeOffset instance)
		=> FloorDiv(GetUtcTicks(instance) - UnixEpochTicks, BigIntValue("10000000"));
	...
	{
		var now = new Date();
		var offsetTicks = BigIntValue(-now.GetTimezoneOffset()) * OffsetMinuteTicks;
