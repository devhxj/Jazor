...
public static partial class css
{
    ...
    private static void EnsureDomStyle(CssContext context)
    {
        ...
        if (context.Target is null)
        {
            ...
            if (head is null)
            {
                ...
                Fail("ECMAScript.Style requires document.head to inject styles.");
            }
        ...
        else
        {
            ...
            if (ownerDocument is null)
                Fail("ECMAScript.Style target must belong to a document.");
        ...
        if (existing.LocalName != "style")
            Fail("ECMAScript.Style StyleId '" + context.StyleId + "' is already used by a non-style element.");

        var existingStyle = (HTMLStyleElement)existing;
        var text = existingStyle.TextContent ?? "";
        if (!text.StartsWith(RootMarker))
            Fail("ECMAScript.Style StyleId '" + context.StyleId + "' is not owned by ECMAScript.Style.");

        if (context.Nonce is not null && existingStyle.Nonce != context.Nonce)
            Fail("ECMAScript.Style nonce does not match the existing style element.");
    ...
    private static void AdoptDomEntries(CssContext context, Document document, string text)
    {
        ...
        while (offset < text.Length)
        {
            if (!text.Substring(offset).StartsWith(EntryMarkerPrefix))
                Fail("ECMAScript.Style style element contains a malformed entry marker.");
            ...
            if (idEnd <= idStart || headerEnd < 0)
                Fail("ECMAScript.Style style element contains a malformed entry header.");
            ...
            if (bodyEnd > text.Length)
                Fail("ECMAScript.Style style entry extends beyond the style text.");

            var body = text.Substring(bodyStart, bodyLength);
            if (adoptedBodyById.Has(id))
            {
                if (adoptedBodyById.Get(id) != body)
                    Fail("ECMAScript.Style style element contains one ID with different CSS bodies.");
            }
            else
            {
    ...
    {
        if (value.Length == 0)
            Fail("ECMAScript.Style style entry length is empty.");
        ...
        for (var index = 0; index < value.Length; index++)
        {
            ...
            if (codeUnit < 48 || codeUnit > 57)
                Fail("ECMAScript.Style style entry length is invalid.");
    ...
    private static void AppendDomEntry(CssContext context, string id, string body)
    {
        ...
        if (document is null)
            Fail("ECMAScript.Style style element must belong to a document.");
