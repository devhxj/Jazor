...
namespace Jazor.RazorVue.Analysis.RenderTree;
...
internal sealed class RazorVueRenderTreeExtractor
{
    ...
    private sealed class ComponentBuilder(string componentName, string componentFullName, ImmutableArray<RazorVueSourceOrigin> origins)
        : OpenNodeBuilder(origins)
    {
        public override RazorVueRenderNode Build()
            => new RazorVueComponentNode(componentName, componentFullName, BuildAttributes(), BuildChildren(), Origins);
    }
}
