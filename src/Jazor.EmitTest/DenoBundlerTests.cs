...
[TestClass]
public sealed class DenoBundlerTests
{
    [TestMethod]
    public async Task BundleAsync_SingleRootModule_PreservesExports()
    {
        ...
        var manifest = new ManifestModel(
            ...
            Modules:
            [
                new ModuleEntry("SdkSmoke", "SampleModule", "SampleModule", "SdkSmoke/SampleModule.mjs", "hash-1")
            ]);
    ...
    public async Task BundleAsync_MultiProjectHostBundle_ReExportsRootAssemblyMembers()
    {
        ...
        var manifest = new ManifestModel(
            ...
            Modules:
            [
                new ModuleEntry("Sample.Contracts", "Sample.Contracts.GreetingSharedModule", "Sample.Contracts.GreetingSharedModule", "shared/greetings.mjs", "hash-1"),
                new ModuleEntry("Sample.Features", "Sample.Features.GreeterModule", "Sample.Features.GreeterModule", "features/greeter.mjs", "hash-2"),
                new ModuleEntry("Sample.Host", "Sample.Host.AppModule", "Sample.Host.AppModule", "host/app.mjs", "hash-3")
            ]);
        manifest.Save(workspace.ManifestPath);
    ...
    [TestMethod]
    public async Task BundleAsync_WritesExternalSourceMapAndRazorSourceMapUrl()
    {
        using var workspace = new TestWorkspace();
        WriteMappedModule(
            workspace.InputDirectory,
        ...
        var manifest = new ManifestModel(
            ...
            Modules:
            [
                new ModuleEntry("Sample.Host", "Sample.Host.AppModule", "Sample.Host.AppModule", "host/app.mjs", "hash-1")
            ]);
        manifest.Save(workspace.ManifestPath);
    ...
    public void ManifestModel_SaveAndLoad_PreservesExplicitFrontendAssets()
    {
        ...
        Assert.AreEqual(AssetEntry.KindVueSfc, loaded.Assets[0].Kind);
        Assert.AreEqual("hash-asset-1", loaded.Assets[0].Hash);
        CollectionAssert.AreEqual(
            new[] { "vue", "vue-router" },
            loaded.Modules.Single().PackageImports?.ToArray());
    }
    ...
    private static string FindVueLibraryManifest()
    {
        ...
        throw new FileNotFoundException("Could not locate the packaged Vue library manifest from the test output.");
    }
