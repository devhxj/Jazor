...
public sealed class RazorVueSfcArtifactFactoryTests
{
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithStringOnAttributeAndEventModifier_EmitsPlainHtmlAttribute()
    {
        var context = CreateContext(
            """
            using System;
        ...
        StringAssert.Contains(artifact.TemplateText, "Go");
        Assert.IsFalse(artifact.TemplateText.Contains("@click", StringComparison.Ordinal), artifact.TemplateText);
        Assert.IsFalse(artifact.TemplateText.Contains(".prevent", StringComparison.Ordinal), artifact.TemplateText);
        Assert.IsFalse(artifact.TemplateText.Contains(":onclick", StringComparison.Ordinal), artifact.TemplateText);
    }
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithSideEffectingAddAssignStep_UsesRenderFunctionToPreservePerIterationEvaluation()
    {
        var context = CreateContext(
            """
            using System;
        ...
        Assert.AreEqual(VueSfcArtifactRenderMode.RenderFunction, artifact.RenderMode);
        StringAssert.Contains(artifact.SfcText, "for (let i = 0; i < props.count; i += ++props.step)");
        Assert.IsFalse(artifact.SfcText.Contains("__jazorVueForRange", StringComparison.Ordinal), artifact.SfcText);
    }
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithDeclarativeSiblingsAroundSimpleConstantSwitchStatementInBuildRenderTree_LowersTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
                    ...
                    protected override void BuildRenderTree(RenderTreeBuilder builder)
                    {
                        ...
                        switch (Count)
                        {
                            ...
                            case 1:
                                ...
                                builder.AddContent(5, "one");
                                builder.CloseElement();
                                break;
                            default:
                                builder.OpenElement(6, "section");
        ...
        Assert.IsTrue(artifact.UsesScriptSetup, artifact.SfcText);
        Assert.IsFalse(artifact.SfcText.Contains("<script lang=\"ts\">", StringComparison.Ordinal), artifact.SfcText);
        StringAssert.Contains(artifact.TemplateText, "<header>");
        StringAssert.Contains(artifact.TemplateText, "start");
        StringAssert.Contains(artifact.TemplateText, "<template v-for=\"(__jazorSwitchValue) in [props.count]\">");
        StringAssert.Contains(artifact.TemplateText, "<template v-if=\"__jazorSwitchValue === 0\">");
        StringAssert.Contains(artifact.TemplateText, "<p>");
        StringAssert.Contains(artifact.TemplateText, "empty");
        ...
        StringAssert.Contains(artifact.TemplateText, "end");
        Assert.AreEqual(HmrBoundaryKind.TemplateOnly, artifact.Identity.HmrBoundaryKind);
    }

    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithElementChildrenAroundSimpleConstantSwitchStatementInBuildRenderTree_LowersTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
                    ...
                    protected override void BuildRenderTree(RenderTreeBuilder builder)
                    {
                        ...
                        switch (Count)
                        {
                            case 0:
                                ...
                                builder.AddContent(4, "empty");
                                builder.CloseElement();
                                break;
                            default:
                                builder.OpenElement(5, "section");
                                builder.AddContent(6, Count);
                                builder.CloseElement();
                                break;
                        }

                        builder.OpenElement(7, "footer");
                        builder.AddContent(8, "end");
                        builder.CloseElement();
                        builder.CloseElement();
                    }
                }
        ...
        Assert.IsTrue(artifact.UsesScriptSetup, artifact.SfcText);
        Assert.IsFalse(artifact.SfcText.Contains("<script lang=\"ts\">", StringComparison.Ordinal), artifact.SfcText);
        StringAssert.Contains(artifact.TemplateText, "<article>");
        StringAssert.Contains(artifact.TemplateText, "<header>");
        StringAssert.Contains(artifact.TemplateText, "start");
        StringAssert.Contains(artifact.TemplateText, "<template v-for=\"(__jazorSwitchValue) in [props.count]\">");
        StringAssert.Contains(artifact.TemplateText, "<template v-if=\"__jazorSwitchValue === 0\">");
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithLocalNameCollisionAroundSimpleConstantSwitchStatementInBuildRenderTree_LowersTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
            ...
            namespace Demo.Components
            {
                ...
                public class SimpleSwitchCard : ComponentBase, IVueComponent
                {
                    ...
                    protected override void BuildRenderTree(RenderTreeBuilder builder)
                    {
                        var __jazorSwitchValue = Title;
                        builder.OpenElement(0, "header");
                        builder.AddContent(1, __jazorSwitchValue);
                        builder.CloseElement();
                        ...
                        switch (Count)
                        {
                            case 0:
                                builder.OpenElement(2, "p");
                                builder.AddContent(3, __jazorSwitchValue);
                                builder.CloseElement();
                                break;
                            default:
                                builder.OpenElement(4, "section");
                        ...
                        builder.AddContent(7, __jazorSwitchValue);
                        builder.CloseElement();
                    }
                }
            }
        ...
        Assert.IsTrue(artifact.UsesScriptSetup, artifact.SfcText);
        Assert.IsFalse(artifact.SfcText.Contains("<script lang=\"ts\">", StringComparison.Ordinal), artifact.SfcText);
        StringAssert.Contains(artifact.TemplateText, "<template v-for=\"(__jazorSwitchValue) in [props.title]\">");
        StringAssert.Contains(artifact.TemplateText, "<header>");
        StringAssert.Contains(artifact.TemplateText, "{{ __jazorSwitchValue }}");
        StringAssert.Contains(artifact.TemplateText, "<template v-for=\"(__jazorSlotContext) in [props.count]\">");
        StringAssert.Contains(artifact.TemplateText, "<template v-if=\"__jazorSlotContext === 0\">");
        StringAssert.Contains(artifact.TemplateText, "<p>");
        StringAssert.Contains(artifact.TemplateText, "<section>");
        StringAssert.Contains(artifact.TemplateText, "{{ props.count }}");
        StringAssert.Contains(artifact.TemplateText, "<footer>");
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithDeclarativeSiblingsAroundTryCatchFinallyNoOpRecoveryCleanupInBuildRenderTree_LowersTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
            ...
            namespace Demo.Components
            {
                ...
                public class TryCard : ComponentBase, IVueComponent
                {
                    ...
                    protected override void BuildRenderTree(RenderTreeBuilder builder)
                    {
                        ...
                        {
                            builder.OpenElement(2, "section");
                            builder.AddContent(3, localTitle);
                            builder.CloseElement();
                        }
                        catch
                        {
                        ...
                        builder.AddContent(5, localTitle);
                        builder.CloseElement();
                    }
                }
            }
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithLabeledBlockInMixedImperativeRenderSegment_LowersTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
        ...
        StringAssert.Contains(artifact.TemplateText, "<footer>");
        StringAssert.Contains(artifact.TemplateText, "done");
        Assert.AreEqual(HmrBoundaryKind.TemplateOnly, artifact.Identity.HmrBoundaryKind);
    }
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithDeclarativeSiblingsAroundDefaultUsingStatementInBuildRenderTree_LowersTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithDeclarativeSiblingsAroundLockThisStatementInBuildRenderTree_LowersTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithDeclarativeSiblingsAroundReadonlyObjectGateLockStatementInBuildRenderTree_LowersTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_LowersConcatenatedStaticAddMarkupContentCarriers()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_LowersConcatenatedStaticFactoryAddMarkupContentCarrierWithCapturedArgument()
    {
        var context = CreateContext(
            """
            using System;
            ...
            namespace Demo.Components
            {
                ...
                public class MarkupCard : ComponentBase, IVueComponent
                {
                    ...
                    protected override void BuildRenderTree(RenderTreeBuilder builder)
                    {
                        const string close = "</section>";
                        builder.AddMarkupContent(0, CreateOpen(Title) + "<span>safe</span>" + close);
                    }

                    private string CreateOpen(string? ignored)
                        => "<section class=\"hero\">";
                }
            }
            """);
        ...
        Assert.AreEqual(VueSfcArtifactRenderMode.Template, artifact.RenderMode);
        StringAssert.Contains(artifact.TemplateText, "<template v-for=\"(ignored) in [props.title]\">");
        StringAssert.Contains(artifact.TemplateText, "<section class=\"hero\">");
        StringAssert.Contains(artifact.TemplateText, "<span>");
        StringAssert.Contains(artifact.TemplateText, "safe");
    }
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_LowersConditionalStaticAddMarkupContent_ToTemplateConditional()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithConditionalUnsafeAddMarkupContentBranch_ThrowsCanonicalizationFailed()
    {
        var context = CreateContext(
            """
            using System;
                    ...
                    protected override void BuildRenderTree(RenderTreeBuilder builder)
                    {
                        builder.AddMarkupContent(
                            ...
                            Featured
                                ...
                                : "<section onclick=\"alert('x')\">unsafe</section>");
                    }
                }
            }
            """);
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithDirectiveMarkupStringAddContent_ThrowsCanonicalizationFailed()
    {
        var exception = AssertBuildRenderTreeSfcArtifactThrows(
            "DirectiveMarkupStringCard",
            """
            builder.AddContent(0, (MarkupString)"<section v-html=\"payload\">safe</section>");
            """);
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithSrcdocMarkupStringAddContent_ThrowsCanonicalizationFailed()
    {
        var exception = AssertBuildRenderTreeSfcArtifactThrows(
            "SrcdocMarkupStringCard",
            """
            builder.AddContent(0, (MarkupString)"<section srcdoc=\"<p>unsafe</p>\">safe</section>");
            """);
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithFormActionMarkupStringAddContent_ThrowsCanonicalizationFailed()
    {
        var exception = AssertBuildRenderTreeSfcArtifactThrows(
            "FormActionMarkupStringCard",
            """
            builder.AddContent(0, (MarkupString)"<button formaction=\"/submit\">safe</button>");
            """);
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithExecutableDataUriMarkupStringAddContent_ThrowsCanonicalizationFailed()
    {
        var exception = AssertBuildRenderTreeSfcArtifactThrows(
            "ExecutableDataUriMarkupStringCard",
            """
            builder.AddContent(0, (MarkupString)"<a href=\"data:image/svg+xml,%3Csvg%20onload=alert(1)%3E\">safe</a>");
            """);
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithImmediatelyAssignedMarkupStringLocalCarrierThenReassigned_ThrowsCanonicalizationFailed()
    {
        var exception = AssertBuildRenderTreeSfcArtifactThrows(
            "ReassignedMarkupStringCard",
            """
            MarkupString markup;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithDeclarativeSiblingsAroundDoWhileFalseInBuildRenderTree_LowersTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_LowersGetterReturningSourceStableRenderFragmentLocalCarrier_IntoTemplateScopeWrapper()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_LowersGetterReturningSourceStableRenderFragmentLocalChain_ToTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
            ...
            namespace Demo.Components
            {
                ...
                public class Host : ComponentBase, IVueComponent
                {
                    private RenderFragment<int> Template
                    {
                        get
                        {
                            RenderFragment<int> core = item => itemBuilder =>
                            {
                                itemBuilder.OpenElement(1, "span");
                                itemBuilder.AddContent(2, item);
                                itemBuilder.CloseElement();
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithGetterSideEffectRenderFragmentPropertyCarrier_ThrowsCanonicalizationFailed()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithGetterUnusedRenderFragmentLocalCarrier_ThrowsCanonicalizationFailed()
    {
        var context = CreateContext(
            """
            using System;
            ...
            namespace Demo.Components
            {
                ...
                public class Host : ComponentBase, IVueComponent
                {
                    private RenderFragment<int> Template
                    {
                        get
                        {
                            RenderFragment<int> ignored = item => itemBuilder =>
                            {
                                itemBuilder.OpenElement(1, "em");
                                itemBuilder.AddContent(2, item);
                                itemBuilder.CloseElement();
                            };
                            ...
                            RenderFragment<int> template = item => itemBuilder =>
                            {
                                itemBuilder.OpenElement(3, "span");
                                itemBuilder.AddContent(4, item);
                                itemBuilder.CloseElement();
                            };
                            return template;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithGetterReturningDelegateInvocation_ThrowsCanonicalizationFailed()
    {
        var context = CreateContext(
            """
            using System;
            ...
            namespace Demo.Components
            {
                ...
                public class Host : ComponentBase, IVueComponent
                {
                    private RenderFragment<int> Template
                    {
                        get
                        {
                            Func<RenderFragment<int>> create = () => item => itemBuilder =>
                            {
                                itemBuilder.OpenElement(1, "span");
                                itemBuilder.AddContent(2, item);
                                itemBuilder.CloseElement();
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_LowersCurrentComponentRenderFragmentFactoryBlockBodyReturningSourceStableLocalChain_ToTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
        ...
        StringAssert.Contains(artifact.TemplateText, "{{ title }}");
        StringAssert.Contains(artifact.TemplateText, "{{ item }}");
        Assert.IsFalse(artifact.SfcText.Contains("CreateTemplate", StringComparison.Ordinal), artifact.SfcText);
    }

    [TestMethod]
    public void RazorVue_SfcArtifactFactory_LowersLocalRenderFragmentFactoryBlockBodyReturningSourceStableLocalChain_ToTemplateSfc()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithRenderFragmentFactoryViaDelegateDataflow_ThrowsCanonicalizationFailed()
    {
        var context = CreateContext(
            """
            using System;
            ...
            namespace Demo.Components
            {
                ...
                public class Host : ComponentBase, IVueComponent
                {
                    ...
                    protected override void BuildRenderTree(RenderTreeBuilder builder)
                    {
                        Func<RenderFragment<int>> create = CreateTemplate;
                        builder.AddContent(0, create(), 42);
                    }
                }
            }
            """);
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithCurrentComponentRenderFragmentFactoryBlockBodyReturningDelegateInvocation_ThrowsCanonicalizationFailed()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithRenderFragmentFactoryTemplateBodyReturnedByCallable_ThrowsCanonicalizationFailed()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_LowersLocalRenderFragmentFactoryMethodForwarding_IntoTemplateScopeWrapper()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_WithRecursiveLocalRenderFragmentFactoryMethodForwarding_ThrowsCanonicalizationFailed()
    {
        var context = CreateContext(
            """
            using System;
    ...
    [TestMethod]
    public void RazorVue_SfcArtifactFactory_LowersMultiHopCurrentComponentRenderFragmentFactoryForwarding_PreservingCapturedScopeOrder()
    {
        var context = CreateContext(
            """
            using System;
            ...
            namespace Demo.Components
            {
                ...
                public class Host : ComponentBase, IVueComponent
                {
                    ...
                    private RenderFragment<int> CreateCore(string? label, string? suffix)
                        ...
                        {
                            itemBuilder.OpenElement(1, "span");
                            itemBuilder.AddContent(2, label);
                            itemBuilder.AddContent(3, suffix);
                            itemBuilder.AddContent(4, item);
                            itemBuilder.CloseElement();
                        };
                    ...
                    protected override void BuildRenderTree(RenderTreeBuilder builder)
                    {
                        builder.AddContent(0, CreateOuter(oSecond: Subtitle, oFirst: Title), 42);
                    }
                }
            }
            """);
        ...
        var oSecondIndex = artifact.TemplateText.IndexOf("<template v-for=\"(oSecond) in [props.subtitle]\">", StringComparison.Ordinal);
        var oFirstIndex = artifact.TemplateText.IndexOf("<template v-for=\"(oFirst) in [props.title]\">", StringComparison.Ordinal);
        var mSecondIndex = artifact.TemplateText.IndexOf("<template v-for=\"(mSecond) in [oSecond]\">", StringComparison.Ordinal);
        var mFirstIndex = artifact.TemplateText.IndexOf("<template v-for=\"(mFirst) in [oFirst]\">", StringComparison.Ordinal);
        var labelIndex = artifact.TemplateText.IndexOf("<template v-for=\"(label) in [mFirst]\">", StringComparison.Ordinal);
        var suffixIndex = artifact.TemplateText.IndexOf("<template v-for=\"(suffix) in [mSecond]\">", StringComparison.Ordinal);
        var itemIndex = artifact.TemplateText.IndexOf("<template v-for=\"(item) in [42]\">", StringComparison.Ordinal);

        Assert.IsTrue(oSecondIndex >= 0, artifact.TemplateText);
        Assert.IsTrue(oFirstIndex > oSecondIndex, artifact.TemplateText);
        Assert.IsTrue(mSecondIndex > oFirstIndex, artifact.TemplateText);
        Assert.IsTrue(mFirstIndex > mSecondIndex, artifact.TemplateText);
        Assert.IsTrue(labelIndex > mFirstIndex, artifact.TemplateText);
        Assert.IsTrue(suffixIndex > labelIndex, artifact.TemplateText);
        Assert.IsTrue(itemIndex > suffixIndex, artifact.TemplateText);
        StringAssert.Contains(artifact.TemplateText, "{{ label }}");
        StringAssert.Contains(artifact.TemplateText, "{{ suffix }}");
        StringAssert.Contains(artifact.TemplateText, "{{ item }}");
    }
    ...
    {
        var context = CreateContext(CreateBuildRenderTreeComponentSource(componentName, renderStatements));
        var snapshot = context.CreateSemanticSnapshots().Single(item => item.Descriptor.Name == componentName);
        return Assert.ThrowsExactly<RazorVueCompilationIssueException>(
            () => CreateBuildRenderTreeArtifactFactory().Lower(context, snapshot));
    }

    private static string CreateBuildRenderTreeComponentSource(
        string componentName,
        string renderStatements)
        => $$"""
        using System;
        ...
        namespace Demo.Components
        {
            [ECMAScript.ECMAScriptModule("./components/raw-markup-boundary")]
            public class {{componentName}} : ComponentBase, IVueComponent
            {
                protected override void BuildRenderTree(RenderTreeBuilder builder)
                {
        {{renderStatements}}
