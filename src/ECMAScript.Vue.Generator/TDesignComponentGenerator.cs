...
internal static class TDesignComponentGenerator
{
        ...
        IReadOnlyList<TypeParameter> rootTypeParameters)
    {
        ...
        private static bool TryMapPlatformType(string name, out MappedType type)
        {
            ...
            {
                "bigint" => new MappedType("BigInt", IsReference: false),
                // Browser File is exposed as ECMAScript.FileRef to avoid colliding with
                // System.IO.File in consumer projects; its JavaScript ABI remains File.
                // 浏览器 File 在作者侧使用 ECMAScript.FileRef，避免与 consumer 项目的
                // System.IO.File 冲突；JavaScript ABI 仍然是 File。
                "File" => new MappedType("FileRef", IsReference: true),
                "Document" => new MappedType("DocumentRef", IsReference: true),
                "Window" => new MappedType("WindowRef", IsReference: true),
                "Event" => new MappedType("EventRef", IsReference: true),
                "History" => new MappedType("HistoryRef", IsReference: true),
                "Location" => new MappedType("LocationRef", IsReference: true),
