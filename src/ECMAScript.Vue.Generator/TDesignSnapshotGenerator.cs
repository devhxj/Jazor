...
using HttpClient = global::System.Net.Http.HttpClient;
using HttpCompletionOption = global::System.Net.Http.HttpCompletionOption;
using HttpVersion = global::System.Net.HttpVersion;
using HttpVersionPolicy = global::System.Net.Http.HttpVersionPolicy;
using MemoryStream = global::System.IO.MemoryStream;
using SocketsHttpHandler = global::System.Net.Http.SocketsHttpHandler;

internal static class TDesignSnapshotGenerator
{
    private const string PackageName = "tdesign-vue-next";
    ...
    new ExternalTypePackage("tdesign-icons-vue-next", "0.4.7", "tdesign-icons-vue-next")
        ...
        var snapshotRoot = options.SnapshotDirectory is null
            ? Path.Combine(repoRoot, "src", "ECMAScript.Vue.Generator", "upstream", PackageName, options.Version)
        ...
        IReadOnlyList<ExternalTypePackage> externalTypes)
    {
                ...
                !Directory.Exists(Path.Combine(snapshotRoot, "external", externalType.ModuleName))))
        {
            throw new InvalidOperationException(
                $"TDesign declaration snapshot does not match {PackageName}@{version}. Run `dotnet run --project src/ECMAScript.Vue.Generator -- tdesign snapshot`.");
        }
    }
    ...
    sealed record ExternalTypePackage(string PackageName, string Version, string ModuleName);

    sealed record GeneratorOptions(string Version, string? SnapshotDirectory, bool Check, bool Refresh)
    {
