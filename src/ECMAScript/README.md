...
本项目是 JS resource library，不是纯 Jazor library。它发布时由 `manifest.json` 声明
`src/ECMAScript/clr/**` embedded carrier，携带已有的 ECMAScript/System JavaScript；C# 程序集
只提供映射和 authoring contract，不生成或替代 `Jazor.Generated.ModuleCatalog`。`ModuleCatalog` 只属于开发者
