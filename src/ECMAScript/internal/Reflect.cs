...
/// <see cref="object"/> or <see cref="PropertyKey"/> shapes. Do not reduce these to string-only or catch-all APIs for superficial C# convenience.
/// <c>Reflect</c> 保留 JavaScript 的属性键、receiver 和返回值规则，因此部分参数必须保留
/// <see cref="object"/> 或 <see cref="PropertyKey"/> 形状；不要为表面简洁而改成仅 string 或 catch-all API。
/// </remarks>
