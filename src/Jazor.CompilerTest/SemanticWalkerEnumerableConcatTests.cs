...
[TestClass]
public sealed class SemanticWalkerEnumerableConcatTests
{
    [TestMethod]
    public void Visit_EnumerableConcat_UsesBoundImportAndPreservesSourceOrder()
    {
        var block = GetBlockOperation(
            """
            using System.Linq;

            public static class EnumerableConcatScenarios
            {
                public static int[] Evaluate(int[] firstReleaseIds, int[] secondReleaseIds)
                {
        ...
        CollectionAssert.AreEqual(new[] { "concat" }, importNames);
        StringAssert.Contains(body, "concat(firstReleaseIds, secondReleaseIds)", StringComparison.Ordinal);
