...
[TestClass]
public sealed class JazorVueHostFallbackTelemetryTests
{
    [TestMethod]
    public async Task NullVueAnalysisClient_AnalyzeJazorAsync_ReportsFallbackOncePerDocumentPath()
    {
        ...
        try
        {
            var client = new NullVueAnalysisClient();
    ...
    [TestMethod]
    public void VueAnalysisClientFactory_CreateDefault_ReportsFallbackOnce()
    {
        var events = new List<string>();
        FallbackTelemetry.ResetForTests();
        FallbackTelemetry.SetTestSinkForTests(events.Add);
