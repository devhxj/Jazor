...
[TestClass]
public sealed class SemanticWalkerEnumerableMinMaxTests
{
    [TestMethod]
    public void Visit_EnumerableMinMax_UsesExactNumericBoundImports()
    {
        var block = GetBlockOperation(
            """
            using System.Linq;

            public static class EnumerableMinMaxScenarios
            {
        ...
        CollectionAssert.AreEquivalent(
            new[] { "minInt", "maxInt", "minInt64", "maxInt64", "minSingle", "maxSingle", "minDouble", "maxDouble", "minDecimal", "maxDecimal" },
            importNames);
        StringAssert.Contains(body, "minInt(integers)", StringComparison.Ordinal);
        StringAssert.Contains(body, "maxInt64(int64s)", StringComparison.Ordinal);
        StringAssert.Contains(body, "minSingle(singles)", StringComparison.Ordinal);
        StringAssert.Contains(body, "maxDouble(doubles)", StringComparison.Ordinal);
        StringAssert.Contains(body, "minDecimal(decimals)", StringComparison.Ordinal);
