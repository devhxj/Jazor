...
[TestClass]
public sealed class SemanticWalkerEnumerableSkipTakeLastTests
{
    [TestMethod]
    public void Visit_EnumerableSkipTakeLast_UsesBoundImportsAndMaterializedPipeline()
    {
        var block = GetBlockOperation(
            """
            using System.Linq;

            public static class EnumerableSkipTakeLastScenarios
            {
                public static int[] Evaluate(int[] releaseIds, int count)
                {
                    return releaseIds.SkipLast(count).TakeLast(count).ToArray();
                }
            }
            """);
        ...
        CollectionAssert.AreEquivalent(
            new[]
            {
                "static System.Linq.Enumerable.SkipLast<TSource>(System.Collections.Generic.IEnumerable<TSource>, int)",
                "static System.Linq.Enumerable.TakeLast<TSource>(System.Collections.Generic.IEnumerable<TSource>, int)",
        ...
        CollectionAssert.AreEquivalent(new[] { "skipLast", "takeLast" }, importNames);
        StringAssert.Contains(body, "skipLast(releaseIds, count)", StringComparison.Ordinal);
        StringAssert.Contains(body, "takeLast(", StringComparison.Ordinal);
    ...
    private static IBlockOperation GetBlockOperation(string source)
    {
        ...
        var compilation = CSharpCompilation.Create(
            "EnumerableSkipTakeLastScenarios",
            [syntaxTree],
