...
| compiler -> catalog -> emit | import/source-map catalog、路径冲突、manifest、外部 Razor consumer 的 DOM `@bind:set` 与 descriptor 子组件 `@bind-Value:get/@bind-Value:set` | `ESGeneratorSourceMapCatalogTest.cs`、`ESGeneratorModulePathCollisionTests.cs`、`SdkIntegrationTests.cs` | catalog carrier、source map、materialized relative import、model update 经 setter 回写与 DenoHost artifact |
