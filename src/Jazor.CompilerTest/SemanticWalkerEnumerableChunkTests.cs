...
[TestClass]
public sealed class SemanticWalkerEnumerableChunkTests
{
    [TestMethod]
    public void Visit_EnumerableChunk_UsesBoundImportAndNestedArrayMaterialization()
    {
        var block = GetBlockOperation(
            """
            using System.Linq;

            public static class EnumerableChunkScenarios
            {
                public static int[][] Evaluate(int[] releaseIds, int size)
                {
        ...
        CollectionAssert.AreEquivalent(
            new[]
            {
                "static System.Linq.Enumerable.Chunk<TSource>(System.Collections.Generic.IEnumerable<TSource>, int)",
        ...
        CollectionAssert.AreEqual(new[] { "chunk" }, importNames);
        StringAssert.Contains(body, "chunk(releaseIds, size)", StringComparison.Ordinal);
