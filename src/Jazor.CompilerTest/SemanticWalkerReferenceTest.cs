...
public sealed class SemanticWalkerReferenceTest
{
  ...
  let appended = (sb.push('\n'), sb);
	...
	[TestMethod]
	public void Visit_Reference_SourceDataCarrierMemberChainWithoutOptIn_Throws()
	{
		var block = GetBlockOperation(@"
            ...
            class TestClass
            {
                void TestMethod(bool firstRender)
                {
                    ...
                    var ready = readyEnvelopes[1].State.Value;
                }
            }
        ");
	...
	[TestMethod]
	public void Visit_Reference_SourceDataCarrierMemberChainWithOptIn_LowersStructurally()
	{
		var block = GetBlockOperation(@"
            using System.Collections.Generic;
	...
	AssertScriptEqual(@"{
  let readyEnvelopes = [{ state: { value: false } }, { state: { value: firstRender } }];
  let ready = _d389c31d59037b42(readyEnvelopes, 1).state.value;
}", script);
	}

	[TestMethod]
	public void Visit_Reference_TupleStructuralMemberChainWithOptIn_LowersStructurally()
	{
		var block = GetBlockOperation(@"
            public readonly struct ReadyState
            {
            ...
            class TestClass
            {
                void TestMethod(bool firstRender)
                {
                    var ready = (firstRender, new ReadyState(firstRender)).Item2.Value;
                }
            }
        ");
		...
		AssertScriptEqual(@"{
  let ready = { firstRender: firstRender, item2: { value: firstRender } }.item2.value;
}", script);
	}
  ...
  let name = person.name;
	...
	public void Visit_ECMAScriptRecordProxyIndexerAccess_AllowsJavaScriptComputedAccess()
	{
		var block = GetBlockOperation(@"
            ...
            public class TestClass
            {
                void TestMethod(ListenerBag listeners)
                {
                    var handler = listeners[""on:update""];
                }
            }
        ");
		...
		AssertScriptEqual(@"{
  let handler = listeners[""on:update""];
}", script);
	}
