...
public partial class ConfigPage : AppComponentBase, IVueContainerComponent
{
    ...
    private void Load()
    {
        ...
        if (View == ConfigView.Clients)
            JazorAdminApiClient.GetClients().Then(ApplyClients);
        else
            JazorAdminApiClient.GetScopes().Then(ApplyScopes);
    }

    private void ApplyClients(AdminApiOutcome outcome)
    {
        loading = false;
        if (!outcome.Ok)
        {
        ...
        clients = JazorAdminApiClient.ToClients(outcome.Data);
    }

    private void ApplyScopes(AdminApiOutcome outcome)
    {
        loading = false;
        if (!outcome.Ok)
        {
        ...
        scopes = JazorAdminApiClient.ToScopes(outcome.Data);
    }
    ...
    private void CreateClient()
    {
        ...
        JazorAdminApiClient.CreateClient(
            clientId,
            clientDisplayName,
            [redirectUri],
            [postLogoutUri],
    ...
    private void CreateScope()
    {
        ...
        JazorAdminApiClient.CreateScope(scopeName, scopeDisplayName).Then(outcome =>
        {
            if (!outcome.Ok)
            {
