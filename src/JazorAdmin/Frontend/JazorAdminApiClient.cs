// Owns same-origin HTTP transport through browser-standard WebIDL bindings.
// 使用浏览器标准 WebIDL 绑定承载同源 HTTP 传输，业务代码不依赖手写 JavaScript bridge。
using ECMAScript;
...
[ECMAScriptModule("components/jazor-admin-api-client.mjs")]
public static class JazorAdminApiClient
{
    public static IPromise<AdminApiOutcome> GetSession()
        => Get("/api/auth/session");

    public static IPromise<AdminApiOutcome> SignIn(string email, string password)
        => Send("/api/auth/login", "POST", new LoginRequest(email, password));

    public static IPromise<AdminApiOutcome> SignOut()
        => Send("/api/auth/logout", "POST");

    public static IPromise<AdminApiOutcome> GetOrganizations()
        => Get("/api/organizations/");

    public static IPromise<AdminApiOutcome> GetOrganization(string organizationId)
        => Get("/api/organizations/" + organizationId);

    public static IPromise<AdminApiOutcome> GetMembers(string organizationId)
        => Get("/api/organizations/" + organizationId + "/members");

    public static IPromise<AdminApiOutcome> GetRoles(string organizationId)
        => Get("/api/organizations/" + organizationId + "/roles");

    public static IPromise<AdminApiOutcome> GetRoleGrants(string organizationId, string roleId)
        => Get("/api/organizations/" + organizationId + "/roles/" + roleId + "/grants");

    public static IPromise<AdminApiOutcome> GetAuthorizationResources(string organizationId)
        => Get("/api/organizations/" + organizationId + "/authorization-resources");

    public static IPromise<AdminApiOutcome> CreateOrganization(string code, string displayName)
        => Send("/api/organizations/", "POST", new CreateOrganizationRequest(code, displayName));

    public static IPromise<AdminApiOutcome> CreateChildOrganization(
    ...
    public static IPromise<AdminApiOutcome> CreateRole(
        string organizationId,
        string code,
        string displayName)
        => Send(
    ...
    public static IPromise<AdminApiOutcome> ReplaceRoleGrants(
        string organizationId,
        string roleId,
        string[] grantKeys)
    {
    ...
    public static IPromise<AdminApiOutcome> CreateMember(
        string organizationId,
        string email,
        string[] roleIds)
        => Send(
    ...
    public static IPromise<AdminApiOutcome> ReplaceMemberRoles(
        string organizationId,
        string membershipId,
        string[] roleIds)
        => Send(
    ...
    public static IPromise<AdminApiOutcome> GetAccounts()
        => Get("/api/accounts/");

    public static IPromise<AdminApiOutcome> CreateAccount(
        string email,
        string displayName,
        string password,
    ...
    public static IPromise<AdminApiOutcome> SetAccountEnabled(string userId, bool enabled)
        => Send("/api/accounts/" + userId + "/enabled", "PUT", new UpdateAccountStateRequest(enabled));

    public static IPromise<AdminApiOutcome> ResetAccountPassword(string userId, string password)
        => Send("/api/accounts/" + userId + "/password", "PUT", new ResetAccountPasswordRequest(password));

    public static IPromise<AdminApiOutcome> GetClients()
        => Get("/api/configuration/clients");

    public static IPromise<AdminApiOutcome> CreateClient(
        string clientId,
        string displayName,
        string[] redirectUris,
    ...
    public static IPromise<AdminApiOutcome> GetScopes()
        => Get("/api/configuration/scopes");

    public static IPromise<AdminApiOutcome> CreateScope(string name, string displayName)
    ...
    private static IPromise<AdminApiOutcome> Get(string path)
        => Send(path, "GET");

    private static IPromise<AdminApiOutcome> Send(string path, string method, object? body = null)
    {
    ...
    private static IPromise<AdminApiOutcome> ReadResponse(Response response)
    {
        ...
        {
            return Promise<AdminApiOutcome>.Resolve(new AdminApiOutcome
            {
                Ok = response.Ok,
        ...
        return Promise<object>.Resolve(response.Json()).Then(data => new AdminApiOutcome
        {
            Ok = response.Ok,
            Data = data,
