...
/// <summary>
/// File-backed manifest, inventory, and upstream drift checks for the vendored WangEditor runtime.
...
{
    [TestMethod]
    public void WangEditor_Manifest_DeclaresLockedClosureWithVerifiableHashes()
    {
        ...
        Assert.AreEqual(0, imports.GetProperty("@wangeditor/editor").GetProperty("productionDependencies").EnumerateArray().Count());
        CollectionAssert.AreEquivalent(
            new[] { "@wangeditor/editor" },
            imports.GetProperty("@wangeditor/editor-for-vue").GetProperty("productionDependencies").EnumerateArray().Select(static value => value.GetString()!).ToArray());
        ...
        CollectionAssert.AreEquivalent(
            new[] { "dist/@wangeditor/editor/style.css" },
    ...
    [TestMethod]
    public void WangEditor_VendoredDist_ExactlyMatchesTheManifestClosure()
    {
        using var manifest = JsonDocument.Parse(File.ReadAllText(GetProjectPath("manifest.json")));
    ...
    [TestMethod]
    public void WangEditor_VendoredModules_AreBrowserSafe()
    {
    ...
    [TestMethod]
    public void WangEditor_BoundExports_ExistInTheUpstreamEntry()
    {
        // 宿主函数条目（@# 描述）与组件代理（Transform.Component 的 ExportName）都必须在上游入口存在；
        // 默认导出的组件代理改用 default 导出断言。
        var upstream = ReadUpstreamEntryExports();
        var hostNames = ReadBoundExportNames(typeof(WangEditor));
        var componentExports = ReadComponentExports(typeof(WangEditor).Assembly);
    ...
    [TestMethod]
    public void WangEditor_Inventory_RecordsPackageVersionsAndFingerprint()
    {
        var inventory = GetInventory();
        var packages = inventory.GetProperty("packages");
        Assert.AreEqual(inventory.GetProperty("version").GetString(), packages.GetProperty("@wangeditor/editor-for-vue").GetString());
        Assert.IsTrue(packages.TryGetProperty("@wangeditor/editor", out var core));
        Assert.AreEqual("5.1.23", core.GetString());
    ...
    private static IEnumerable<string> Entries => new[] { GetProjectPath("dist", "@wangeditor", "editor", "index.esm.js"), GetProjectPath("dist", "@wangeditor", "editor-for-vue", "index.esm.js") };
