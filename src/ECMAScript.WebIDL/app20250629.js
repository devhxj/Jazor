...
export function collect(source) {
    try {
        ...
        const res = { source: source, namespace: namespace, using: '', code: '' };
        res.code = `namespace ${res.namespace ? `Jazor.ECMAScript.${res.namespace}` : 'Jazor.ECMAScript'};\n`;
        const toSharpType = function (idlType, using = false, defval = 'object') {
        ...
        const generateTypedef = function (item) {
            const name = toPascalCase(item.name);
            ...
            if (!exists)
                usings.set(name, value);
            return `global using ${name} = ${value};${mark}`;
        };
        const generateInterface = function (item) {
        ...
        const generateDictionary = function (item) {
                ...
                case 'interface':
                    {
                        ...
                        if (classes.has(item.name)) {
                            classes.get(item.name)?.push(item);
                        }
                        else {
                            classes.set(name, [item]);
                        }
                    }
                    break;
...
//# sourceMappingURL=app20250629.js.map