...
const cssPropertyName = function (name) {
    const camel = name
        .replace(/^-(\w)/, (_, c) => c)
        .replace(/-(\w)/g, (_, c) => c.toUpperCase());
    return camel === "float" ? "_float" : camel;
};
const generateCssIdl = function (properties) {
    return `partial interface CSSStyleDeclaration {${properties
        .map((property) => `\n  [CEReactions] attribute [LegacyNullToEmptyString] CSSOMString ${cssPropertyName(property)};`)
        .join("")}\n};`;
};
...
function normalizeDeclaration(root) {
...
async function collectInterfaceEvents() {
    ...
    return [...eventMap.entries()]
        ...
        const normalizedEvents = [...events.entries()]
            .sort(([left], [right]) => left.localeCompare(right))
...
async function collectInventory() {
    ...
    for (const [fileName, source] of [...files.entries()].sort(([left], [right]) => left.localeCompare(right))) {
        const ast = webidl2.parse(source).sort((left, right) => (declarationPriority[left.type] ?? 999) -
    ...
    return {
        ...
        stats: {
            ...
            declarationsByKind: Object.fromEntries([...declarationsByKind.entries()].sort(([left], [right]) => left.localeCompare(right))),
        },
    };
}
