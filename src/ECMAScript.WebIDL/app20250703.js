...
const recordCaches = new Set();
const classCaches = new Map();
const classInhers = [];
const classIndexs = [];
const iterableClasses = [];
const maplikeClasses = [];
const setlikeClasses = [];
...
const toCamelCase = function (input) {
    if (!input)
        return ''; // �������ַ���
    return input
        // �滻���з���ĸ�����ַ�Ϊ�ո񣬲�����������д��ĸ�ı߽磨�� "NameOrIndex"��
        .replace(/([a-z])([A-Z])/g, '$1 $2') // Сд��Ӵ�д �� ����ո�
        .replace(/[^a-zA-Z0-9]+/g, ' ') // ��������ĸ�����ַ� �� �ո�
        // �ָ�ʲ����˿�ֵ
        .split(' ')
        .filter(word => word.length > 0)
        // ����ÿ�����ʣ�����ĸ��д����һ����������ĸСд��
        .map((word, index) => index === 0
        ? word[0].toLowerCase() + word.slice(1).toLowerCase() // ��һ����������ĸСд
        : word[0].toUpperCase() + word.slice(1).toLowerCase() // ������������ĸ��д
    )
        // ƴ�ӳ������ַ���
        .join('');
};
        ...
        case "":
            {
                if (idlType.union) {
                    ...
                    if (types.length == 1 && hasUndefined > 0) {
                        ...
                        if (using)
                            sharpType = `Jazor.ECMAScript.Either<${types.join(', ')}>`;
                        else
...
const getValue = function (val, namespace = undefined) {
    if (!val)
        return 'null';
    switch (val.type) {
        case 'string':
...
const generateCallbackInterface = function (item, namespace = undefined) {
    ...
    const code = item.members.filter(x => x.type == "operation").map(member => {
...
const generateMethod = function (item, namespace = undefined) {
    ...
    const returnType = item.idlType ? toSharpType(item.idlType, namespace, false, 'void') : 'void';
    if (item.name == 'setBid' && item.parent.name == "InterestGroupBiddingScriptRunnerGlobalScope") {
        let a = 9;
    }
    let params = [], comments = [];
    ...
    if (item.parent.inheritance) {
        ...
        const item1 = item.special + ':' + toPascalCase(item.name || "");
        ...
        const cache = classCaches.get(parentName);
        const a = cache.methods.filter(x => x.item1 == item1 && x.item2 == item2);
    ...
    if (item.name)
        ...
        if (item.special == 'stringifier')
            ...
            throw new Error("C# not support deleter.");
    }
    if (name == 'Item')
        name = 'GetItem'; //���ⷽ������������ͻ
    if (!name)
        throw new Error("operation must have name.");
    let code = `${P}/// <summary>${N}` +
        `${P}/// ${item.name}${N}` +
        `${P}/// </summary>${N}` +
    ...
    if (item.arguments.length > 0) {
        ...
        if (lastArg.idlType.union) {
            ...
            code += lastArg.idlType.idlType.map(type => {
...
const generateConst = function (item, namespace = undefined) {
...
const generateIterable = function (item, namespace = undefined) {
...
const generateMaplike = function (item, namespace = undefined) {
...
const generateSetlike = function (item, namespace = undefined) {
...
const targetDir = `${thisDir}/Jazor/ECMAScript/Generate`;
...
fs.writeFileSync(`${targetDir}/GlobalUsings.cs`, commonUsings
...
//# sourceMappingURL=app20250703.js.map