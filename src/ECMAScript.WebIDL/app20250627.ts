...
const keywords: Set<string> = new Set<string>([
    'event', 'namespace', 'default', 'this', 'params','base',
...
const defs: Map<string, string> = new Map<string, string>();
...
// 主转换函数
export function convert(idlContent: string): ConvertResult {
    try {
        const priority = {
            'typedef': 0,
            'namespace': 1,
        ...
        const generateType = function (idlType: webidl2.IDLTypeDescription, using: boolean = false): string {
        ...
        const generateValue = function (val: webidl2.ValueDescription): string {
            switch (val.type) {
                case 'string':
                    return `"${val.value}"`;
                case 'number':
        ...
        const generateEnum = function (enumDef: webidl2.EnumType): string {
...
`;
    ...
    ${toPascalCase(enumValue || 'Empty')} = ${index},`;
        ...
        const generateTypedef = function (typedef: webidl2.TypedefType): string {
            if (typedef.name === "AlgorithmIdentifier") {
                let a = 123;
            }
            ...
            const value = exists ? defs.get(type) || type : type;
        ...
        const generateInterface = function (interfaceDef: webidl2.InterfaceType): string {
            if (interfaceDef.name === "TimeEvent") {
                let a = res.source;
            }

            let name = toPascalCase(interfaceDef.name);
            const exists = classes.has(name);
            if (!exists) classes.add(name);
...
{`;
        ...
        const generateDictionary = function (dictDef: webidl2.DictionaryType): string {
            const name = toPascalCase(dictDef.name);
            const exists = records.has(name);
            if (!exists)
                records.add(name);

            if (dictDef.name === "GPUCopyExternalImageSourceInfo") {
                let a = res.source;
            }
...
{${exists ? '' : `\n    public extern ${name}();`}`;
            ...
            for (const field of dictDef.members) {
                ...
                if (field.default) {
                    const isEnumType = enums.has(type);
                    let value = generateValue(field.default);
                    if (isEnumType)
                        value = `${type}.${value == '""' ? 'Empty' : toPascalCase(value)}`;
        ...
        const generateProperty = function (attribute: webidl2.AttributeMemberType): string {
            ...
            const propName = toPascalCase(attribute.name);
