...
public static class ValueTupleModule
{
    ...
    {
        private readonly Set<string> _names;

        public Tuple(Array<object> values)
        {
            this.Super();
            _names = [];
            ...
            for (uint index = 0; index < values.Length; index++)
            {
                ...
                object? value;
                if (values[index] is Array<object> array)
                {
                    if (array.Length == 2 && array[0] is string str)
                    {
                        name = str;
                        value = array[1];
                    }
                ...
                if (_names.Has(name))
                    throw new ArgumentException("存在重复的键值");

                _names.Add(name);
                Push(value);
                Object.DefineProperty(this, name, new()
                {
                    Value = this[index],
                    Writable = false,
                    Enumerable = true,
            ...
            // 只冻结自己（嵌套如果是 Tuple 已经冻结）
            Object.Freeze(this);
        }

        public Tuple With(Map<string, object?> updates)
        {
            Array<object> meta = [];
            _names.ForEach((name, index, set) =>
            {
                var value = updates.Has(name) ? updates.Get(name) : this.GetPropertyValue(name);
                Array<object?> item = [name, value];
                meta.PushItem(item);
            });
            return new Tuple(meta);
        }
        ...
        public static Tuple Create(Array<object> values) => new(values);
    }
