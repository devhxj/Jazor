...
public sealed partial class JazorVueParser
{
    ...
    public static JazorVueDocument Parse(string filePath, string sourceText)
    {
        if (filePath is null)
        {
        ...
        string template)
    {
        ...
        var inferredComponentNames = new HashSet<string>(
            inferredVueImports
    ...
    private static TemplateParseResult ParseTemplate(string sourceText, CodeParseResult codeParseResult)
    {
        ...
        var markupEnd = codeParseResult.DirectiveIndex >= 0
            ...
            : sourceText.Length;
        var markup = sourceText.Substring(0, markupEnd);
    ...
    {
        var relativePath = GetRelativePath(documentDirectory, absolutePath)
            .Replace('\\', '/');
    ...
    {
        var fromUri = new Uri(EnsureTrailingDirectorySeparator(Path.GetFullPath(relativeTo)));
