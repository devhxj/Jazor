...
const double minimumBranchRate = 0.97;
...
try
{
    ...
    {
        "test",
        Path.Combine(repoRoot, "src", "Jazor.RazorVue.Sg.Test", "Jazor.RazorVue.Sg.Test.csproj"),
        "--configuration",
        options.Configuration,
        "--settings",
        Path.Combine(repoRoot, "src", "Jazor.RazorVue.Sg.Test", "coverlet.runsettings"),
        "--collect:XPlat Code Coverage",
        "--logger",
        "trx;LogFileName=razorvue.trx",
        "--results-directory",
        resultRoot,
        "--verbosity",
        "minimal"
    ...
    var trxPath = RequireSingleFile(resultRoot, "razorvue.trx");
    ...
    if (failures.Count > 0)
        throw new InvalidOperationException("RazorVue coverage gate failed: " + string.Join("; ", failures));
}
