...
#:project ../../src/ECMAScript/ECMAScript.csproj
...
var snapshotRoot = Path.Combine(projectRoot, "upstream", "tdesign-vue-next", version);
...
if (!File.Exists(contractsPath) || !File.Exists(bindingsPath))
    throw new InvalidOperationException("Missing TDesign contracts. Run generate-tdesign.cs and generate-tdesign-bindings.cs first.");
...
if (check)
{
    if (!File.Exists(outputPath) || !string.Equals(File.ReadAllText(outputPath), source, StringComparison.Ordinal))
        throw new InvalidOperationException("Generated TDesign component slice is stale. Run scripts/csharp/generate-tdesign-components.cs.");
...
file sealed record Contract(
    string Tag,
    string AuthoringType,
    Property[] Properties,
    ComponentEvent[] Events,
    ComponentSlot[] Slots);
file sealed record Binding(
    string Tag,
    string RuntimeExport,
    string? PropsDeclaration,
    string? PropsSource,
    HashSet<string> Slots);
file sealed record Component(Contract Contract, Binding Binding);
file sealed record Property(string Name, string Type, string SourcePath, bool Optional);
file sealed record ComponentEvent(string Name, string Property, string Type, string SourcePath, bool Optional);
file sealed record ComponentSlot(string Name, string? Property, string Type, string SourcePath);
file sealed record GenerationAttempt(Component Component, GeneratedComponent? Generated, string? Failure);

file sealed record MappedProperty(Property Source, MappedType Type, string CSharpName);
file sealed record MappedSlot(ComponentSlot Source, MappedType Type, string CSharpName);
file sealed record MappedEvent(ComponentEvent Source, MappedType? PayloadType, string CSharpName);
file sealed record GeneratedComponent(
    Component Component,
...
file sealed record MappedType(string Name, bool IsReference, bool IsNullable = false);
file sealed record MappedParameter(string SourceName, string CSharpName, MappedType Type, bool Optional);
file sealed record MappedShapeProperty(string SourceName, string CSharpName, MappedType Type, bool Optional);
file sealed record MappedShape(string[] TypeParameters, MappedShapeProperty[] Properties);

file static class CSharpIdentifier
{
...
file enum TypeScriptDeclarationKind
{
    Interface,
    TypeAlias,
    Class
...
file sealed record TypeScriptDeclaration(
    string Name,
    string SourcePath,
...
file sealed record TypeParameter(string Name, string? DefaultSource);
file sealed record ImportedType(string ExportName, string[] Targets);
