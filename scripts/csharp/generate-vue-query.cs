...
// Validate the locked TanStack Vue Query ESM entries and regenerate package metadata and upstream
// inventory. Runtime modules remain in the restored npm packages.
// 上游以 npm registry integrity 锁定版本；`@tanstack/vue-query` 是多模块 ESM 树，引用 query-core、
// match-sorter-utils 与 vue-demi；vue peer 与 Vue devtools API 由 ECMAScript.Vue 资源库提供。
//
// Usage:
//   dotnet run --file scripts/csharp/generate-vue-query.cs -- --version 5.103.1
//   dotnet run --file scripts/csharp/generate-vue-query.cs -- --source .tmp/p3b/node_modules --version 5.103.1
...
const string libraryId = "vue-query";
const string primaryPackage = "@tanstack/vue-query";
const string documentation = "https://tanstack.com/query/latest/docs/framework/vue/overview";
const string licenseId = "MIT";

var projectRoot = Path.Combine(root, "src", "ECMAScript.VueQuery");
...
{
    ("@tanstack/vue-query", "@tanstack/vue-query", version, "build/modern/index.js"),
    ("@tanstack/query-core", "@tanstack/query-core", "5.103.1", "build/modern/index.js"),
    ("@tanstack/match-sorter-utils", "@tanstack/match-sorter-utils", "8.19.4", "build/lib/index.mjs"),
    ("vue-demi", "vue-demi", "0.14.10", "lib/index.mjs"),
};

// 由 ECMAScript.Vue 资源库提供的 peer specifier；不进入本包闭包，只写入 dependencies 与 requires。
var peerSpecifiers = new HashSet<string>(StringComparer.Ordinal) { "vue", "@vue/devtools-api" };

// 绑定只支持 Vue 3；上游 peer 区间为 `^2.6.0 || ^3.3.0`，因此在此显式声明 Vue 3 子集。
var peerRequires = new SortedDictionary<string, string>(StringComparer.Ordinal) { ["vue3"] = "^3.3.0" };
...
Console.WriteLine($"Validated {closures.Values.Sum(static closure => closure.Count)} {libraryId} ESM module(s); license: {licenseId}.");
...
static void CopyExistingPackageVersions(string manifestPath, IDictionary<string, string> packageVersions)
{
    ...
    {
        if (!packageVersions.ContainsKey(property.Key) && property.Value?["version"]?.GetValue<string>() is { } packageVersion)
