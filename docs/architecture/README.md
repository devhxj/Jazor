...
本目录只承接长期有效的参考入口，不负责当前执行导航。你如果是来继续往前推开发，先去看 `docs/status/` 和 `docs/plans/`，莫在这儿找当天的推进安排。
...
- 本目录偏长期有效参考，不承担 active execution 导航；执行入口请看 [docs/plans/README.md](../plans/README.md)
