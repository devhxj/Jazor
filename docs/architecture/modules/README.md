...
本文档负责桥接“模块级 operational docs”，不是替代深度设计文档。

它回答的问题是：

- 如果我按项目模块工作，先看哪份 README
- 哪些模块已有深度局部文档，哪些主要靠 operational README
- 模块 README 与 repo-level 状态/计划文档如何衔接
...
- `Jazor.Emit` 是 materialization / manifest / bundling 入口
- `src/Jazor.Emit/doc/` 是 emit lane 的局部 deep-doc 入口
- `Jazor.EmitTest` 保留 emit-side regression coverage 入口
...
- 新增模块时，优先在模块目录本地补一份薄 README，再决定是否需要 repo-level bridge 扩充
- 若后续再次出现 discoverability gap，优先继续在本文记录，不单独扩成新的 repo-level backlog 文档
...
这类模块不需要为了进入 repo-level 主阅读路径而强行搬迁文档。
