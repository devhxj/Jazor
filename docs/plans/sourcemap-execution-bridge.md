...
> Status: active plan
> Positioning: Repository-level execution bridge for the SourceMap programme and its current narrower active lane.
...
4. 再根据具体任务进入 broad programme 或 narrower active lane

## 当前非目标

- 不把 broader SourceMap programme 误写成 fully active
- 不让 narrower active lane 覆盖 broad programme 规则
- 不在 repo-level bridge 中重复局部专题细节

## 下一步维护要求

如果 SourceMap 的 broad programme 或 narrower active lane 状态发生变化，至少同步更新：
