...
发布候选应先手动运行 `Release Candidate Verification` workflow，或执行 `scripts/csharp/verify-release-candidate.cs`；该入口统一归档 API 兼容性、质量门禁、RazorVue diagnostics、包形状和 SPA/SSR consumer 证据。它通过后才进入正式发布工作流。
