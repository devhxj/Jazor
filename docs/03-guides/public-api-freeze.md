...
> 状态：冻结前审查和机器候选基线已建立，尚未授权发布 `1.0.0`。本文是 1.0 发布前的契约基线；任何新增、删除、重命名或签名改变都必须先更新本文、测试和 CHANGELOG。

## 审查结论

当前 API 可以进入冻结候选阶段。ECMAScript 绑定仍处于冻结前迁移窗口，本轮已完成浏览器/桥接类型的 `*Ref` 命名统一；该破坏性变更必须在 1.0 冻结前完成，公开面清单见[1.0 公共 API 基线清单](./public-api-baseline.md)。机器候选快照已保存在 `public-api-baseline.snapshot.md`，冻结前仍必须在发布 ref 上重新生成并通过兼容性检查，以及全部质量门禁和发布消费者门禁，才能最终放行。
