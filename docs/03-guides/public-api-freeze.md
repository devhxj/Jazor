...
当前 API 可以进入冻结候选阶段。ECMAScript 绑定仍处于冻结前迁移窗口，本轮已完成浏览器/桥接类型的 `*Ref` 命名统一；该破坏性变更必须在 1.0 冻结前完成，公开面清单见[1.0 公共 API 基线清单](./public-api-baseline.md)。冻结前仍必须完成一次针对发布 ref 的机器 API 兼容性检查，并以全部质量门禁和发布消费者门禁的成功结果作为最终放行条件。
