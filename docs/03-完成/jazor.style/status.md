...
`Jazor.Style` 的产品合同已经实现。该模块作为独立 opt-in 包，复用 Jazor 的常规编译、模块目录、物化与打包链路；没有引入 CSS 专用 Hook、RazorVue lowering、分析器例外或 MSBuild 配置。

公共入口统一为小写静态类 `css`。消费方既可使用 `css.style(...)`、`css.px(...)`，也可通过 `using static Jazor.Style.css;` 直接调用 `style(...)`、`px(...)`。模型类型继续采用 `CssRule`、`CssContext`、`CssOptions` 等标准 PascalCase 名称。

debug 运行时入口统一为 `jazorStyle.mjs`。内部 `jazor-css:v1`、`jz-*`、`jz-k-*`、默认 StyleId 与 DOM 条目帧保持稳定，因此模块路径调整不会改变既有样式名称或 hydration 内容。

## 已完成能力

| 领域 | 当前合同 | 证据 |
| --- | --- | --- |
| 独立包 | `Jazor.Style` 精确依赖同版本 `Jazor`，不安装 CSS targets | pack 与本地 NuGet 消费 |
...
| 确定性 | 属性排序、顺序敏感内容、领域隔离、固定 hash、碰撞失败 | `Jazor.Style.Test` |
...
| RazorVue | 普通导入 `jazorStyle.mjs`，类名仍为普通字符串 | RazorVue 聚焦回归 |
...
- `none`：不物化前端产物；
- `debug`：输出 `jazorStyle.mjs`、source map、消费模块与 manifest；
...
dotnet run --file scripts/csharp/verify-jazor-style-browser.cs
...
- `Jazor.Style.Test` 22 项通过；
- Compiler 内联运算符回归 1 项通过；
- RazorVue 聚焦回归 1 项通过；
- Emit catalog 回归 2 项通过；
- 本地 NuGet debug/release 消费回归 1 项通过。
