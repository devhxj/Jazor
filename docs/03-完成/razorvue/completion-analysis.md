...
13. 这条扩面没有放宽 async 语义边界。`async` helper、`Task` / `ValueTask` 返回 helper、`ref` / `out` / `in` 参数，以及 body 超出现有 compiler-owned statement lowering 支持面的 helper 仍然显式报 `UnsupportedSetupLogicLowering`；这里修掉的是“人工深度上限”，不是把 setup-side logic 扩成任意方法执行模型。
...
18. 这条扩面同样刻意保持 fail-fast：getter property 链一旦形成循环依赖，会在编译期直接报 `UnsupportedSetupLogicLowering`；当时的 declaration-initialized value-like property / field 仍按 source-stable 合同处理，后续可观察写入会 fail-fast。当前这一点已在 2026-05-23 进一步拆分为 source-stable `const` carrier 与 private mutable setup `let` carrier 两类合同。
...
22. 这条补齐没有放宽 `SetParametersAsync` 的执行模型。当前依然只接受 no-op、base pass-through，以及“base pass-through 后可带 source-stable local / local function / callable local 前缀，再进入单个受支持 `InvokeAsync` emit”这类可诚实映射到单个 Vue `watch` 的形态；重复 emit、额外 mutation、控制流或更一般的方法体仍显式 fail-fast。
...
36. 这条 helper payload 扩面仍刻意保持 fail-fast：`async` helper、`Task` / `ValueTask` 返回 helper、`ref` / `out` / `in` 参数、越出当前 setup lowering / `SemanticWalker` 支持面的 helper body、不能作为 setup carrier 或 source-stable carrier 的 mutable/later-written member、一般外部 invocation、同一个 lifecycle body 内重复 emit / 额外 mutation / 控制流 / 多语句执行模型，以及更宽的动态 payload 仍继续显式回到 `UnsupportedLifecycleLowering` / analyzer `JAZORVUE005`；这里关闭的是“受控 helper-call payload 不能进入 lifecycle lowering”的缺口，不是把 lifecycle lowering 扩成任意 setup helper 执行通道。
...
41. 2026-05-23 本轮继续把 lifecycle payload 的 compiler-owned fallback 从 `firstRender` 专用路径扩到普通 lifecycle 的受控 local payload。`var label = Prefix + Value; ValueChanged.InvokeAsync(label + "!")` 这类 source-stable local、`string FormatLabel(int value) => "Count: " + value; ValueChanged.InvokeAsync(FormatLabel(Value))` 这类 local function payload，以及 `Func<int, int> increment = static value => value + 1; ValueChanged.InvokeAsync(increment(Value))` 这类 callable local payload，现在都会在 `OnInitialized*` / `OnParametersSet*` 等普通 hook body 内先发射稳定 prelude alias / local callable，再把最终 payload 交回 `EmitSetupExpression -> SemanticWalker -> Jazor.Compiler`。对 `OnAfterRender*`，`firstRender` 使用检测也会递归进入 source-stable local initializer、local function body 与 callable local initializer，避免 local function 闭包捕获 `firstRender` 时漏发 `currentFirstRender` snapshot。
42. 当前新增 focused 回归已补通过：
   - `dotnet test src/Jazor.RazorVue.Test/Jazor.RazorVue.Test.csproj --filter 'FullyQualifiedName~SourceStableLocalLifecyclePayload|FullyQualifiedName~LocalFunctionLifecyclePayload|FullyQualifiedName~LocalFunctionCapturedFirstRenderLifecyclePayload|FullyQualifiedName~LocalFunctionFirstRenderPayload' -v minimal`
43. 普通 lifecycle 的 helper/local-function payload 参数绑定本轮也与 shared invocation binder 对齐：omitted optional default、按 Roslyn 绑定成单数组形参的 `params`、以及 named argument out-of-order 都已覆盖到 analyzer / generator / pipeline 三层。这里仍没有在 RazorVue 内拼装 CLR 调用语义；参数表达式会沿 `EmitSetupExpression -> SemanticWalker -> Jazor.Compiler` lower，外层 binder 只负责调用点求值顺序与形参落位。
44. 这次补齐同时纠正了旧测试/文档里把 `OnInitialized*` / `OnParametersSet*` 中局部名 `firstRender` 误当成 after-render 协议风险的口径：普通 lifecycle 里的 `var firstRender = 1; ValueChanged.InvokeAsync(firstRender);` 现在按普通 source-stable local payload 支持，且不会生成 `currentFirstRender` snapshot。`currentFirstRender` 仍只属于真实 `OnAfterRender*` 参数快照协议。
45. 当前 focused ordinary lifecycle 回归已补通过：
   - `dotnet test src/Jazor.RazorVue.Test/Jazor.RazorVue.Test.csproj --filter FullyQualifiedName~PayloadOnParametersSet -v minimal`
   - `dotnet test src/Jazor.RazorVue.Test/Jazor.RazorVue.Test.csproj --filter FullyQualifiedName~OptionalParamsAndNamed -v minimal`
   - `dotnet test src/Jazor.RazorVue.Test/Jazor.RazorVue.Test.csproj --filter FullyQualifiedName~CallableLocalLifecyclePayload -v minimal`
   - `dotnet test src/Jazor.RazorVue.Test/Jazor.RazorVue.Test.csproj --filter FullyQualifiedName~LocalFirstRenderPayloadOnInitialized -v minimal`
   - `dotnet test src/Jazor.RazorVue.Test/Jazor.RazorVue.Test.csproj --filter FullyQualifiedName~LocalNamedFirstRenderOnInitializedPayload -v minimal`
46. 同一 compiler-owned local-prefix 支持现也补齐到 `SetParametersAsync` 的 base-pass-through emit 子集：`await base.SetParametersAsync(parameters); var label = Prefix + Value; await ValueChanged.InvokeAsync(label + "!");`、local function payload、以及 `Func<...>` callable local payload 都会保留为同一个 watch/emit 生命周期计划。analyzer 侧复用 lowering support-shape，不再维护一套独立语法矩阵，避免 `JAZORVUE006` 与 pipeline/generator 漂移。
47. 当前 focused SetParametersAsync 回归已补通过：
   - `dotnet test src/Jazor.RazorVue.Test/Jazor.RazorVue.Test.csproj --filter 'FullyQualifiedName~BaseThenSourceStableLocalEmitSetParametersAsync|FullyQualifiedName~BaseThenLocalFunctionEmitSetParametersAsync|FullyQualifiedName~BaseThenCallableLocalEmitSetParametersAsync|FullyQualifiedName~SupportedSetParametersAsyncBaseThenSourceStableLocalEmit|FullyQualifiedName~SupportedSetParametersAsyncBaseThenLocalFunctionEmit|FullyQualifiedName~SupportedSetParametersAsyncBaseThenCallableLocalEmit|FullyQualifiedName~SetParametersAsyncWithBaseEmitAndDerivedEmit|FullyQualifiedName~NonTrivialSetParametersAsync|FullyQualifiedName~PassThroughSetParametersAsyncToExternalOverrideWithoutSource|FullyQualifiedName~UnsupportedSetParametersAsyncLifecycle|FullyQualifiedName~BaseEmitPlusDerivedEmitSetParametersAsync' -v minimal`
...
本轮继续补齐 Razor IR imperative render、局部 `RenderFragment` carrier 与 setup value member 的真实生产路径，重点是消除“前端看似支持、后续 lowering 丢 carrier/local 可见性”的裂缝。

1. `Jazor.Compiler` 新增 `SemanticWalkerHost.RewriteSimpleAssignmentPreorder(...)` seam，RazorVue 在 imperative render bridge 中通过该 seam 重写局部 `RenderFragment` / `RenderFragment<T>` 简单赋值。这样 immediate-assignment carrier 仍经 `SemanticWalker` 主链完成语句/表达式 lowering、import/alias/reference 语义收集，而不是在 RazorVue 内手拼 CLR-aware JS。
2. Razor IR 局部 `RenderFragment` / `RenderFragment<T>` carrier 现在可跨 trailing imperative tail 保活。例如 carrier 后接 `while` / conditional `return` 再赋给组件 typed slot/template 参数时，frontend 会提升必要片段为 `RazorVueImperativeBlockNode`，并保留 carrier/local 的可见性。
3. 对“先声明、再立即赋值”的 local carrier，如果 tail 后仍有 `index` 这类 local 读取，segmentation planner 会把 declaration/immediate-assignment 前缀并入同一 imperative segment；这修掉了重复 `index` 声明、post-loop local 丢失，以及 slot 参数消费时 `template` 失效的问题。
4. typed child-content / typed slot body 的 direct Razor control-block 也补齐到正式 imperative render 主线：`@while (...) { ... }` 会 lower 成 scoped slot callback 中的 imperative render IIFE，`@if (Hide) { return; } <p>@item</p>` 会保留提前返回对后续 tail markup 的真实控制流。
5. setup-side value-like member 合同拆清为两类：只读/getter-only/source-stable declaration initializer 继续作为 setup `const`；private mutable field / private-setter auto-property 则作为 setup `let` carrier，允许源码中存在 later writes。
6. private mutable setup carrier 会保留声明点 initializer；无 initializer 时按 CLR 默认值发射；若存在 initializer 但无法通过 `Jazor.Compiler` / `SemanticWalker` lower，则显式报 `UnsupportedSetupLogicLowering`。
7. direct template expression、setup helper、lifecycle payload 与 imperative render body 现在共用同一 setup carrier 主线。尤其 private mutable field 经 helper body 读取的路径已补齐，不再出现“直接模板读取支持、helper 间接读取失败”的裂缝；带自定义 getter/setter 的 mutable property 仍显式 fail-fast，不会被错误当成 auto-property 状态 carrier。
8. 这次扩面没有放宽 `RenderFragment` / `MarkupString` / static markup 的 source-stable carrier 合同。它们仍要求可证明稳定，later writes 仍按各自 source-stable 合同 fail-fast。
