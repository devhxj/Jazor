...
41. 这条扩面同样没有把 lifecycle payload 放宽成任意表达式执行模型。deeper object-construction/member-chain、local lambda / local-function、property-pattern、依赖额外 source-stable object boxing/local carrier 的 declaration-pattern / pattern-var、null-conditional + coalesced 组合，以及更宽 dataflow 形状仍显式 fail-fast；这里开放的是“现有 compiler-owned lowering 能诚实承载的 firstRender 表达式”，不是额外发明 RazorVue 私有 JS 语义。
