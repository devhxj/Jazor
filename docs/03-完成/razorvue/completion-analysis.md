...
36. 这条 helper payload 扩面仍刻意保持 fail-fast：`async` helper、`Task` / `ValueTask` 返回 helper、非精确 arity helper 调用、`ref` / `out` / `in` 参数、越出当前 setup lowering / `SemanticWalker` 支持面的 helper body、不能作为 setup carrier 或 source-stable carrier 的 mutable/later-written member，以及更宽的动态 payload 仍继续显式回到 `UnsupportedLifecycleLowering` / analyzer `JAZORVUE005`；这里关闭的是“受控 helper-call payload 不能进入 lifecycle lowering”的缺口，不是把 lifecycle lowering 扩成任意 setup helper 执行通道。
...
41. 2026-05-23 本轮继续把 lifecycle payload 的 compiler-owned fallback 从 `firstRender` 专用路径扩到普通 lifecycle 的受控 local payload。`var label = Prefix + Value; ValueChanged.InvokeAsync(label + "!")` 这类 source-stable local、以及 `string FormatLabel(int value) => "Count: " + value; ValueChanged.InvokeAsync(FormatLabel(Value))` 这类 local function payload，现在都会在 hook body 内先发射稳定 prelude alias / local function，再把最终 payload 交回 `EmitSetupExpression -> SemanticWalker -> Jazor.Compiler`。对 `OnAfterRender*`，`firstRender` 使用检测也会递归进入 source-stable local initializer、local function body 与 callable local initializer，避免 local function 闭包捕获 `firstRender` 时漏发 `currentFirstRender` snapshot。
...
43. 同一 compiler-owned local-prefix 支持现也补齐到 `SetParametersAsync` 的 base-pass-through emit 子集：`await base.SetParametersAsync(parameters); var label = Prefix + Value; await ValueChanged.InvokeAsync(label + "!");`、local function payload、以及 `Func<...>` callable local payload 都会保留为同一个 watch/emit 生命周期计划。analyzer 侧复用 lowering support-shape，不再维护一套独立语法矩阵，避免 `JAZORVUE006` 与 pipeline/generator 漂移。
44. 当前 focused SetParametersAsync 回归已补通过：
