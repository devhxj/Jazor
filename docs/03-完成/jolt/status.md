...
`Jolt` 当前七阶段里程碑验收项已全部收口，进入稳态巡检与持续压实阶段。
...
- `src/Jazor.CompilerTest` 中 `Jolt*Tests` 文件已达到 20+，覆盖 DevServer/LSP/Build/Debug/SourceMap 等主域
- 当前本地工作区存在长时间运行的 `Jolt --lsp` 进程时，`dotnet test` 可能出现 dll 锁定失败；验证时需先清理占用进程
...
- `dotnet test src/Jazor.CompilerTest/Jazor.CompilerTest.csproj --filter 'FullyQualifiedName~Jolt' --no-restore -v minimal`：**466/466 通过**
- `dotnet test src/Jazor.CompilerTest/Jazor.CompilerTest.csproj --filter 'FullyQualifiedName~JoltLspTests|FullyQualifiedName~JoltPhase7ExtensionTests|FullyQualifiedName~JoltDebugProtocolTests|FullyQualifiedName~JoltBuildTests|FullyQualifiedName~JoltFrontendLaneTests' --no-restore -v minimal`：**178/178 通过**
...
- `dotnet test src/Jazor.CompilerTest/Jazor.CompilerTest.csproj --filter 'FullyQualifiedName~JoltPhase7ExtensionSecurityAndBuiltinTests|FullyQualifiedName~JoltPhase7ExtensionTests' --no-restore -v minimal`：**32/32 通过**
