# ECMAScript.Vben 抽象架构

## 结论

`ECMAScript.Vben` 应该是**后台框架抽象层 + 自有原生组件层**，而不是第三方组件库适配层。

因此：

- `ECMAScript.Vben.TDesign` 不应作为正式产品项目存在
- `ECMAScript.Vben.ElementPlus` 也不应作为正式产品项目存在
...
而不是进入 `ECMAScript.Vben` 主产品线。

## 为什么 `ECMAScript.Vben.TDesign` 是错误分层
...
一旦出现 `ECMAScript.Vben.TDesign` 这种命名，就意味着框架层已经默认：
...
3. 扩展成本失控  
   后面每接一个库，就要再造一条 `ECMAScript.Vben.Xxx` 线，最后维护的是 N 条耦合支线，而不是一个稳定框架内核。
...
- 如何把 `VbenNavItem` 投影到第三方菜单
- 如何在 `VbenPageContainer` 的 slot 里放入第三方按钮/表单/表格
- 如何让应用层主题和 `Vben` 布局一起工作

但这些都不应回流为 `ECMAScript.Vben.*第三方库名*` 的正式核心项目。
...
正确方式不是做 `ECMAScript.Vben.TDesign` 这类正式适配项目，而是：
...
1. `ECMAScript.Vben` 抽象 contract
2. `ECMAScript.Vben` 自有原生壳层组件
...
- `ECMAScript.Vben.TDesign`
- `ECMAScript.Vben.ElementPlus`
- 某个 UI 库的整套 adapter project

## 当前纠偏结论

基于当前目标，仓库里的 `src/ECMAScript.Vben.TDesign/` 应视为：
