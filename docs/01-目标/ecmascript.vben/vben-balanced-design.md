# ECMAScript.Vben 平衡式目标设计

## 定位

`ECMAScript.Vben` 不是对 `src/vben/` 源码的直接搬运，也不是某个 UI 组件库的又一套薄绑定。  
...
4. 与现有 `ECMAScript.TDesign`、未来 `ECMAScript.ElementPlus` 保持解耦：`ECMAScript.Vben` 不反向污染底层组件绑定，也不为第三方库建立正式 adapter 产品线。
5. 让第一阶段先跑通最小后台壳层，不把表单、表格、schema、权限系统等大块能力一次性塞进首包。

## 非目标

- 不直接复刻 `src/vben/` 全部工程结构、脚手架、构建逻辑与前端运行时。
- 不把 `ECMAScript.Vben` 设计成 `Element Plus` 或 `TDesign` 的完整组件代理层。
...
`ECMAScript.Vben` 建立在 `ECMAScript.Vue3` 之上，但不扩张 Vue3 核心绑定边界。  
它是外部库线里的更高层 authoring surface。

### 与 `Jazor.RazorVue`

`ECMAScript.Vben` 的组件 authoring 需要继续遵守 RazorVue 当前原生组件规则：
...
因此 `ECMAScript.Vben` 会在公共层设计上继续保证对 Element Plus 友好，但不会为其建立专属主线 adapter project。
