...
3. 对于可空返回的文档应该说明 JavaScript 可能产生 `undefined`，并且 C# 投影将该 absence 映射到 `null`。
4. 内部编译器和 runtime 层可能仍然在 JavaScript 语义需要的地方发出或测试真实 JavaScript `undefined`。
...
- 当现有兼容性表面必须保持不可空索引器时，记录该索引器镜像直接 JavaScript 属性访问，并且应该优先使用诸如 `At()` 之类的 APIs 进行 absence-aware 读取。
...
对于这些 APIs，调用者应该将值读取与显式存在检查（如 `Has`）配对。
投影应该青睐 JavaScript 的 host 形状，而文档解释 C# null-projection 行为及其限制。

## 当前仓库策略

- 公共 ECMAScript host 映射仅暴露 `null` 作为 C# 无值表面。
- 编译器和 CLR 桥接内部可能仍然使用真实 JavaScript `undefined`。
- 如果未来的 API 需要比 `null` 可以表达的更强的区别，优先使用 accompanying 协议（如 `Has`）而不是暴露公共 `undefined` 值。
