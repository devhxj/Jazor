...
1. emit 如何组装最终 bundle
2. import 重写如何保持在 emit 内部
3. 模块级 SourceMap 生成目前处于何处
4. 狭窄的 active SourceMap lane 如何通过 emit 继续
...
这将 bundling 保持在 host-facing lane，而不是让它泄漏回编译器语义。
...
- bundling 保持为 emit 关注点，绝不能强制编译器重新设计
...
当前 emit 中的 SourceMap 情况是有意保持狭窄的：

- 模块级 map 生成已经位于此处
- 与 RazorVue 相关的 SourceMap 继续是活跃的
- 更广泛的 SourceMap 推广仍然比此切片更保守

这意味着 emit 已经是 SourceMap 写入的操作主页，
但不是整个广泛 SourceMap 程序的所有者。
