...
- 这条扩面仍保持生产级保守边界：`async` local helper / local lambda、`Task` / `ValueTask` 返回 local helper、无法从源码稳定恢复初始化器或声明前缀的 callable local、依赖额外 source-stable object boxing/local carrier 的 declaration-pattern / pattern-var，以及更宽 dataflow 形状继续 fail-fast。这里开放的是“现有 compiler-owned lowering 真能诚实承载的 firstRender 表达式”，不是继续靠 RazorVue 特判堆矩阵
