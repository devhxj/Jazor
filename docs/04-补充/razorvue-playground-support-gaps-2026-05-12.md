...
- 这条扩面仍保持生产级保守边界：deeper object-construction/member-chain、local lambda / local-function、property-pattern、依赖额外 source-stable object boxing/local carrier 的 declaration-pattern / pattern-var、null-conditional + coalesced 组合，以及更宽 dataflow 形状继续 fail-fast。这里开放的是“现有 compiler-owned lowering 真能诚实承载的 firstRender 表达式”，不是继续靠 RazorVue 特判堆矩阵
