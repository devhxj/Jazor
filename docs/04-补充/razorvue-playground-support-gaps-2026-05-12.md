...
- setup-side helper lowering 本轮也完成了一次边界校准：旧的“helper 最多只支持 2 层组合”不再是实际 contract。当前同步 current-component helper 只要仍处在源码可分析、单表达式 / 单返回、无 async 的既有受控子集内，就会继续递归收集并 lower 到同一 setup scope；`FormatOuter -> FormatMiddle -> FormatInner` 这类三层及以上同步 helper 组合现已正式支持
...
- setup-side declaration-initialized value-like property 现也已从缺口转入正式支持面，但合同仍刻意收窄为 source-stable member：getter-only / declaration-initialized property 会直接发射为 setup value binding；private mutable property 只有在“声明点初始化 + 源码可证明无后续写入”时才接受，helper body 与 direct template expression 都共用同一条 setup binding 主线
- 同一条 value-like member 合同现在也收紧到了 setup field：declaration-initialized field 只有在 `readonly` 或“private mutable + 无后续写入”时才允许进入 setup lowering；不再把带 later writes 的 field 静默 hoist 成看似稳定的 setup 变量
- 当前也已正式锁定 fail-fast 边界：getter property 链若形成循环依赖，会在编译期直接报 `UnsupportedSetupLogicLowering`；declaration-initialized value-like property / field 一旦后续再次出现可观察写入，也会直接报 `UnsupportedSetupLogicLowering`，而不是继续沿首次初始化静默恢复旧值。当前仍明确不支持的是依赖构造/写入时序语义的 property，以及超出单表达式 / 单返回受控子集的 getter
- 这条扩面没有放宽到 async helper 或任意方法体：`async` helper、`Task` / `ValueTask` 返回 helper、以及超出当前受控 body 形状的 helper 仍显式报 `UnsupportedSetupLogicLowering`。因此这里关闭的是“人工深度上限”缺口，不是把 setup-side logic 变成通用执行模型
...
- `var pair = (firstRender, new ReadyState(firstRender)); var (_, readyState) = pair; readyState.Value` 这类 tuple deconstruction source-stable local payload 经过 focused 回归重新核对，当前仍未真正支持；现有 deconstruction initializer 投影骨架还没有把这条 lifecycle declaration 形态完整接上，所以这里继续保留 fail-fast 记录。
- 这条扩面仍保持生产级保守边界：local lambda / local-function、tuple deconstruction payload、依赖额外 source-stable object boxing/local carrier 的 declaration-pattern / pattern-var，以及更宽 dataflow 形状继续 fail-fast。这里开放的是“现有 compiler-owned lowering 真能诚实承载的 firstRender 表达式”，不是继续靠 RazorVue 特判堆矩阵
