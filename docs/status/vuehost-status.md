...
- Phase 4：DAP 基础链路已落地，CDP 实浏览器闭环仍未完成
...
- SourceMap 当前仍以行级为主，调试消费链路尚未完整打通
...
| Phase 2 Source Map | 大部分完成 | 行级映射与链式映射已落地，列级精度与调试消费待补 |
| Phase 3 HMR | 已完成并持续增强 | 工作区变更、去重、style/js/full-reload 分类已实现 |
| Phase 4 Debug (DAP + CDP) | 部分完成 | DAP 基础可用，CDP 真浏览器调试闭环待补 |
...
1. **Phase 4 调试闭环**
   - 打通 DAP 到 CDP 的真实浏览器调试传输
   - 把现有 SourceMap 服务接入调试调用栈与断点映射主路径
