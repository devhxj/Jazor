...
当前已验证的 Compiler 基线为 `98.94%` 行覆盖率和 `96.01%` 分支覆盖率，不再作为后续发布版本的独立门槛。RazorVue `0.4` 门禁已于 2026-08-09 验证：`4613/4613` 个官方 Razor SG 场景通过，行覆盖率为 `8263/8630 = 95.75%`，分支覆盖率为 `3758/4175 = 90.01%`。当前源码打出的本地 NuGet 包也已通过 Deno 执行与真实浏览器 Counter smoke；主线因此进入 `0.4` 阶段，下一门槛为 `0.5` 的 RazorVue 分支覆盖率 `96%`。
