...
> **日期**：2026-07-22
>
> **状态**：Draft v3 - G0 已通过；Task 0.5 清理已验证但尚未提交；Task 1.1 与 Task 1.2 已完成；Task 1.3 compiler closure/rewrite、SG bridge seam、最小内存 `.mjs` framing、browser-visible state/event 与 dynamic dispatch/source-origin diagnostic 首切片已完成；Task 1.4 carrier/Emit/runtime asset/canonical manifest、expression-level `.mjs.map`、stale cleanup、writer repeat-write determinism 与外部 consumer clean-build artifact hash 首切片已通过；Task 1.5 真实 browser Counter 首切片已通过；Task 2.2 DOM `@bind` v1（`CreateBinder` 简单 state assignment + `SetUpdatesAttributeName` hint）、component `@bind-X` 常见 `X`/`XChanged` 首切片、default/named 非泛型 `RenderFragment` slot 首切片、`RenderFragment<T>` component slot fail-fast diagnostic 首切片、父子 component parameter lower-camel bridge、EventCallback 参数 `emits: [...]` metadata 首切片、EventCallback await/error/invalidation 规则与 sibling component import rebasing 已接线；Task 2.5 benchmark protocol、runtime partial measurement 与 三 fixture official SG generated artifact size/hash 首切片（含 handwritten `.mjs` baseline）已加入，完整 G2 采样仍待完成
...
> Status: first slice extended（2026-07-27：`defineComponent` 输出 `props: ["title", ...]` 来自组件全部 `[Parameter]`；`EventCallback`/`EventCallback<T>` 的 `Invoke`/`InvokeAsync` 降为 `props.onX?.(args)`；EventCallback 参数会派生 Vue `emits: [...]` metadata（例如 `ValueChanged`→`valueChanged`）；EventCallback await/error/invalidation v1 规则已固定：lowering 不包 `Promise.resolve(props...)`、不 catch listener error、不由 EventCallback 自动触发 `StateHasChanged`，源码 `await` 由编译器 await lowering 保留；DOM multiple attributes 会规范化事件名，component multiple attributes 保留参数/emit 名原样；DOM `@bind` v1 支持 `EventCallback.Factory.CreateBinder(this, value => state = value, state)` 简单 state assignment 与 `SetUpdatesAttributeName` render-context hint；component `@bind-X` 首切片支持 official SG 常见 `AddComponentParameter("X", value)` + `AddComponentParameter("XChanged", CreateBinder(...))`；descriptor bind 首切片读取 class-scoped `[VueProp(Name=...)]` / `[VueLibraryEmit(Name=...)]`，支持 `Value`/`ValueChanged` 映射到 `modelValue`/`onUpdate:modelValue`，并将 generated child 自身 `props/emits` catalog 输出为 descriptor runtime name；default 与 named 非泛型 `RenderFragment` 参数走 Vue slots/default 或 slots/name bridge，不进入 props；`RenderFragment<T>` component slot 现在显式 fail-fast 并提示 typed slot descriptor lowering；`AddComponentParameter("Title"/"OnValueChanged", ...)` 到生成子组件 `props.title`/`props.onValueChanged` 的 lower-camel bridge 已接线，sibling component import 会按 importing `.mjs` 路径重写为相对 import；Razor-side unknown/required/type mismatch 由 Razor/C# 编译链路负责，lowering 不重复校验；typed slot descriptor/lowering matrix 仍未接线）
...
- [x] component bind descriptor breadth 首切片：`OpenComponent<T>` 读取目标 component class-scoped `[VueProp(Name=...)]` / `[VueLibraryEmit(Name=...)]` descriptor，`AddComponentParameter("Value"/"ValueChanged", ...)` 经 render-context map 输出 `modelValue` / `onUpdate:modelValue`；generated child 的 full `[Parameter]` props/emits catalog 不再被 reachable closure 裁剪。
...
- [x] sync/async `EventCallback` 的 await、error propagation 和 render invalidation 次数有统一规则：`Invoke`/`InvokeAsync` 直接降为 optional Vue listener call，discarded call 不 await、不包 `Promise.resolve(props...)`，源码 `await` 保留为 JS `await`，listener error 由 JS call/Promise 自然传播，EventCallback 本身不触发 render invalidation。
