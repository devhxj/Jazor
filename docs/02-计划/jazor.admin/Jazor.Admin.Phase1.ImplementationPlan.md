# Jazor.Admin 第一阶段实施计划

> Status: 活跃计划  
> Updated: 2026-05-15  
> Positioning: 基于 `docs/01-目标/jazor.admin/` 的设计边界，为 `src/Jazor.Admin/` 建立第一阶段可落地实施顺序。当前关注点是后台壳层核心与原生实现闭环，而不是完整复刻前端后台框架，也不是为第三方组件库建立专属 adapter project。
...
- `src/JazorAdmin/` 不是正确产品方向，只能视为临时 spike
- Phase 1 不再把任何 `Jazor.Admin.*第三方库名*` 项目作为正式交付物

当前实现状态补充：

- `src/Jazor.Admin/` 已形成原生 Razor 壳层组件骨架并可构建
- `src/Jazor.Admin.Test/` 已建立独立聚焦验证，不再依赖当前被 `ECMAScript.ElementPlus` 阻塞的共享测试主线
- RazorVue 已补齐“引用程序集中的原生 `IVueComponent` 组件也可进入组件注册解析”的能力，确保 `Vben` 作为独立包时仍可被消费
- `AdminLayout` / `SidebarMenu` / `HeaderBar` / `PageContainer` 已明确承担 `IVueContainerComponent` 容器契约角色
...
1. **Phase A: 核心 contract**
   - 新建 `Jazor.Admin` 公共语义层
   - 定义后台壳层结构组件和导航模型

2. **Phase B: 原生壳层闭环**
   - 在 `Jazor.Admin` 内实现自有原生后台壳层
...
- `AdminLayout` 负责区域组织
- `SidebarMenu` 负责导航树与 selected/expanded/collapsed
- `HeaderBar` 负责 logo/title/actions/user-region 这类顶部结构
- `PageContainer` 负责页面标题、副标题、extra actions、正文容器

当前补充约束：

- 以上公开壳层组件默认就是原生实现，同时也是正式容器 contract
- 若应用层要替换整个 `PageContainer` 或 `HeaderBar`，必须通过 `VueInject` 选择实现，而不是引入 `Vben.*第三方库*` 主线工程
...
- 默认原生解析回归
- `AdminLayout` / `SidebarMenu` / `HeaderBar` / `PageContainer` 的 `[VueInject]` 替换回归
...
- `samples/Jazor.Admin.ElementPlusInject/` 是 pre-G0 真实 sample：根组件只 authoring `Vben` contract，sample-local `IVueContainerImplementation<TContainer>` 负责 Element Plus 具体实现，但 consumer 仍走已退役的 SFC bridge / `razorvue-consumer-entry` 主链，等待当前 `.mjs` 路线迁移。
...
- `samples/Jazor.Admin.ElementPlusInject/` 已按这个方向建立，但当前仍是 legacy/pending-migration sample；
...
1. 新建 `docs/01-目标/jazor.admin/` 与 `docs/02-计划/jazor.admin/`
2. 新建 `src/Jazor.Admin/` 项目骨架
3. 建立基础 base types 与 nav/page model
4. 建立 `AdminLayout` / `PageContainer`
5. 建立 `SidebarMenu`
6. 补原生实现验证 sample
