...
实现 Jolt 的可扩展分析器系统、高级 LSP 特性补充（Signature Help、Inlay Hints、Workspace Symbol、Folding Range）、VS Code 扩展集成、故障隔离与健康治理，构建完整的开发生态。
...
namespace Jolt.Extensions.Loading;
...
```json
{
  "name": "jolt",
  "displayName": "Jolt",
  "description": "Jolt - Razor-first Vue development experience",
  "version": "0.1.0",
  "publisher": "jazor",
  "engines": {
    "vscode": "^1.85.0"
    ...
    "configuration": {
      ...
      "properties": {
        "jazor.trace.server": {
          ...
          "default": "off",
          "description": "Traces the communication between VS Code and the Jolt server."
        },
        "jazor.server.path": {
          "type": "string",
          "default": "",
          "description": "Path to the Jolt executable. If empty, uses bundled version."
        },
...
export async function activate(context: vscode.ExtensionContext) {
    console.log('Jolt extension is activating...');
    ...
    if (!serverPath) {
        vscode.window.showErrorMessage('Jolt server not found. Please install Jolt.');
        return;
    }
    ...
    console.log('Jolt extension is now active!');
}

function findBundledServer(context: vscode.ExtensionContext): string | undefined {
    // Look for bundled Jolt
    const bundledPath = path.join(context.extensionPath, 'server', 'Jolt.csproj');
...
function registerCommands(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('jazor.restartServer', async () => {
            if (client) {
                vscode.window.showInformationMessage('Restarting Jolt server...');
                await client.stop();
                await client.start();
                vscode.window.showInformationMessage('Jolt server restarted.');
            }
        })
    );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('jazor.startDevServer', async () => {
            ...
            terminal.sendText('dotnet run --project Jolt -- --dev');
            terminal.show();
        })
    );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('jazor.build', async () => {
            ...
            terminal.sendText('dotnet run --project Jolt -- --build');
            terminal.show();
        })
    );
}
