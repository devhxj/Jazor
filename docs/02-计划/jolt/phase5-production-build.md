...
```javascript
// build-plugin.mjs (由 Jolt 生成)
export default {
  name: 'jazor-plugin',
  setup(build) {
    // 拦截 .jazor 和 .vue 文件
    build.onLoad({ filter: /\.(jazor|vue)$/ }, async (args) => {
      // 通过 HTTP 调用 Jolt 编译服务
    ...
    // 拦截 .ts 文件（可选，让 esbuild 内置处理或委托 Jolt）
    build.onLoad({ filter: /\.ts$/ }, async (args) => {
      // 方案 A: 让 esbuild 内置处理（更快）
      // 方案 B: 委托 Jolt（统一 Source Map）
...
public sealed class EsbuildPluginGenerator
{
    ...
    public async Task<string> GenerateAsync()
    {
        ...
        var pluginCode = $@"
// Auto-generated by Jolt BuildOrchestrator
...
```bash
# 开发模式
dotnet run --project Jolt -- --dev

# 生产构建
dotnet run --project Jolt -- --build

# 指定配置
dotnet run --project Jolt -- --build --config ./jazor.prod.json

# 预览构建产物
dotnet run --project Jolt -- --preview
```
