...
- 当前聚焦验证已通过：
  ...
  - `samples/RazorVue.TodoList/todo-consumer` 已通过 `npm run build`，底层调用仓库内 bundled `deno.exe`，证明生成 SFC 可被纯 Deno SFC 预编译 + `deno bundle` production build 消费
  - `samples/RazorVue.TodoList/todo-consumer` 已通过 `npm run smoke:ssr`，证明生成 SFC 可经纯 Deno 预编译、Vue server renderer 和 Vuetify plugin 做 runtime render
  - `samples/RazorVue.TodoList/todo-consumer` 已通过 `npm run smoke:bundle-api`，证明同一预编译入口也可被 `Deno.bundle()` API 消费
