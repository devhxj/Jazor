...
> Status: 活跃计划
> Positioning: RazorVue HMR 的分阶段清单；0.7 已完成一个保守运行时通道。
> Note: 当前通道只覆盖编译器证明的 `template-only` 更新和应用显式处理器，不是通用 Vue 热替换实现。
...
- 宿主可以接收编译器变更元数据
- 宿主只在模板哈希变化且描述符/逻辑/身份不变时尝试最小热更新
- 宿主可以在不被允许时干净地回退
...
- 一个仅模板案例在真实浏览器中通过 WebSocket、动态导入和已注册处理器工作正常
- 一个描述符变更案例强制完全重载
- 没有不安全的热补丁案例被静默接受
...
3. 边界分类测试
4. 清单往返测试
5. 真实浏览器的宿主运行时行为测试（`verify-development-hmr.cs`）
