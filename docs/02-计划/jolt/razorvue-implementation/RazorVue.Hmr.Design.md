...
> Status: 活跃参考
> Positioning: RazorVue HMR 的设计参考，记录已交付的保守运行时通道和未交付的扩展边界。

本文档定义 RazorVue HMR 的设计。

当前仓库已包含一条狭窄的 RazorVue HMR 运行时通道：开发宿主仅对编译器证明的 `template-only` manifest 差异发送更新，浏览器只在应用注册 `JazorHmr.accept(moduleId, handler)` 后动态导入并交给该处理器。它不是通用 Vue 热替换运行时，也不自动替换实例或保留状态。
...
- manifest 驱动的相邻构建比较
- 仅模板哈希变化的 WebSocket `module-update`
- cache-busted 动态导入和显式的 `JazorHmr.accept` 处理器
- 未被处理或无法证明安全时的完整刷新回退
...
- 宿主对 manifest 的保守失效接线
- 仅模板更新路径
- 显式的完全重载回退路径

验证只证明一个带注册处理器的模板更新工作流；它不等价于 Vue 自身的自动热替换能力。
