...
- 终局蓝图以 `docs/architecture/jolt-capabilities.md` 为准：三 Lane、Deno Worker、ProjectionMap/Source Map 分层、单进程 LSP + DevServer。
- 当前实施约束以 `docs/architecture/jolt-single-project.md` 为准：`Jolt` 是唯一宿主边界，迁移先于删除旧项目，P1 文档范围是 `didOpen/didChange/didClose/completion/hover/definition/diagnostics aggregation`。
- 现状代码尚未达到该约束：
  - `src/Jolt/Services/JoltService.cs` 和 `src/Jolt/Jazor/Projection/JazorProjectionService.cs` 仍直接依赖 `Jazor.Vue` / `Jazor.VueContracts`。
  - `src/Jolt/VirtualDocuments/Mapping/ProjectionMap.cs` 仅有 `CreateWholeDocument(...)`，尚无段级/字符级映射能力。
  - `src/Jolt/Lsp/Routing/DocumentProjectionResolver.cs` 仅按区域粗分 lane，未进行位置重映射。
  - `src/Jolt/Lsp/Lanes/FrontendLaneService.cs` 当前大量依赖正则、磁盘扫描和 fallback，本地启发式承担了真实能力。
  - `src/Jolt/Lsp/LspSession.cs` 当前对外已暴露 references / rename / codeAction，但这不作为 P1 承诺范围。
...
5. **迁移先于删除，但删除要进入本轮计划尾部**：一旦新路径覆盖并有测试兜底，即移除 `Jolt -> Jazor.Vue* / Jazor.Vite` 的依赖。
...
在新主路径通过测试后，解除 `Jolt` 对旧项目的直接依赖，并准备物理删除旧资产。

### 工作项

1. 删除 `src/Jolt` 内对以下项目的直接依赖：
   - `Jazor.Vue`
   - `Jazor.VueContracts`
