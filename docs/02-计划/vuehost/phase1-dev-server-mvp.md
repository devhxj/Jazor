...
实现 `dotnet run --project Jazor.VueHost -- --dev` 启动 HTTP 开发服务器，浏览器可访问并运行 `.jazor` 编写的 Vue 应用。
...
| `Jazor.VueHost.csproj` | 嵌入 `Client/jazor-hmr.js` 为资源 |
    ...
    string absolutePath, CancellationToken cancellationToken)
{
    ...
    var kind = VueHostWorkspaceResolver.MapDocumentKind(absolutePath);
...
public ResolveResult Resolve(string requestPath, string? importerPath = null)
{
    ...
    return new ResolveResult
    {
        ...
        ResolvedUrl = requestPath,
        DocumentKind = VueHostWorkspaceResolver.MapDocumentKind(staticPath),
        IsVirtual = false,
...
**测试**:
- 手动验证: 通过 `StdioVueHostRpcServer` 发送 `compile/sfc` 请求，验证返回 JS
- 新增 `JazorVueHostCompilationTests.cs`: 测试 `JazorVueCompiler` 输出 → Deno 编译 → JS 结果
...
- 修改 `Jazor.VueHost.csproj` — 嵌入 `jazor-hmr.js`
...
- Dev Server 和 LSP **共享** `VueHostService`、`JazorVueCompiler`、`DenoVolarHost`
- Dev Server **不共享** LSP 的 `LspSession`、Lane 体系 — 这是两个独立的运行模式
- 两者共享 `IVueHostWorkspaceStore`，但 Dev Server 使用文件系统而非 LSP `didChange`
