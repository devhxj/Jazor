...
> **CLR 模块边界（2026-08-27）**：任何进入支持面的 Blazor CLR 类型都必须先由 `Jazor.CLR.Generator` 从真实 ASP.NET Core reference symbol 生成初始 module/doc，再把选定骨架复制到 `src/Jazor.CLR/module/` 与 `src/Jazor.CLR/doc/` 完善实现。原始 CLR type/member key、`[Jazor]`/`Op.*` 声明与实际 runtime module/helper 的源码 owner 均为 `Jazor.CLR`；生成的 `WhiteList.cs.Generate.cs` 仍由 `Jazor.Compiler` 持有，最终 runtime module 进入 `src/ECMAScript` 的 `manifest.json + dist` 外部资源，不得在 `ECMAScript.Blazor` 维护第二份映射或资源清单。
> **ECMAScript.Blazor 边界（2026-08-27）**：`ECMAScript.Blazor` 只提供与 `ECMAScript/internal/Math.cs` 同类的标准 ECMAScript 模拟/投影扩展，可使用 `[ECMAScript]`、`[ECMAScript("specifier")]`、`[ECMAScriptInline]` 与 `Description` 等公开元数据；不得使用 `[Jazor]`、`Op.*`、`[ECMAScriptModule]`，不得成为 whitelist source-root 或 CLR mapping/runtime resource owner。没有额外作者扩展需求时，不为维持程序集存在而复制 CLR mapping。
...
| --- | --- | --- | --- |
| `NavigationManager` 基础导航、`LocationChangedEventArgs`、`NotFoundEventArgs`、URL-backed `System.Uri` | `v0.19.0` 已包含对应成员/runtime；更宽的 routing family 由 M5 ledger 的已声明内部 route/navigation 子集裁决，未覆盖 profile 明确排除 | 沿用既有 runtime，并增加导航取消能力所需的 host 状态 | S1 的基础，不重复实现 |
| `System.Threading.Tasks.Task`、`Task<TResult>` | 已有 Promise carrier 与受控成员面 | 不因新切片自动扩大 Task API | S1/S3/S5 只复用已批准路径 |
| `RegisterLocationChangingHandler(...)`、`LocationChangingContext` | 未发布 | CLR mapping、navigation state machine、compiler tests、Blazor reference oracle、official Razor SG + Deno、真实 HTTP-origin browser 与 isolated Release package consumer 已完成 | **Support（browser interactive 内部导航子集）**：仅同一 base URI 的内部 `NavigateTo` 承诺；popstate/hashchange cancellation、SSR/prerender 与 server circuit 不声明 |
...
| `MouseEventArgs`、`KeyboardEventArgs`、`FocusEventArgs` | Razor SG 可绑定 handler；首批 DOM carrier 与只读 getter 已由 `Jazor.CLR` modules 提供 | `Jazor.CLR.Generator` 已生成 skeleton，选定 modules/docs 已复制并完善；getter 使用原生 DOM carrier，未批准 constructor/setter 保持 `Op.Discard` | **Support（browser interactive）**：official SG、Deno、真实 browser 与 isolated Release package consumer 已通过 |
| `ChangeEventArgs` | Razor SG 可绑定 handler；`Value` 已通过 listener 边界 capture 与 `Jazor.CLR` `WeakMap` helper 投影 | generator skeleton、`ChangeEventArgsModule` mapping/helper 与 RazorVue capture framing 已收敛到单一 CLR owner | **Support（browser interactive）**：reference metadata/reader、official SG、Deno、真实 browser 与 isolated Release package consumer 已通过 |
| `@ref` capture / `ElementReference.FocusAsync` | capture 已由 render emitter 支持；`Jazor.CLR` 已把 `ElementReference` 视为 `HTMLElement`，并通过 generated CLR Import helper 接入两个 `FocusAsync` overload | `ElementReference`/extensions modules/docs 已由 generator skeleton 复制并完善；official SG/Deno、真实 browser 与 isolated Release consumer 已覆盖 DOM lifecycle、options 与 failure contract | **Support（browser interactive）**；SSR/prerender、server renderer identity 与任意 DOM methods 不在声明内 |
| `PointerEventArgs` | Razor SG 可绑定 `@onpointerdown`；只读 getter 直接读取原生 `PointerEvent`，WebIDL `long`/`int` 值保持 JavaScript `Number` | `Jazor.CLR.Generator` 已生成 skeleton，`PointerEventArgsModule`/doc 已复制并完善；setter 与构造器保持 `Op.Discard` | **Support（browser interactive）**：Blazor `EventHandlers` reference metadata、official SG/Deno、真实 BrowserSmoke 与 isolated Release package consumer 已通过；作者侧 `System.Int64` 的 `BigInt` 表示与该 DOM getter 的 `Number` carrier 分开处理 |
| `WheelEventArgs` | Razor SG 可绑定 `@onwheel`；`DeltaX/Y/Z/DeltaMode` getter 直接读取原生 `WheelEvent`，不在 listener 时刻 materialize | `Jazor.CLR.Generator` 已生成 skeleton，`WheelEventArgsModule`/doc 已复制并完善；setter 与构造器保持 `Op.Discard` | **Support（browser interactive）**：Blazor `EventHandlers` reference metadata、official SG/Deno、真实 BrowserSmoke 与 isolated Release package consumer 已通过；WebIDL `unsigned long DeltaMode` 是 `Number`，不是 `BigInt` |
| `DragEventArgs`、`DataTransfer` | Razor SG 可绑定 drag handler；`DataTransfer` 保持原生 browser carrier，首批只读字段直接读取 | `Jazor.CLR.Generator` 已生成 skeleton，`DragEventArgsModule`/`DataTransferModule`/doc 已复制并完善；files/items、setter 与构造器保持 `Op.Discard` | **Support（browser interactive）**：Blazor `EventHandlers` reference metadata、official SG/Deno、真实 BrowserSmoke 与 isolated Release package consumer 已通过；不承诺 `DataTransferItem`/File payload |
| `ClipboardEventArgs` | Razor SG 可绑定 clipboard handler；`Type` 直接读取原生 `ClipboardEvent` | `Jazor.CLR.Generator` 已生成 skeleton，`ClipboardEventArgsModule`/doc 已复制并完善；payload/权限 API、setter 与构造器保持 `Op.Discard` | **Support（browser interactive）**：Blazor `EventHandlers` reference metadata、official SG/Deno、真实 BrowserSmoke 与 isolated Release package consumer 已通过；clipboard payload/permission APIs 仍不在声明内 |
| `TouchEventArgs`、`TouchPoint` | Razor SG 可绑定 touch handler；TouchList 在属性访问时惰性 `Array.from(...)` 转成数组 carrier | `Jazor.CLR.Generator` 已生成 skeleton，`TouchEventArgsModule`/`TouchPointModule`/doc 已复制并完善；setter 与构造器保持 `Op.Discard` | **Support（browser interactive）**：Blazor `EventHandlers` reference metadata、official SG/Deno、真实 BrowserSmoke 与 isolated Release package consumer 已通过；不承诺 TouchList 非 getter 操作 |
| `ErrorEventArgs` | Razor SG 可绑定 error handler；公开 getter 直接读取原生 `ErrorEvent` | `Jazor.CLR.Generator` 已生成 skeleton，`ErrorEventArgsModule`/doc 已复制并完善；setter 与构造器保持 `Op.Discard` | **Support（browser interactive）**：Blazor `EventHandlers` reference metadata、official SG/Deno、真实 BrowserSmoke 与 isolated Release package consumer 已通过 |
| `ProgressEventArgs` | Razor SG 可绑定 progress handler；公开 getter 直接读取原生 `ProgressEvent` | `Jazor.CLR.Generator` 已生成 skeleton，`ProgressEventArgsModule`/doc 已复制并完善；setter 与构造器保持 `Op.Discard` | **Support（browser interactive）**：Blazor `EventHandlers` reference metadata、official SG/Deno、真实 BrowserSmoke 与 isolated Release package consumer 已通过 |
...
`RazorVueM5CapabilityLedger` 的 route/navigation rows 现已具备 reference、BrowserSmoke 和 PackageConsumer 证据：应用自有 route host 的同一 base URI 内部 `push/replace`、`HistoryEntryState`、`LocationChanged` 和 history length 子集由 `P0-route-layout-page-state`/`P1-navigation-router` 记录；独立的 `P1-blazor-clr-navigation-location-changing` row 继续记录同源内部 `NavigateTo` 的取消、supersede 和 registration dispose。三者都不是整个 routing family 的 Support 声明：popstate/hashchange cancellation、server circuit 和 SSR/prerender route identity 仍由各自的 ledger/host 边界裁决；已发布版本章节不因当前开发证据回写。
...
Jazor.CLR.Generator
  ...
  -> Jazor.Compiler.Generator 生成 WhiteList，并生成 ECMAScript 的 manifest.json + dist runtime resource
...
| `Jazor.RazorVue` | Vue listener 的原生 event 传递、`@ref` 生命周期、Vue `provide`/`inject`、组件/路由/表单 framing，以及对已由 `Jazor.CLR` 放行 symbol 的产品级 translation hook | 持有 `[Jazor]`/`Op.*` CLR member mapping 或成为 whitelist source-root；为每种 `EventArgs` 手工构造 payload；用手拼 JS 替代 C# 成员/函数语义 lowering；把导航、认证、表单状态机新增到既有 hand-written runtime `.mjs` |
| `ECMAScript` resource | 由 generator 从 `Jazor.CLR` module source 生成 `manifest.json + dist/System/**`，并声明 module/dependency/hash | 因拆分 Blazor binding 而迁移、复制或另建第二份 CLR resource |
| `Jazor.Emit` | 读取 `ECMAScript` resource 和纯 Jazor `ModuleCatalog`，按真实 import closure 物化产物 | 在 RazorVue 中直接写入文件、重新解释 module 语义或绕过统一收集流程 |
...
5. 运行 `Jazor.Compiler.Generator`，由 `Jazor.CLR` module source 统一生成 `WhiteList.cs.Generate.cs` 和 `ECMAScript` 的 `manifest.json + dist`。Emit 读取该资源以及纯 Jazor 的 `ModuleCatalog`，不读取退役 catalog 载体。
...
4. 已删除 `Jazor.RazorVue/RazorSdk/Catalog` whitelist source-root 和旧 mapping 声明，metadata/key 测试已迁到 `Jazor.CLR.Test`；RazorVue SG 测试只验证 official SG 输入、hook emission、生命周期和失败路径。
...
| --- | --- | --- |
| `Jazor` | 框架无关 compiler、Emit、基础 contract/runtime，以及由 `Jazor.CLR` module 生成的静态 mapping 和 ECMAScript resource | 不包含 `ECMAScript.Blazor` DLL 或 ASP.NET Core framework reference；未使用的 CLR module 由 closure DCE 排除 |
| `Jazor.CLR` | Blazor/BCL CLR type module、`[Jazor]` mapping、carrier 与 runtime helper 的唯一源码 owner | module 源进入既有 generator；不因 Blazor 支持另建资源清单或 provider |
| `ECMAScript` | 标准 ECMAScript host 类型，以及 `manifest.json + dist` runtime resource | runtime resource 由 generator 从 `Jazor.CLR` 更新；不生成纯 Jazor `ModuleCatalog` |
...
4. 新增支持类型时必须先运行 `Jazor.CLR.Generator` 生成并核对 module/doc；复制并完善 `Jazor.CLR` 源后，再运行 `Jazor.Compiler.Generator`，同时提交 `WhiteList.cs.Generate.cs` 与 `ECMAScript/manifest.json + dist` 资源生成结果。
...
7. Blazor 专属 mapping 与 runtime 行为从第一版就共同位于生成后完善的 `Jazor.CLR` module，并生成 `ECMAScript` 的 `manifest.json + dist` resource，由统一 Emit 管道按闭包物化；`ECMAScript.Blazor` 与 `Jazor.RazorVue` 都不持有对应 member key。不得新增 hand-written `.mjs`。现有 RazorVue `.mjs` 只保留 Vue 生命周期、渲染 framing 和到 `Jazor.CLR` 模块入口的薄转发，不承载新增状态机或成员语义。
...
让组件可使用标准 `NavigationManager.RegisterLocationChangingHandler(Func<LocationChangingContext, ValueTask>)` 阻止或观察内部导航，并得到与浏览器 history 交互一致的注销和异步行为。当前已证明的支持面是同一 base URI 的 `NavigateTo` 内部路径：`PreventNavigation`、异步 supersede/cancellation、query/hash/history state 和 registration dispose 均保持可观察语义。`popstate`/`hashchange` 目前只触发 `LocationChanged`，不执行可取消的 handler；该边界以及 SSR/prerender、server circuit identity 不属于本切片。
...
6. 同源内部 `NavigateTo` 的 browser、Release package、reference、module 和 Deno 行为一致后，将该声明子集标为 **Support**；不适用的 SSR/prerender 不作为该 browser-interactive 子集的隐含证据，明确不可保真的成员仍保持 `Op.Discard`，不因切片升级而放行。
...
- Razor SG/browser：组件注册 handler 后完成允许导航、阻止同一 base URI 的内部导航、组件卸载和快速连续导航；`RazorSgBlazorReferenceOracleTests.BlazorReferenceNavigationManager_SupersedesPendingLocationChangingDispatch` 与 `SdkIntegrationTests.Build_LocalReleasePackages_WithExternalNavigationLocationChangingRazorConsumer_ProvesInternalCancellationInRealBrowser` 已覆盖 reference、official SG、Deno、真实 HTTP-origin browser、query/hash/history state、supersede cancellation 和 isolated Release package。back/forward、hash 变化和无法取消的 browser event 仍需单独裁决 replay/restore 或 Guidance。
...
6. 在 official Razor SG/Deno fixture 覆盖 method group、lambda、async handler、原样转发 native event 的组件 `EventCallback<T>`、`preventDefault`、`stopPropagation`、`@bind` 与 typed `@onchange` 共存；`RazorSgOfficialCoreDomEventRuntimeTests` 另以 Blazor `EventHandlers`/`ChangeEventArgsReader` 作为 reference metadata/value-shaping oracle。`SdkIntegrationTests.Build_LocalReleasePackages_WithExternalCoreDomEventsRazorConsumer_HandlesNativeEventsInRealBrowser` 通过 isolated Release NuGet consumer、materialized `ChangeEventArgsModule.js`、bundle source map 与真实 browser DOM dispatch 覆盖 mouse/keyboard/focus/input/change/checkbox/multiple-select，并证明 async continuation 读取到的是触发时的 `ChangeEventArgs.Value`，而不是之后修改的 DOM value。合成 `new EventArgs` 路径必须稳定失败；capture 调用插入后，source map 仍须指向作者 handler，而不是 CLR helper 或 listener bridge 内部。
...
| `ElementReferenceExtensions.FocusAsync(ElementReference)` | 由短的 generated CLR `Import` helper 调用 `HTMLElement.Focus()`，并返回已完成的 `ValueTask`/Promise carrier；default/unmounted ref 保留 framework failure | 仅处理由 `@ref` 捕获的真实 DOM element；不伪造 server renderer 身份。 |
| `ElementReferenceExtensions.FocusAsync(ElementReference, bool preventScroll)` | 由同一 generated CLR `Import` helper 以 `{ preventScroll }` 调用 `HTMLElement.Focus(FocusOptions)` | 不通过宽松 options `object` 替代标准 bool overload；只转发标准 bool overload 所拥有的字段。 |

`ElementReference` 与 framework 自带的 `ElementReferenceExtensions` 已加入 `Jazor.CLR.Generator`，分别生成并复制对应 module/doc。两个 `FocusAsync` overload 在 `Jazor.CLR` module 中走 generated CLR `Import` helper：它在调用前统一裁决 default/unmounted ref 的 framework failure，成功后调用原生 `HTMLElement.Focus` 并返回完成的 `ValueTask`/Promise carrier；bool overload 只转发 `preventScroll`。DOM node 生命周期和 Vue ref framing 仍由 RazorVue 处理。`scrollIntoView`、selection、measurement 等非标准 `ElementReference` API 应走已有或新增的强类型 WebIDL binding，不应借此把任意 DOM 方法塞进 CLR 模块。

当前已证明两个类型来自模块生成器、module/doc 已进入 `Jazor.CLR`，旧 `ECMAScript.Blazor` `[Jazor]` mapping 已删除；official SG/Deno 与真实 browser/isolated Release consumer 已覆盖同一元素重新渲染、条件卸载、empty/unmounted ref、`OnAfterRenderAsync` 调用时机、`preventScroll`、Import helper 的 `ValueTask` emission 与资源 closure。该 Support 声明仅适用于 browser interactive；SSR/prerender 不在本切片声明内。
...
| S1 | 导航拦截：`ValueTask` + `LocationChangingContext` + 注册句柄 | 已落地的 `NavigationManager` 基础 runtime、`ValueTask`/cancellation 最小 carrier；reference oracle、official SG/Deno、真实 HTTP-origin browser 与 isolated Release package consumer 已通过 | **Support（browser interactive 内部导航子集）**；popstate/hashchange cancellation、SSR/prerender 与 server circuit 不声明 |
...
| S4 | Pointer/Wheel/Drag/Clipboard/Touch/Error/Progress 事件组 | 七组类型均已进入 generator，并由复制到 `Jazor.CLR` 的 modules/docs 完成 native carrier getter；Touch 集合只在属性访问时 `Array.from(...)` | 各组均为 **Support（browser interactive）** 的强类型 getter-only handler；Blazor `EventHandlers` reference metadata、official SG/Deno、真实 BrowserSmoke 与 isolated Release package consumer 已通过，SSR/prerender 不声明 |
...
| 切片 | 当前状态 | 稳定证据 | 缺口/裁决 |
| --- | --- | --- | --- |
| S0 | **InProof（完整 framework mapping profile；静态包边界已 Support）** | `Jazor.CLR.Generator` 已覆盖 `NavigationManager`、navigation event/context、共用 async/cancellation 类型、RazorVue primitives、DOM EventArgs 与 ElementReference；`BlazorClrGeneratorOutputTests`、`BlazorClrWhitelistTests`、`RazorVueCatalogOwnershipTests`、`RazorVueM5CapabilityLedgerTests.Ledger_BlazorClrSlicesDeclareAuditableContractMetadata` 与 `SdkIntegrationTests.Build_LocalReleasePackages_CoreAndVueConsumers_RespectBlazorClrPackageBoundary` 固定生成输出、唯一 owner、key/Op、Release package 边界和核心/Vue 消费者资产分离。M5 的 `P0-blazor-clr-mapping-package` 只对已证明的生成/whitelist/静态 Release 包边界声明 `Support`，不等价于所有 framework API 的运行时 Support | 完整 framework mapping profile 仍缺标准 reference oracle、完整 BrowserSmoke 与各能力切片的 isolated package consumer 证据；不新增 per-compilation mapping-contribution contract |
| S1 | **Support（browser interactive 内部导航子集）** | CLR metadata: `NavigationManagerClrWhitelistTests`; CLR/compiler/runtime: `RazorSgNavigationRuntimeTests`（覆盖 URI/history、push/replace、`HistoryEntryState`、`LocationChanged`/`OnNotFound`、prevent/dispose/supersede）；reference: `RazorSgBlazorReferenceOracleTests.BlazorReferenceNavigationManager_SupersedesPendingLocationChangingDispatch`; module: `NavigationManagerModule`、`ValueTaskModule`、cancellation modules；official SG/Deno 与真实 HTTP-origin browser + isolated Release package: `samples/RazorVue.Authoring/verify-smoke.cs`（PathBase replace/history length/LocationChanged）与 `SdkIntegrationTests.Build_LocalReleasePackages_WithExternalNavigationLocationChangingRazorConsumer_ProvesInternalCancellationInRealBrowser` | 同源内部 `NavigateTo` 已覆盖 push/replace、HistoryEntryState、LocationChanged、handler、PreventNavigation、query/hash/history state、异步 supersede/cancellation 和 registration dispose；popstate/hashchange cancellation、server circuit、SSR/prerender route identity 仍不声明 |
| S2 | **Support（browser interactive；ownership 已收敛）** | M5 `P0-bind-events`、`BlazorClrMappingTests`、`BlazorClrWhitelistTests`、`Jazor.CLR.ChangeEventArgsModule`、RenderEmitter typed `onchange` wrapper、`RazorSgOfficialCoreDomEventRuntimeTests` 的 Blazor `EventHandlers`/`ChangeEventArgsReader` oracle + official SG/Deno fixtures，以及 `SdkIntegrationTests.Build_LocalReleasePackages_WithExternalCoreDomEventsRazorConsumer_HandlesNativeEventsInRealBrowser` 已证明 native getter emission、method-group/lambda typed callbacks、native event forwarding、direct `@bind` coexistence、string/bool/multiple-select shaping、event-time async capture、Release resource closure、bundle source-map anchoring 与真实 DOM dispatch | 仅 constructor/setter/identity、file input、synthetic payload 与 SSR/prerender 继续 Reject/不声明 |
| S3 | **Support（browser interactive）** | `BlazorClrMappingTests`、`BlazorClrWhitelistTests`、`RazorSgOfficialReferenceAuthoringTests.BuildComponent_OfficialRazorElementReferenceFocus_UsesDomCarrierMapping`、`RazorSgOfficialReferenceAuthoringTests.BuildComponent_OfficialRazorElementReferenceFocus_PreservesMountAndUnmountFailureContractOnDenoHost` 与 `SdkIntegrationTests.Build_LocalReleasePackages_WithExternalElementReferenceRazorConsumer_FocusesAndHandlesUnmountInRealBrowser` 已证明 `ElementReference -> HTMLElement`、两个 `FocusAsync` overload、`@ref` mount/update/unmount、empty/unmounted failure、`preventScroll` 与 Release resource closure；对应 modules/docs 已由 generator skeleton 完善 | 不声明 SSR/prerender、server renderer identity 或任意 DOM methods |
| S4 | **Support（browser interactive；全部七组）** | `PointerEventArgsModule`、`WheelEventArgsModule`、`DragEventArgsModule`/`DataTransferModule`、`ClipboardEventArgsModule`、`TouchEventArgsModule`/`TouchPointModule`、`ErrorEventArgsModule`、`ProgressEventArgsModule` 均由 `Jazor.CLR.Generator` 生成 skeleton 并复制到 `Jazor.CLR`；`BlazorClrWhitelistTests`、`BlazorClrGeneratorOutputTests`、`BlazorClrMappingTests` 与 `RazorSgOfficialExtendedDomEventRuntimeTests` 已证明 reference `EventHandlers` mapping、alias/getter、Number/BigInt carrier、native handler 与 Deno 行为，`SdkIntegrationTests.Build_LocalReleasePackages_WithExternalExtendedDomEventsRazorConsumer_HandlesNativeEventsInRealBrowser` 已证明真实浏览器与 isolated Release package closure | SSR/prerender 不声明；setter/constructor、synthetic payload、DataTransfer files/items、TouchList 非 getter 操作继续 Reject |
| S5 | **Guidance** | M5 row `P2-authentication` 保持 authored `JAZORVCA007`；没有默认 browser provider，也未选择 generator type slice | 必须先解决 host descriptor、refresh、claims carrier 和 endpoint authorization 分离，再生成获准 modules；当前使用 typed endpoint/provider registration，内置认证 UI 不在范围 |
...
6. **交付**：至少确认 debug/release artifact；涉及 runtime import 的切片还要确认 isolated package consumer 的 closure。仅引用 `Jazor` 的 consumer 不应出现 `ECMAScript.Blazor` 资产或 ASP.NET Core framework reference，但 `Jazor.CLR` 生成的静态 mapping 和 `ECMAScript` manifest/dist resource 仍按核心包既有方式提供且未使用 module 不得物化；引用 `Jazor` + `Jazor.Vue` 的 consumer 应获得 `ECMAScript.Blazor` 普通 authoring reference、RazorVue analyzer/reference 资产和按闭包选取的 CLR resource，无需 mapping contribution 注册、源码复制或第二 runtime carrier。支持 SSR/hydration 时另行覆盖一次性副作用与状态 handoff。
...
完成某个切片后，更新 [当前状态](./current-status.md)、作者指南和 CHANGELOG 的面向用户行为描述；已发布版本章节不回写。生成器输入、`Jazor.CLR` module/doc、生成 whitelist、`ECMAScript` manifest/dist resource、RazorVue framing 和测试必须同一提交评审，避免出现“文档称支持但类型未经过生成器”“白名单已放行但没有浏览器 carrier”或“旧 `ECMAScript.Blazor`/`Jazor.RazorVue` mapping 仍形成双 owner”的灰区。
