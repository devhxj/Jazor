...
> P2 推进由强类型 C# 契约、现有 Razor Source Generator 和最终 Vue 产物共同证明的能力。证据闭环前的条目使用 Guidance/Reject 状态。

## 目标与范围

P2 的目标是完善 P1 之后的协议、渲染和生态能力，同时保持 RazorVue 作为支持 Razor/Blazor 语法的 JSX-like 范式。
...
| P2-D 性能与交付质量 | `Jazor.Compiler`、`Jazor.Emit`、测试脚本 | 以固定 benchmark 为依据，实施保持语义、导入稳定性和 source map 的优化 |
...
1. C# public surface 保持强类型，并能通过官方 Razor SG；运行时语义通过明确参数、union、overload 与 binding 表达。
2. compiler/lowering、runtime/binding 和 diagnostic 各有针对性回归；相邻形状在使用点具有稳定诊断。
3. 有 Deno/runtime 证据和真实浏览器交互证据；涉及发布时必须增加隔离 Release package consumer。
4. 涉及 SSR 或 hydration 时，证明 envelope 所有权、错误传播、重复执行和资源闭包；协议以版本化 bootstrap envelope 描述。
...
- 为表单场景提供显式 typed endpoint/bootstrap 示例；状态、表单与防伪通过应用 endpoint 协议处理。

完成后，“显式 bootstrap DTO + envelope”进入 Support with constraints；内置表单协议使用 Guidance/Reject。

### P2-B：高级渲染评估

Microsoft Blazor 内置 UI 组件通过 `JAZORVGA021` 进入明确诊断路径：
...
真实浏览器和 Release consumer 证据形成公共 adapter 的验收条件；证据收集阶段更新 ledger 与指导文档。

### P2-C：JS 互操作边界

- `IJSRuntime`、`IJSObjectReference`、`JSInvokable` 使用稳定 Reject 状态。
- 确保诊断包含源位置、使用点和 typed ECMAScript/WebIDL 替代建议。
- 对常见使用形状补充 compiler/analyzer 回归；运行时行为通过 typed binding 进入模块。

### P2-D：性能与交付质量

- 使用 `benchmark-razorvue-g2.cs` 和 `benchmark-razorvue-build.cs` 的固定参数记录 clean/incremental/HMR/Release、render/update 和 gzip 基线。
- 优化前后比较相同输入、样本数和 SDK；主链路优化以可复现收益、source map 和导入稳定性为验收条件。
...
本轮 goal 以以下可执行验收为完成条件；`Reject` 表示已确定的产品边界。验收证据定义 P2 完成状态。
...
| localization | Guidance：使用应用自有 typed resource/locale contract 或 ECMAScript Intl；request-culture middleware、IStringLocalizer 注入和 SSR locale handoff 由应用宿主协议提供 |
...
SSR 状态输入边界由 `Jazor.EmitTest.JazorSsrHostingTests` 锁定：空 provider key、空白 provider key、重复 provider key 和认证保留 key 冲突均在启动 Deno worker 前报告失败；runner 保持完整产物状态。
