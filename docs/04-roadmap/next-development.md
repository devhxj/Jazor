...
| 高级渲染 | Microsoft Blazor 内置 UI 组件统一 Reject；StreamRendering、localization 和复杂 validation 单独评估。 | 当前 SDK 内置 UI 组件（含 `ImportMap`、`AntiforgeryToken`、`FormMappingScope`、`Virtualize`、`QuickGrid`、Section）已由 `JAZORVGA021` Reject；其余语义保持 Guidance |
