...
> 本页说明下一轮投入的优先次序，而不是新增 `Support` 声明。能力是否可用，始终以[当前状态](./current-status.md)、源码和可复现验证为准。

## 投入准则

下一阶段不重开编译产物、类库 carrier 或 Razor-to-Vue 主链路的设计。投入从高频页面和真实消费者出发：先让既有能力更自然、更易诊断、更容易在发布环境中证明；只有 C# 类型系统无法表达，或浏览器语义确有缺口时，才新增 runtime 协议。
...
这些方向具有明确价值，但会触及状态所有权、生命周期或 browser history 语义。在协议、失败矩阵和真实消费者证据完整之前，它们保持 `Guidance` 或 `Reject`；局部实现不能被提前宣传为 `Support`。
