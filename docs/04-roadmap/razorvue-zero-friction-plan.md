...
2. **Binding/API gap**：TDesign 泛型/非泛型组件、typed slot、EventCallback、`@bind`、union、
   required parameter 的自然 Razor 写法还没有经过一套独立矩阵；JazorAdmin 中的 callback
   转发、cast 和
...
| 领域 | 当前状态 | 已有证据 | 边界/未纳入声明 |
| --- | --- | --- | --- |
| 普通 markup、泛型、fragment/slot、control flow、bind/event、参数 lifecycle | `Support` | official SG、module、Deno 回归；JazorAdmin 消费者 | 仍需保持独立自然写法矩阵，避免应用侧 callback/cast 掩盖回归 |
| 复杂 async lifecycle（rejection、cancellation、重复 render、async disposal race；浏览器与首屏 SSR 子集） | `Support`（Compatibility Adapter） | `RazorSgOfficialComplexLifecycleRuntimeTests`：official SG、module artifact、Deno；isolated Release package consumer + real browser smoke；Windows SSR Release consumer 已证明初始 `OnInitializedAsync` 完成后才序列化 HTML 并在 hydration 后保留状态；`RazorSgBlazorReferenceOracleTests` 作为 framework primitive 对照 | 完整 SSR/prerender lifecycle identity、SSR rejection/cancellation 与 hydration 副作用 parity 明确排除 |
| `@page`/`@layout`/route metadata 与页面状态 | `Support`（Compatibility Adapter） | route catalog、official SG/module、Deno、`samples/RazorVue.Authoring` isolated Release package 与真实浏览器 journey；覆盖内部 push/replace、`HistoryEntryState`、`LocationChanged` 和 query/route 刷新 | Microsoft Router/RouteView/LayoutView/NavLink、SSR/prerender route identity 与更宽的 server routing 明确排除 |
| `ParameterView`/`SetParametersAsync`（sparse/default/slot/queue 与首屏 SSR hydration） | `Support`（Compatibility Adapter） | sparse/alias/slot/queue adapter 测试；isolated Release package/browser consumer 已覆盖参数替换与 lifecycle ordering；Windows SSR Release consumer 已证明 serialized props 在 server HTML/hydration 中经 `SetParametersAsync` 应用并等待初始 async task | 完整 snapshot/reference parity、取消深度、作者 SSR 异常传播明确排除；`TryGetValue`/枚举/`ToDictionary` 继续 Guidance |
| browser `[Inject]` property adapter（writable property 与 serialized provider handoff） | `Support`（Compatibility Adapter） | provider、lifecycle、missing-provider 测试；isolated Release package/browser consumer 已覆盖嵌套与 recreated component activation；SSR runner 与 hydration document 通过同一 serialized application provider 传递并由 TodoList Release consumer 验证 | provider lifetime/reference parity、SSR 更新/副作用语义明确排除；constructor injection 保持 Reject |
| cascading values（typed/named/nested browser updates 与首屏 SSR cascade） | `Support`（Compatibility Adapter） | typed/named/nested Deno 测试；isolated Release package/browser consumer 已覆盖 nested override、`IsFixed`、same-value、dispose 与 update propagation；Windows SSR Release consumer 已证明 named cascade 在 server HTML 与 hydration 中传递到子组件 | 完整 reference parity、SSR 更新传播与 hydration 副作用明确排除 |
| `NavigationManager` 与 LocationChanging | `Support`（Compatibility Adapter，内部导航子集） | URI/navigate 基础映射与 CLR whitelist；reference oracle、official SG、Deno、真实 HTTP-origin browser、Release consumer；Authoring PathBase smoke 证明 replace 后 URI、`HistoryEntryState`、`history.length` 不变以及 `LocationChanged` 实际 URI/非零计数 | 仅同一 base URI 的内部 `NavigateTo` 承诺 handler、取消、query/hash/history state 和 dispose；popstate/hashchange cancellation、SSR/prerender 与 server circuit 不承诺 |
...
| --- | --- | --- | --- | --- |
| ZF-P0-01 | TDesign 页面曾需要 sample-local 输入/表单薄桥接，作者被迫接触 callback 转发和 cast。 | Binding/API；`ECMAScript.TDesign` + `Jazor.RazorVue` | 已完成 direct typed TDesign authoring；公共 prop/event contract 和 direct render 路径已收敛，JazorAdmin 已删除该组 sample-local bridge。 | natural-authoring fixture 无手写 builder/cast 完成 CRUD 交互；Release browser smoke 与 isolated package consumer 通过。目标：`Support`（完成）。 |
...
| ID | 用户摩擦 / 当前状态 | 类型与 owner | 实施方向和依赖 | 验收与退出状态 |
| --- | --- | --- | --- | --- |
| ZF-P1-01 | `SetParametersAsync(ParameterView)` 已完成 sparse/default/slot/queue 与首屏 SSR hydration 子集。 | Compiler/runtime；ParameterView adapter | 固定 CLR default -> base apply -> overlay -> lifecycle -> queued update 的顺序；只实现已定义的成员，`TryGetValue`/枚举/`ToDictionary` 继续 Guidance。 | `RazorSgSetParametersAsyncRuntimeTests` 与 isolated Release package/browser consumer 已覆盖生命周期顺序、参数替换和队列；`verify-windows-ssr-release.cs` 已证明 serialized props、首次 async 参数任务、server HTML 与 hydration 后交互；完整 snapshot/reference parity、取消深度和作者 SSR 异常明确排除；`Support`。 |
| ZF-P1-02 | `[Inject]` browser service 已完成 writable property activation 与 serialized provider handoff。 | Runtime/host；`VueInjectRegistry` + `VueModuleBuilder` | SSR 请求携带 JSON provider envelope，runner 与 hydration 使用同一 application provider；constructor injection、primary constructor、`this(...)`、`base(args)` 保持 `Reject`。 | official SG + Deno + isolated Release package/browser 已覆盖嵌套与 recreated component；`JazorSsrHostingTests` 与 Windows SSR Release consumer 已证明 server provider activation 及 hydration provider handoff；provider lifetime/reference parity 与 SSR 更新/副作用明确排除；`Support`。 |
| ZF-P1-03 | cascading 参数已完成 typed/named/nested browser 更新与首屏 SSR cascade 子集。 | Runtime；cascading adapter | 以 Vue scope 建立 typed/named provider；一次更新只触发必要 consumer；不可写属性继续 `JAZORVCA008`。 | `RazorSgCascadingValueRuntimeTests` 与 isolated Release package/browser 已覆盖 nested override、replacement、same-value、`IsFixed`、dispose 与 update propagation；`verify-windows-ssr-release.cs` 已证明 named cascade 在 server HTML/hydration 中到达子组件；完整 reference parity、SSR 更新传播与 hydration 副作用明确排除；`Support`。 |
