...
| `@page`/`@layout`/route metadata 与页面状态 | `Support`（Compatibility Adapter） | route catalog、official SG/module、Deno、`samples/RazorVue.Authoring` isolated Release package 与真实浏览器 journey | Microsoft Router/RouteView/LayoutView/NavLink、SSR/prerender route identity 与更宽的 server routing 明确排除 |
