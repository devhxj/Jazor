...
> 本计划定义 Jazor 在现有 Vue、Vue Router、Pinia、UI、图表和图标绑定之上的下一批应用级生态绑定。每个条目以标准 npm/JSR 入口、强类型契约和真实消费者证据完成交付。

## 目标

为 RazorVue 应用提供一组可组合的、强类型的 Vue 应用基础库绑定，覆盖响应式工具、表单、国际化、日期、拖拽、编辑器、文件上传、服务端状态和浮动定位。

每个交付物都是独立的 `ECMAScript.<Name>` binding，拥有自己的 C# contract、npm/JSR identity、许可证、README、版本快照和测试项目。运行时模块与样式由上游 package 提供，编译器保持通用 lowering 语义。
...
Monaco 与 WangEditor 是两个独立包。Monaco 直接绑定核心 `monaco-editor`；WangEditor 使用官方 Vue 适配器，并在根 `package.json` 声明编辑器核心依赖。Floating UI 以 `@floating-ui/vue` 为作者入口，DOM 层函数按实际 authoring contract 补充。
...
- 每个库按需导入公开 ESM entry；`SemanticWalker` 负责表达式和成员翻译，`Jazor.Emit` 生成标准项目，NetPack 负责 bundle 与资源图。
...
组件类绑定的策略见[npm/JSR 包绑定指南](../03-guides/js-resource-binding.md#组件绑定)：组件数少时使用手写代理与 descriptor，组件数多且上游有机器可读 metadata 时生成代理与导出目录。两种形式都映射到上游公开 ESM specifier。
...
退出条件：三个包可独立 pack，标准 package identity 与 ESM 入口可解析，compiler/SG 测试通过，至少一个真实 RazorVue 页面和浏览器 smoke 使用每个包。
...
3. 上游 runtime 的锁定版本、`manifest.json` schema 2、npm/JSR identity、公开入口、许可证和 `README.md`。
4. 生成或手工维护的 export/prop/event/slot inventory、文档来源和 contract fingerprint。
5. 对应 `src/ECMAScript.<Name>.Test`：metadata、compiler emission、Razor SG、标准项目/NetPack 输出和适用的 browser smoke。
...
- Debug/Release/SSR（适用时）使用同一 `jazor/` 项目、`node_modules` 与 frozen lock；
