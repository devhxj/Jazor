...
| 绑定漂移 | Vuetify、Element Plus、TDesign 已有锁定快照、生成检查和 coverage；原始注释来源已在各包记录。`verify-vue-binding-contracts.cs` 统一执行生成检查并校验版本/文档/manifest，同时输出组件/export/prop/event/slot inventory 计数与 fingerprint。 | 后续把 inventory fingerprint 扩展为逐项 added/removed/changed diff，并在上游升级时接入发布阻断。 |
