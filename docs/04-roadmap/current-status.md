...
应用自有 RazorVue route host 的 `@page`、`@layout`、typed route/query 参数、not-found、query refresh 和 browser history 已达到 `Support`（Compatibility Adapter）：official Razor SG、module/source map、Deno 与 `samples/RazorVue.Authoring` 的 isolated Release package browser journey 均已覆盖。该声明不引入 Microsoft `Router`/`RouteView`/`LayoutView`/`NavLink` 标签，不包含 `LocationChanging` cancellation，也不声明 SSR/prerender route identity。
