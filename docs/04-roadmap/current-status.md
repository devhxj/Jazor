...
| P0：Debug/HMR/SPA/SSR 交付 | 2026-09-12 `v0.60.0` 发布候选已全阶段通过：API、质量门禁、RazorVue diagnostics、隔离 NuGet 包、Release bundle、`/docs` PathBase、Chrome SPA 与 `/todo` SSR consumer 均成功；归档证据见 `artifacts/rc-v060/report.md`，长期 `wiki-verify` 与 `quality-gates` 额外上传 `toolchain-matrix.md`，关联实际 SDK、Node、Chrome、操作系统和 commit。 |
...
| P0：组件绑定统一门禁 | `dotnet run --file scripts/csharp/verify-vue-binding-contracts.cs` 依次执行 Element Plus、Vuetify、TDesign 的生成检查，并验证三套 manifest 与上游快照版本、README 原始注释来源保持一致；`--report` 生成 schema `1.0` JSON/Markdown 审阅证据，包含组件/export/prop/event/slot inventory 计数与 fingerprint，Quality Gates 与 release candidate 均归档该报告。 |
