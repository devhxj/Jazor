...
| P2：协议边界与评估 | `jazor-ssr-state` v1 现在拒绝重复 provider key；`IJSRuntime` 家族保持 `JAZORVGA022` Reject 并有作者面回归；所有当前 SDK Microsoft Blazor 内置 UI 组件统一由 `JAZORVGA021` Reject（包括 `ImportMap`、`AntiforgeryToken`、`FormMappingScope`、`Virtualize`、`QuickGrid`、Section）；StreamRendering、localization 与复杂 validation 仍是独立 Guidance；运行时 benchmark 基线已记录，未宣称优化收益。 | [P2 执行计划](./p2-plan.md) |
