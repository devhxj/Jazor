...
RazorVue 的生产输出是 direct Vue render-function `.mjs`。当前已接受的 static hoist、精确 static VNode cardinality、按需 raw-markup runtime、child-level block tree（dynamic string text、mixed text 与 nested block）、保守 patch flags、setup-instance handler cache，以及已证明安全的 `foreach` `renderList` / keyed-unkeyed fragment lowering 边界见 [RazorVue Direct Render 性能评审](./razorvue-direct-h-performance.md)。production Vue gate 已确认 keyed list reorder 保留原 DOM identity，unkeyed list 不会误标为 stable；后续 slot、SSR 与交付性能工作遵循 [RazorVue 极致性能路线图](./razorvue-extreme-performance.md) 的依赖和真实 runtime 门禁。
