...
| P1：中型应用基线 | `scripts/csharp/benchmark-razorvue-g2.cs` 已建立 plain-text、counter、keyed-list-100、static-vnode 的 render/update 吞吐和 gzip 基线；`benchmark-razorvue-build.cs` 另记录 clean/incremental/HMR/Release 耗时，且对 clean/incremental 记录生成模块/source map 数量、原始与 gzip 体积及增量产物变化；后续优化必须使用同一参数重测。 |
