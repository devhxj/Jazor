...
截至 2026-09-12，以下范式工作已经有实现、回归和可重复入口：
...
| P0：Debug/HMR/SPA/SSR 交付 | 2026-09-12 当前 HEAD `47436388`（运行时代码验收提交 `02b164e4`）的 Release Candidate 全阶段通过：`verify-release-candidate.cs -- --tag v0.50.0 --output-directory artifacts/rc-final`，其中 SPA consumer 使用隔离 NuGet 包、Release bundle、`/docs` PathBase 与 Chrome 浏览器旅程，SSR consumer 使用 `/todo`；[范式证据矩阵](../02-architecture/razorvue-paradigm.md#p0p1-验收证据入口) 固定复现命令。长期 `wiki-verify` 与 `quality-gates` 工作流额外上传 `toolchain-matrix.md`，关联实际 SDK、Node、Chrome、操作系统和 commit。 |
