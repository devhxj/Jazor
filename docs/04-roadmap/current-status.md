...
`2026-08-31` 的 JazorAdmin Release 本地 NuGet consumer gate 已通过：native 与 VueInject 两个 consumer 均完成 0 warning/0 error 构建，最终 browser mount smoke 通过；同时 `vue-data-ui` selected entry 的相对 ESM closure 已完整物化，Emit 资源回归 185/185 通过。这确认了规范化后的包与资源交付链路，但不替代逐项 Blazor framework capability 的语义证据；复杂页面的 bridge 摩擦和明确排除/Guidance 的边界仍保留。
