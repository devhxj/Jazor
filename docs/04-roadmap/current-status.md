...
| 切片 | 当前证据 |
| --- | --- |
| P0：失败诊断与生成稳定性 | RazorVue SG 测试 `4982/4982` 通过（由 `dotnet test src/Jazor.RazorVue.Sg.Test/Jazor.RazorVue.Sg.Test.csproj` 当前运行确认）；诊断路径排序具有 Ordinal tie-breaker；生成失败不留下 partial artifact。RazorVue.Authoring 还通过了 typed `TPrimaryTableColCell<TaskRow>`、typed `OnRowClick` 及 package consumer smoke。 |
