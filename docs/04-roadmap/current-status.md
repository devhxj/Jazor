...
RazorVue 的生产输出是 direct Vue render-function `.mjs`。当前已接受的 static hoist、精确 static VNode cardinality、按需 raw-markup runtime、保守 block tree/patch flags 与 setup-instance handler cache 边界见 [RazorVue Direct Render 性能评审](./razorvue-direct-h-performance.md)；后续 child-level block tree、list/slot、SSR 与交付性能工作遵循 [RazorVue 极致性能路线图](./razorvue-extreme-performance.md) 的依赖和真实 runtime 门禁。
