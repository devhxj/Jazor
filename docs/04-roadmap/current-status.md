...
| P2：协议边界与评估 | `jazor-ssr-state` v1 现在拒绝重复 provider key；`IJSRuntime` 家族保持 `JAZORVGA022` Reject 并有作者面回归；`Virtualize`、`QuickGrid`、Section、StreamRendering、localization 与复杂 validation 继续 Guidance，等待 typed contract、浏览器语义和 Release consumer 证据；运行时 benchmark 基线已记录，未宣称优化收益。 | [P2 执行计划](./p2-plan.md) |
