...
| P1：范式级调试 | `dotnet run --file scripts/csharp/inspect-razorvue-chain.cs -- --source ... --generated ... --artifact ... --map ... --json` 输出 source → generated → module → map 链路并在映射断裂时失败；2026-09-11 使用 `RazorVue.TodoList/TodoApp.razor` 实际产物验证通过（generated C# 1095 行、module 86 行、source map 映射成功）。 |
