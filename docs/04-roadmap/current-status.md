...
公共 API 冻结审查基线已建立，详见[1.0 公共 API 冻结审查](../03-guides/public-api-freeze.md)。当前包名、命名空间、`AddJazor*` / `UseJazor*` 扩展面和配置模型没有计划中的重命名；但在发布候选 ref 上完成 API 兼容性基线、全部质量门禁、SPA/SSR 消费者门禁和 CHANGELOG 证据前，仍不将 1.0 标记为可发布。冻结状态必须由一组可追溯的门禁结果和对应 `1.0.0-rc.1` 或 `1.0.0` CHANGELOG 条目共同确认。
