...
| P0：Debug/HMR/SPA/SSR 交付 | 2026-09-12 `v0.60.0` 发布候选已全阶段通过：API、质量门禁、隔离 NuGet 包、Release bundle、`/docs` PathBase、Chrome SPA 与 `/todo` SSR consumer 均成功；归档证据见 `artifacts/rc-v060/report.md`，长期 `wiki-verify` 与 `quality-gates` 额外上传 `toolchain-matrix.md`，关联实际 SDK、Node、Chrome、操作系统和 commit。 |
...
| P1：范式级调试 | `inspect-razorvue-chain.cs` 输出 source → generated → module → map 链路，支持文本、JSON、SARIF 2.1.0 和 schema `1.0` remediation 报告；映射断裂时以非零退出码失败，并在两种机器输出中保留 `JAZORVGA020`/`JAZORVGA026`、HelpLink、作者源位置和最小替代建议。2026-09-11 使用 `RazorVue.TodoList/TodoApp.razor` 实际产物验证通过（generated C# 1095 行、module 86 行、source map 映射成功）。 |
