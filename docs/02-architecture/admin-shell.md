...
库负责壳层结构和类型化交互，例如布局、导航、路由目标、受控折叠状态和通用 slot。表单、认证、业务页面、权限模型、应用配置和具体 UI 库组合都归属应用层，位于该包的职责之外。

## 示例应用的职责

`samples/JazorAdmin` 是 dogfood 示例应用，用于验证管理壳库在真实 RazorVue、Vue Router、样式和浏览器环境中的组合效果。它可以包含业务功能和特定 UI 库选择，而这些内容并不构成 `Jazor.Admin` 的公共 API 承诺。

文档或发布说明引用两者时，应明确标注“库行为”与“示例行为”；架构说明以库的公开契约为依据。
