...
`Jazor.Emit` 消费编译器和 RazorVue 生成的 catalog，不参与 C# 或 Razor 语义降低。它负责程序集读取、模块收集、确定性文件物化、manifest 维护、本地库资源复制与浏览器打包。

| 输出 | 所属组件 | 说明 |
| --- | --- | --- |
| ESTree、模块文本与 source-map carrier | `Jazor.Compiler` | 编译语义和来源锚点 |
| Vue render catalog | `Jazor.RazorVue` | Razor 组件的内存产物描述 |
...
`JazorDir` 默认指向 `$(MSBuildProjectDirectory)\wwwroot\jazor\`。应用启用 `JazorSsrEnabled=true` 时，release 产物还会保留用于服务器渲染的原始模块图；该图与优化后的浏览器 bundle 分开维护。
