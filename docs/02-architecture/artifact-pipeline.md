...
二者并列，都是 Emit 的一等输入。`ModuleCatalog` 承载生成 Jazor 模块，`manifest.json + dist` 承载已有 JavaScript 资源。Emit 在内存中归一化二者，得到统一的资源记录；该记录归属本次物化过程。
...
- 类库资源以 `ModuleCatalog` 或 `manifest.json + dist/**` 进入 Emit；provider、descriptor 与 catalog 专名属于各自的 metadata owner。
- `ArtifactCatalog`、`RuntimeProviderCatalog` 和 source-map catalog 的数据分别归属 `ModuleCatalog`、manifest/dist 或纯编译期 metadata。
