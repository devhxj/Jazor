...
Jazor analyzer DLL 不得放入 NuGet 自动导入的 `analyzers/dotnet/cs`。它们位于 package 的 `tools/net11.0/analyzers/`，只能由直接引用时导入的 `build/` target 注册给 Roslyn；这样普通程序集/NuGet 依赖不会意外执行 analyzer、generator 或 Emit。
...
B 如果没有自己的 Jazor 输入，不需要为消费 A 而直接引用 Jazor 工具；如果 B 也编写纯 Jazor 源码，B 才直接引用 Jazor，并只生成 B 自己的 `ModuleCatalog`。A 的 catalog 通过程序集依赖到达 Console，不被 B 改写。
...
B 不复制 A 的 `dist`，A 的 manifest 也不会转化为 B 的 ModuleCatalog。缺失 A 的 manifest、版本不满足或入口冲突必须在 Emit 预检阶段失败。
...
Emit 的内部记录必须保留：owner、library/version、logical id/specifier、`type`、相对路径、内容/hash、profile 路径、source map 关联和显式依赖。内部记录本身不作为持久化 carrier。
...
`type` 不能写成 `js-resource`、`modulecatalog`、`clr`、`runtime-provider` 或任何项目/生成器名称。manifest 的 `imports[*]` 是 `module`，`requires` 只表达 library 版本约束；未来新增资源语义时在同一条目结构中增加新的 `type` 和必需字段，不另造 carrier。
...
编译期 `RouteCatalog`、source text registry 等只供分析/生成使用，不属于资源 carrier，也不进入 Emit resource closure。

## Package 和输出边界

- JS resource package 的包根是 `manifest.json + dist/**`；style、license 和 static 文件由 manifest 明确声明。
- 纯 Jazor package 通过程序集携带 `Jazor.Generated.ModuleCatalog`；下游无法对其 generated module 内容重新 lowering。
- `jazor-manifest.json`、import map、bundle、SSR runner、HMR snapshot 是最终输出层的投影，只描述本次选中的闭包，不作为下一次构建的 producer 输入。
...
3. 最终宿主只物化 root 闭包，未选中的 module、map、style、license 和 static 不会因程序集引用被全量复制。
